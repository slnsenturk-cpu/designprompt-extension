...
Product screenshots as the primary visual — real UI in context. Rounded corners, soft shadow. Light screenshots on light sections, dark on dark.
...
2 split-column sections — alternate text/visual side each row for scroll rhythm. **Typography**
...
Section: SENTINEL Our platform helps teams ship faster than ever before. — 2 image layers: Layer 1 (z:0, static): img — "SENTINEL Our platform helps teams ship faster than ever before." 425×416px Layer 2 (z:0, static): img — "SENTINEL Our platform helps teams ship faster than ever before." 90×117px Use position:relative on container, position:absolute on overlay layers. **Spacing & rhythm**
container max-width: 1200px. card gap: 32px. **Spacing system:** 4px base grid (64% conformity). Common values: 4px, 8px, 12px, 16px, 32px, 48px, 80px. Use multiples of 4px for ALL spacing — padding, gap, margin. Exceptions: 1px, 2px, 6px, 66px. **Component language**
...
- **H3 — feature titles, card headings:** `16px/700`, "RoundHog"
...
- Active animations:
 - `0.399773s ease-out forwards rough-notation-dash`
 - `0.400227s ease-out 0.399773s forwards rough-notation-dash`
 - `0.220329s ease-out forwards rough-notation-dash`
 - `0.170593s ease-out 0.220329s forwards rough-notation-dash`
...
**Section 1: hero** — [SAMPLE COPY] "SENTINEL Our platform helps teams ship faster than ever before." [statement · technical · ~10 words]
 ...
 Background: #e5e7e0
 Layout: 2-column split — left 43% text+visual, right 43% text+visual
 Hero CTA size: `268×54px`, radius: `8px`, padding: `0px`. NOTE: Much larger than nav button — treat as wide feature card, not a compact pill.
 Floating illustration cards (1 elements, scattered around hero — `position:absolute`, decorative):
 - 268×52px bg:`#eb9d2a` pos:center-bottom radius:`8px` (3D-rotated) text:"SENTINEL Our platform helps teams ship faster than ever before."
...
- H3: `16px/24px/700`, font "RoundHog"
- Body: `18px/27px/500`, font "RoundHog" **Spacing:**
- Container max-width: `1200px`
- Card gap: `32px` **Gradients:**
