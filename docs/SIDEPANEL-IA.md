...
Header (sabit, 60px, iki satır):
   satır 1:  [logo 24px]                                   [avatar | Sign in]
...
- **Header ölçüleri:** toplam 60px, iki satır. Satır 1: wordmark 24px yüksekliğinde, solda (giriş yapılmışken aynı satırdaki avatar da 24px — ikisi aynı yükseklikte); hesap kontrolü sağda; ikisinin merkez çizgisi aynı. Satır 2: domain 13px `body` + durum `caption`, **tek satır**, taşarsa ellipsis. Header'ın üstünde/altında başka satır yoktur.
