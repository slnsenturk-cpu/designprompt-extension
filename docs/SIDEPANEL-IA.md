...
```
Header (sabit, 68px, iki satır):
   satır 1:  [logo 32px]                                   [avatar | Sign in]
...
- **Header ölçüleri:** toplam 68px, iki satır. Satır 1: wordmark 32px yüksekliğinde, solda — satırın yüksekliğini wordmark belirler (hesap kontrolü 24px avatar / 28px düğme, daha kısa); hesap kontrolü sağda; ikisinin merkez çizgisi aynı. Satır 2: domain 13px `body` + durum `caption`, **tek satır**, taşarsa ellipsis. Header'ın üstünde/altında başka satır yoktur.
