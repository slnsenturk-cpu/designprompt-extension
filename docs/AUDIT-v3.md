...
| **Transparent bordered cards are not matched** | `cardStyles` is `null` on **all three** real captures — rig.ai, posthog.com and the VibeDesign dashboard — despite every one of them clearly having cards (`hoverStates` proves it). The Cards block is omitted everywhere | Card detection that accepts a transparent background with a visible border |
| **Inputs are not matched** | `inputStyles` is `null` on posthog.com and the dashboard, both of which have forms. The Inputs block is omitted | Input detection that does not depend on a solid background |
