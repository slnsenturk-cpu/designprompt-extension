...
| `ui-components.js` | 624 | **New in v3.** The panel's component library and screen renderers (docs/SIDEPANEL-IA.md §4, §8). PURE — model in, HTML out, no chrome and no state — which is what lets every screen be rendered and asserted in Node | `VD_UI`, `VD_VIEWS`, `module.exports` | `ui-helpers.js`, tests |
| `ui-helpers.js` |     1933 | The panel's wiring: state, chrome calls, event delegation, tab switching, the Settings tab, analysis flow. Everything impure. Rebuilt in v3 against docs/SIDEPANEL-IA.md — was:  app template, provider/model settings, history, auth pill, usage counter, session status, script injection, dev tools | top-level fns + `state` | `popup.js`, `sidepanel.js` |
