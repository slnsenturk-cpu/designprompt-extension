...
(function (global) {
  ...
  async function preflightDirectAuth(provider) {
    ...
    try {
      ...
      console.warn('[vd-auth] direct auth preflight: HTTP ' + res.status + ' ' + body.slice(0, 200));
  ...
  async function _postVerify(cfg, tokenHash, type) {
    var res = await fetch(cfg.SUPABASE_URL + '/auth/v1/verify', {
      method: 'POST',
      headers: {
  ...
  // The identity behind a fresh session. GoTrue returns `user` with the
  // tokens; when it does not, /auth/v1/user with the new access token does.
  async function _identityFor(cfg, sess) {
    if (sess.user && sess.user.email) return sess.user;
    var res = await fetch(cfg.SUPABASE_URL + '/auth/v1/user', {
      method: 'GET',
