...
(function (global) {
  ...
  async function exchangeTokenHash(tokenHash, type) {
    ...
    if (typeof tokenHash !== 'string' || tokenHash.length < 16 || tokenHash.length > 512) {
      return { ok: false, error: 'bad-token-hash' };
    }
    try {
      ...
      if (!res.ok) {
        ...
        console.warn('[vd-auth] token hash rejected', res.status);
        return { ok: false, error: 'verify-' + res.status,
                 message: (detail && (detail.error_description || detail.msg)) || null };
      }
      ...
      return { ok: true, session: sess,
               signedInAs: (sess.user && sess.user.email) || null };
    } catch (e) {
      console.warn('[vd-auth] token exchange threw', e && e.message);
