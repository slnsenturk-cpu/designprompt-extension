...
(function (global) {
  ...
  async function _doRefresh() {
    try {
      ...
      if (!res.ok) {
        // A 4xx here is usually "Invalid Refresh Token: Already Used", which
        // means the stored token was spent by someone else. Deliberately do
        // NOT clear the session: re-peek instead, and if the stored token has
        // moved on, another refresh already succeeded and we're simply late.
