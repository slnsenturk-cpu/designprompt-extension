...
const VD_DOWNLOAD = (() => {
  ...
  // DESIGN-<slug>.md for an image, from the label the user gave it. The
  // active tab is never consulted: an image has no site.
  function imageDesignMdFilename(label) {
    const slug = String(label || '')
      .normalize('NFKD').replace(/[\u0300-\u036f]/g, '')
      .toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-+|-+$/g, '').slice(0, 48);
    return `DESIGN-${slug || 'image'}.md`;
  }
