...
const VD_TOKENS = (() => {
  ...
  // Every easing found in one transition shorthand. Returns [] when there is
  // none — an omitted easing means the CSS default, not a missing measurement.
  //
  // Deliberately NOT gated on the model's timing(): that is a DISPLAY guard
  // capped at 90 characters, and a real Tailwind-style transition shorthand
  // runs past 200 ("color .15s cubic-bezier(…), background-color .15s
  // cubic-bezier(…), …"). Gating on it silently threw away posthog's and the
  // dashboard's button curves. What actually keeps page copy out is the
  // extraction itself: a cubic-bezier match yields four numbers, and a keyword
  // match comes from a fixed allowlist. Neither can carry a sentence.
  function easingsIn(raw) {
    const out = [];
    if (typeof raw !== 'string' || raw.length > 600) return out;
    splitTopLevel(raw).forEach(part => {
      const bez = part.match(/cubic-bezier\([^)]*\)/i);
      if (bez) { out.push(bez[0].replace(/\s+/g, '')); return; }
  ...
  // ── naming an easing ────────────────────────────────────────────────────
  // Ordinals (easing-1, easing-2) are worthless: they change meaning whenever
  // a page adds a transition, and they tell a reader nothing about when to
  // reach for the curve. Digit-derived names (easing-04-0-02-1) are worse —
  // they read as noise and are unusable as a Tailwind token.
  //
  // So a curve is named for its ROLE. First choice is the component it was
  // measured on, which is a fact about this page. Failing that, the curve's
  // own character, which is a fact about the curve.
  function curveCharacter(easing) {
    const bez = easing.match(/^cubic-bezier\(([^)]*)\)$/i);
    if (!bez) return easing.replace(/^ease-?/, '') || 'standard';
    const v = bez[1].split(',').map(n => parseFloat(n));
    if (v.length !== 4 || !v.every(Number.isFinite)) return 'standard';
    const [x1, y1, x2, y2] = v;
    // Leaving the origin flat and braking into the end: decelerate. The
    // reverse: accelerate. Both ends eased: standard. Overshoot past 1: back.
    const easesIn = x1 > 0.01 || y1 > 0.01;
    const easesOut = x2 < 0.99 || y2 < 0.99;
    if (y1 > 1 || y2 > 1) return 'overshoot';
    if (!easesIn && easesOut) return 'decelerate';
    if (easesIn && !easesOut) return 'accelerate';
    if (easesIn && easesOut) return 'standard';
    return 'linear';
  }

  // easing value → role name, for every easing observed on the page.
  function easingRoles(t) {
    const roles = new Map();          // easing → name
    const claim = (easing, name) => { if (easing && !roles.has(easing)) roles.set(easing, name); };

    // Component attribution first — these are the curves a reader will most
    // want by name, and the page told us where each one lives.
    const buttons = (t.buttonStyles && typeof t.buttonStyles === 'object') ? t.buttonStyles : {};
    Object.keys(buttons).forEach(k => {
      const b = buttons[k];
      if (b && b.transition) easingsIn(b.transition).forEach(e => claim(e, 'button'));
    });
    if (t.cardStyles && t.cardStyles.transition) {
      easingsIn(t.cardStyles.transition).forEach(e => claim(e, 'card'));
    }
    if (t.inputStyles && t.inputStyles.transition) {
      easingsIn(t.inputStyles.transition).forEach(e => claim(e, 'field'));
    }

    // Everything else, by curve character. A character already taken by a
    // component curve falls back to the next unused character-based name
    // rather than silently overwriting it.
    const seen = new Set(roles.values());
    (Array.isArray(t.transitions) ? t.transitions : []).forEach(raw => {
      easingsIn(raw).forEach(e => {
        if (roles.has(e)) return;
        let name = curveCharacter(e);
        if (seen.has(name)) {
          // Two distinct curves of the same character. Rather than append a
          // digit, qualify by where it sits in the timeline.
          const alt = name === 'standard' ? 'emphasized' : name + '-soft';
          name = seen.has(alt) ? name + '-alt' : alt;
        }
        seen.add(name);
        roles.set(e, name);
      });
  ...
  // A transition is a shorthand — "color 0.3s cubic-bezier(.25,1,.5,1) 0.1s" —
  // so duration and easing have to be picked out of it. timing()/duration()
  // validate; they do not parse a shorthand.
  function motionTokens(t) {
    const out = {};
    // Same reasoning as easingsIn: duration() returns "<n>ms" or nothing, so
    // the parse is its own guard and the 90-char display cap does not belong.
    const transitions = (Array.isArray(t.transitions) ? t.transitions : [])
      .filter(v => typeof v === 'string' && v.length <= 600);
    ...
    const roles = easingRoles(t);
    [...roles.keys()].sort().forEach(e => {
      const name = 'ease-' + roles.get(e);
      ...
      if (bez) {
        ...
        if (vals.length === 4 && vals.every(v => Number.isFinite(v))) {
          out[name] = { $value: vals, $type: 'cubicBezier' };
          return;
        }
      }
      // CSS keywords are not cubicBezier values; typing them as such would be
      // a lie a consumer cannot detect. See README on $type "other".
      out[name] = { $value: e, $type: 'other', $description: 'CSS easing keyword' };
    });
    return out;
  }
