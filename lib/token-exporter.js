...
function exportW3CTokens(data) {
  ...
  // ── Shadows — W3C composite object format, semantic role labels when available ──
  const shadow = {};
  const _shadowRoles = (typeof inferShadowRoles === 'function') ? inferShadowRoles(data) : new Map();
  (data.shadows || []).slice(0, 4).forEach((s, i) => {
    // Parse shadow string into W3C composite structure
    const parsed = _parseShadow(s);
    if (parsed) {
      // Role > inset > elevation-N — semantic labels (card-resting, button-primary)
      // travel further than anonymous elevation ordinals when the token is consumed
      // by a design-system tool.
      const role = _shadowRoles.get(s.replace(/\s+/g, ' ').trim());
      const label = role || (s.includes('inset') ? `inset-${i + 1}` : `elevation-${i + 1}`);
      shadow[label] = parsed;
    }
  });
...
function _parseShadow(s) {
  ...
  // Extract color first — must cover every modern CSS color function that
  // getComputedStyle may emit, not just rgb/hex/oklch. Missing any one of these
  // dropped the color silently (exported as null), so the shadow lost its tint.
  const colorMatch = s.match(/#[0-9a-f]{3,8}|(?:rgba?|hsla?|hwb|lab|lch|oklab|oklch|color)\([^)]+\)/i);
