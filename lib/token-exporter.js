...
function exportW3CTokens(data) {
  ...
  // Primary action — use same resolution as prompt-builder: button bg > CSS var > accent
  const btnBg = data.buttonStyles?.primary?.backgroundColor;
  const _btnBgValid = btnBg && /^#[0-9a-f]{6,8}$/i.test(btnBg.trim());
  const primaryHex = _btnBgValid ? btnBg
    : (sc.primary && /^#[0-9a-f]{6}$/i.test(sc.primary)) ? sc.primary
    : (data.accentColors || [])[0] || null;
