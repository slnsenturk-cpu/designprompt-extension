...
async function handleAnalyze() {
  ...
  if (state.mode === 'element') {
    try {
      await injectPickerScript(tab.id);
  ...
  if (state.mode === 'image') {
    if (state.capturedImageData) {
      // Image already captured — analyze it
      try {
    ...
    try {
      ...
      if (state.imagePickerActive) {
        await chrome.tabs.sendMessage(tab.id, { type: 'DEACTIVATE_IMAGE_PICKER' });
        state.imagePickerActive = false;
      } else {
        await chrome.tabs.sendMessage(tab.id, { type: 'ACTIVATE_IMAGE_PICKER' });
