...
function computeModelNotice(provider) {
  ...
  if (resolved.fellBack && saved) {
    const fallback = allModels.find(m => m.id === resolved.id);
    return { type: 'fallback', text: `"${saved}" is no longer available — using ${fallback ? fallback.label : resolved.id}.` };
  }
  ...
  if (noticeEl) {
    ...
    if (!notice) {
      ...
      if (notice.type === 'nudge') {
        noticeEl.innerHTML = `<span>${notice.text}</span> — <button class="link-btn" id="switchModelBtn" type="button">Switch</button>`;
        $('switchModelBtn').addEventListener('click', () => selectModel(notice.newerId));
      } else {
...
function setupListeners() {
  $('settingsToggle').addEventListener('click', async () => {
    ...
    if (opening) refreshLiveModelsForCurrentProvider();
  });
...
async function initUI() {
  ...
  if (settings_panel_open) refreshLiveModelsForCurrentProvider();
}
