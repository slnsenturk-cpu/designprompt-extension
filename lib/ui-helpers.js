...
// Builds the document for a tier, or null when the data/builder isn't ready.
function buildDesignMdForTier(tier) {
...
function copyDesignMd(tier) {
  const md = buildDesignMdForTier(tier);
  if (!md) return;
  copyToClipboard(md);
  flashDevButton(tier === 'pro' ? 'devDesignMdProBtn' : 'devDesignMdFreeBtn', md.length);
}

function downloadDesignMd(tier) {
  const md = buildDesignMdForTier(tier);
  if (!md) return;
  if (!self.VD_DOWNLOAD || typeof self.VD_DOWNLOAD.downloadText !== 'function') {
    console.warn('[vd-dev] download helper not loaded');
    return;
  }
  const sourceUrl = (state.lastAnalyzedData && state.lastAnalyzedData.url) || state.currentUrl || '';
  const name = self.VD_DOWNLOAD.designMdFilename(sourceUrl, tier);
  const btn = tier === 'pro' ? 'devDlProBtn' : 'devDlFreeBtn';
  if (self.VD_DOWNLOAD.downloadText(md, name)) flashDevButton(btn, md.length);
}
...
function copyRawTokens() {
  ...
  flashDevButton('devTokensJsonBtn', json.length);
}

function flashDevButton(id, chars) {
  const btn = $(id);
  if (!btn) return;
  const original = btn.textContent;
  btn.textContent = `✓ ${(chars / 1000).toFixed(1)}k chars`;
  setTimeout(() => { if (document.contains(btn)) btn.textContent = original; }, 1800);
}
...
function setupListeners() {
  ...
  if (isUnpackedBuild()) {
    const devRow = $('devToolsRow');
    if (devRow) devRow.style.display = 'flex';
    const dfree = $('devDesignMdFreeBtn'), dpro = $('devDesignMdProBtn'), dtok = $('devTokensJsonBtn');
    if (dfree) dfree.addEventListener('click', () => copyDesignMd('free'));
    if (dpro) dpro.addEventListener('click', () => copyDesignMd('pro'));
    const dlFree = $('devDlFreeBtn'), dlPro = $('devDlProBtn');
    if (dlFree) dlFree.addEventListener('click', () => downloadDesignMd('free'));
    if (dlPro) dlPro.addEventListener('click', () => downloadDesignMd('pro'));
...
const APP_TEMPLATE = `
  ...
  <section class="result-section" id="resultSection" style="display:none" aria-label="Analysis Result">
    ...
    <div class="dev-tools-row" id="devToolsRow" style="display:none" aria-label="Developer tools">
      <span class="dev-tools-label">dev</span>
      <button class="btn btn-sm btn-ghost" id="devDesignMdFreeBtn">Copy DESIGN.md (free)</button>
      <button class="btn btn-sm btn-ghost dev-dl" id="devDlFreeBtn" title="Download DESIGN.md (free)">↓</button>
      <button class="btn btn-sm btn-ghost" id="devDesignMdProBtn">Copy DESIGN.md (pro)</button>
      <button class="btn btn-sm btn-ghost dev-dl" id="devDlProBtn" title="Download DESIGN.md (pro)">↓</button>
      <button class="btn btn-sm btn-ghost" id="devTokensJsonBtn" title="The full extractor output — NOT the W3C token export">Copy RAW capture (dev)</button>
    </div>
