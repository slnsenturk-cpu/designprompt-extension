...
async function savePrompt(url, prompt, source, platform, focus) {
  ...
  // Authed users are no-op'd inside VD_USAGE.incrementUsage itself. Failures
  // here must not break the core save flow.
  try {
    if (self.VD_USAGE && typeof self.VD_USAGE.incrementUsage === 'function') {
      await self.VD_USAGE.incrementUsage();
    }
  } catch (e) { console.warn('[vd-usage] post-save increment failed', e); }
}
...
async function _vdRestoreCloudPrompt(analysis) {
  try {
    ...
    const promptText = (mostRecent && mostRecent.content) || '';
    if (!promptText) {
