...
function switchToNewestModel() {
  ...
  if (notice && notice.modelId) selectModel(notice.modelId);
}

function dismissCurrentNudge() {
  const notice = computeModelNotice(state.provider);
  if (notice && notice.modelId) dismissModelNudge(notice.modelId);
}
...
function renderSettingsPanel() {
  ...
  const aiBody = [
    `<label class="vd-toggle"><input type="checkbox" id="vdAiToggle"${on ? ' checked' : ''} />`
    + '<span class="vd-toggle__track" aria-hidden="true"></span>'
    + '<span class="vd-toggle__label">AI enhancement</span></label>',
  ...
  if (on) {
    ...
    const notice = computeModelNotice(state.provider);
    if (notice && notice.type === 'newer' && !state.dismissedNudges[notice.modelId]) {
      aiBody.push(U.notice({
        id: 'vdModelNudge', tone: 'info',
        text: `Newer model available: ${notice.label}`,
        action: 'Switch', actionId: 'switchModel',
        dismissId: 'dismissNudge',
      }));
    }
  }
