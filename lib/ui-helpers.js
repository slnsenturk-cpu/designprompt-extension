...
async function handleAnalyze() {
  ...
  try {
    ...
    // Ensure URL is resolved: content.js URL → tab URL → state URL → title fallback
    const _contentUrl = _pageData.url || '';
    const _isBlank = !_contentUrl || _contentUrl === 'about:blank' || _contentUrl === 'about:srcdoc';
    const _isEditorUrl = /aura\.build\/editor|figma\.com\/make|replit\.com\/@|v0\.dev\/chat/i.test(_contentUrl);
    if (_isBlank || _isEditorUrl) {
      // Try tab URL, but also check if IT is an editor URL
      const _tabUrl = tab.url || state.currentUrl || '';
      if (_tabUrl && !/aura\.build\/editor|figma\.com\/make/i.test(_tabUrl)) {
        _pageData.url = _tabUrl;
      } else {
        // Last resort: use page title as identifier
        _pageData.url = _pageData.title || 'unknown';
      }
    }
