...
const APP_TEMPLATE = `
  <header class="vd-header">
    <img src="icons/logo-light.png" alt="VibeDesign" class="vd-header__logo" width="317" height="108" />
...
function renderTabBar() {
  ...
  slot.innerHTML = VD_UI.tabBar({
    active: state.tab, ready: analysisReady(),
    // §3: signed-out is advertised by a dot on the Settings tab and nowhere
    // else — there is no floating sign-in pill.
    signedOut: state.account ? !state.account.authed : false,
  });
}
...
function renderSettingsPanel() {
  ...
  parts.push(U.section({ title: 'Account', body:
    (acct.authed
      ? U.kvRow({ label: acct.email || 'Signed in', value: '', mono: false })
        + U.button({ id: 'vdSignOut', variant: 'ghost', label: 'Sign out' })
      : `<p class="vd-line">${U.esc(U.COPY.signInForHistory)}</p>`
        + U.button({ id: 'vdSignIn', variant: 'ghost', label: 'Sign in' }))
...
async function refreshAccount() {
  const before = JSON.stringify(state.account || null);
  try {
    const auth = self.VD_AUTH;
    const session = (auth && typeof auth.peekSession === 'function')
      ? await auth.peekSession() : null;
    const user = session && (session.user || null);
    state.account = { authed: !!(session && session.access_token), email: user && user.email };
  } catch (e) {
    // Never let an auth hiccup take the panel down: unknown reads as anonymous.
    state.account = { authed: false, email: null };
  }
  if (JSON.stringify(state.account) !== before) renderPanel();
}
...
async function signIn() {
  ...
  } catch (e) { console.warn('[vd-ui] sign in failed', e); }
  await refreshAccount();
}
    ...
    primary.addEventListener('click', async () => {
      try {
        ...
        if (self.VD_AUTH && typeof self.VD_AUTH.openAuthFlow === 'function') {
          ...
          if (r && r.ok) await refreshAccount();
        }
