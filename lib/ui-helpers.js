...
function renderProviderUI() {
  ...
  if (cfg.models.length > 0) {
    ...
    const allModels = getModelsForProvider(state.provider);
    const expanded = !!state.expandedModelList[state.provider];
    const shown = expanded ? allModels : allModels.slice(0, 4);
    let html = '<div class="model-row-label"><span class="field-label">Model</span></div>'
      + shown.map(m => {
    ...
    if (allModels.length > 4) {
      html += `<button class="link-btn" id="showAllModelsBtn" type="button">${expanded ? 'Show fewer models' : 'Show all models'}</button>`;
    }
    modelRow.innerHTML = html;
    modelRow.querySelectorAll('[data-model]').forEach(btn =>
      btn.addEventListener('click', () => selectModel(btn.dataset.model)));
    const showAllBtn = $('showAllModelsBtn');
    if (showAllBtn) {
      showAllBtn.addEventListener('click', () => {
        state.expandedModelList[state.provider] = !expanded;
        renderProviderUI();
      });
    }
  } else {
  ...
  const noticeEl = $('settingsModelNotice');
  if (noticeEl) {
    ...
    if (!notice) {
      ...
      if (notice.type === 'nudge') {
        ...
        switchBtn.addEventListener('click', () => selectModel(notice.newerId));
        const dismissBtn = document.createElement('button');
        dismissBtn.className = 'link-btn';
        dismissBtn.type = 'button';
        dismissBtn.textContent = 'Dismiss';
        dismissBtn.addEventListener('click', () => dismissModelNudge(notice.newerId));
        noticeEl.append(switchBtn, ' · ', dismissBtn);
      }
    }
  }
...
function downloadDesignMd() {
  ...
  if (self.VD_DOWNLOAD.downloadText(md, name)) flashButton('downloadDesignBtn', 'Saved');
}
...
function downloadSkillBundle() {
  ...
  if (missing.length) {
    ...
    flashButton('downloadSkillBtn', 'Unavailable');
    return;
  }
  let version = 'dev';
  try { version = chrome.runtime.getManifest().version; } catch (e) { /* keep dev */ }
  const sourceUrl = state.lastAnalyzedData.url || state.currentUrl || '';

  try {
    const model = self.VD_MODEL.buildDesignModel(state.lastAnalyzedData, { sourceUrl: sourceUrl });
    const bundle = self.VD_SKILL.buildSkillBundle(model, {
      sourceUrl: sourceUrl,
      version: version,
      observedAt: new Date().toISOString().slice(0, 10),
    });
    const bytes = self.VD_SKILL.zipSkillBundle(bundle, self.VD_ZIP);
    if (!bytes) { flashButton('downloadSkillBtn', 'Failed'); return; }
    const ok = self.VD_DOWNLOAD.downloadBytes(bytes, bundle.slug + '.zip', 'application/zip');
    flashButton('downloadSkillBtn', ok ? 'Saved' : 'Failed');
  } catch (e) {
    // A broken export must not take the result panel down with it.
    console.warn('[vd-ui] skill bundle failed', e);
    flashButton('downloadSkillBtn', 'Failed');
  }
}
...
function setupListeners() {
  ...
  const dlBtn = $('downloadDesignBtn');
  if (dlBtn) dlBtn.addEventListener('click', downloadDesignMd);
  const skillBtn = $('downloadSkillBtn');
  if (skillBtn) skillBtn.addEventListener('click', downloadSkillBundle);
  if (isUnpackedBuild()) {
    const devSection = $('settingsDev');
    if (devSection) devSection.style.display = 'block';
    const dtok = $('devTokensJsonBtn');
    if (dtok) dtok.addEventListener('click', copyRawTokens);
  }
  document.querySelectorAll('.output-tab').forEach(t =>
    t.addEventListener('click', () => setOutputMode(t.dataset.output)));
  $('resetBtn').addEventListener('click', resetView);
  // lib/token-exporter.js has no control of its own: tokens.json ships inside
  // the skill bundle, so a separate JSON button would be a second door onto
  // the same file.
...
const APP_TEMPLATE = `
  ...
  <section class="settings-panel" id="settingsPanel" aria-label="AI Provider Settings">
    ...
    <div class="settings-model-notice" id="settingsModelNotice" style="display:none"></div>
    ...
    <div class="settings-session-status" id="vdSessionStatus" style="display:none"></div>
    <div class="settings-dev" id="settingsDev" style="display:none">
      <div class="settings-title settings-title--sub">Developer</div>
      <button class="btn btn-sm btn-ghost" id="devTokensJsonBtn" title="The full extractor output — not the W3C token export">Copy RAW capture</button>
      <div class="settings-note">Unpacked build only. Paste into tests/fixtures/ to add a fixture.</div>
    </div>
  </section>
  ...
  <!-- Usage counter (v2.0.0-beta.2) — anonymous users only, hidden when authed -->
  <div id="vd-usage-container"></div>
  ...
  <section class="result-section" id="resultSection" style="display:none" aria-label="Analysis Result">
    ...
    <div class="result-header">
      ...
      <div class="result-actions">
        <button class="btn btn-sm btn-ghost" id="copyBtn" aria-label="Copy prompt to clipboard"><span id="copyIcon" aria-hidden="true">⎘</span> <span class="btn-label">Copy prompt</span></button>
        <button class="btn btn-sm btn-ghost" id="downloadDesignBtn" aria-label="Download DESIGN.md"><span aria-hidden="true">↓</span> <span class="btn-label">Download DESIGN.md</span></button>
        <button class="btn btn-sm btn-ghost" id="downloadSkillBtn" aria-label="Download the design skill bundle as a zip"><span aria-hidden="true">↓</span> <span class="btn-label">Download Skill (zip)</span></button>
        <button class="btn btn-sm btn-ghost btn-icon-only" id="resetBtn" aria-label="Reset and start over" title="Start over">↺</button>
      </div>
    </div>
...
async function initUI() {
  ...
  const stored = await chrome.storage.local.get(['provider','apiKeys','dp_pending','selectedModels','savedModelLabels','savedFocus','vd_nudge_dismissed']);
  ...
  // Update the moment the worker records a refresh, rather than waiting for
  // the next tick.
  try {
    chrome.storage.onChanged.addListener((changes, area) => {
      if (area === 'local' && changes.vd_last_refresh) renderSessionStatus();
    });
...
async function _vdRestoreCloudPrompt(analysis) {
  try {
    ...
    showResult(promptText, { url }, 'page', null, true, null);
  } catch (e) {
