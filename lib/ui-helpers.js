...
// ── STATE ─────────────────────────────────────────────────────────────────
let state = {
  // §4.6: 'website' | 'image'. A source type, not a third mode — the two have
  // different content structures and the tab bar is the same for both.
  source:'website', image:null,
  mode:'page', focus:'all',
  pickerActive:false,
  lastPrompt:'',
...
async function _renderLocalHistory(list) {
  ...
  items.slice(0, MAX_HISTORY).forEach(item => {
    ...
    let iconSpan;
    if (item.sourceType === 'image' && item.image && item.image.dataUrl) {
      iconSpan = document.createElement('img');
      iconSpan.className = 'history-item-icon history-item-thumb';
      iconSpan.src = item.image.dataUrl;
      iconSpan.alt = '';
    } else {
...
function restorePrompt(item) {
  ...
  if (item.sourceType === 'image') {
    ...
    if (state.image && !state.image.label) state.image.label = item.domain;
    state.lastAnalyzedData = { styleProfile: item.styleProfile, url: '', name: item.domain, sourceType: 'image' };
    if (!outputsForSource().some(o => o.value === state.output)) state.output = outputsForSource()[0].value;
    showResult(item.prompt, state.lastAnalyzedData, 'image', item.provider, true);
    return;
  }
  if (state.source === 'image') { state.source = 'website'; state.image = null; }
...
// ── IMAGE SOURCE (§4.6) ─────────────────────────────────────────────────
// The image never leaves this browser except to the provider the user chose.
// It is held in memory and, once analyzed, in chrome.storage.local next to
// the history entry it belongs to. Nothing here calls VD_CLOUD.

const IMAGE_TYPES = { 'image/png': 1, 'image/jpeg': 1, 'image/webp': 1 };
const IMAGE_MAX_BYTES = 8 * 1024 * 1024;

function setSource(source) {
  const next = source === 'image' ? 'image' : 'website';
  if (next === state.source) return;
  state.source = next;
  chrome.storage.local.set({ vd_source: next });
  // Output follows the source: a Skill choice has nowhere to go in Image mode.
  if (!outputsForSource().some(o => o.value === state.output)) state.output = outputsForSource()[0].value;
  resetView();
}

// Website mode works without a key; Image mode cannot. Both facts are the
// same check.
function imageAnalysisAvailable() {
  return !!(state.aiEnabled && state.provider !== 'none' && state.apiKeys[state.provider]);
}

function chooseImage(file) {
  if (!file) return;
  if (!IMAGE_TYPES[file.type]) { showNotice('PNG, JPG or WebP only'); return; }
  if (file.size > IMAGE_MAX_BYTES) { showNotice('Image is over 8 MB'); return; }
  const reader = new FileReader();
  reader.onload = () => {
    state.image = { name: file.name, type: file.type, size: file.size, dataUrl: String(reader.result || ''),
                    label: defaultImageLabel() };
    // A new image is a new subject: the previous result no longer describes it.
    state.model = null; state.lastAnalyzedData = null; state.lastPrompt = '';
    state.tab = 'overview';
    renderPanel();
  };
  reader.onerror = () => showNotice(VD_UI.COPY.imageUnreadable);
  reader.readAsDataURL(file);
}

// "Image style · 2 Sep 2026". The date is the day the image was chosen,
// which is the only thing known about it before analysis.
// Formatted by hand: toLocaleDateString gives "Sept" on some engines and
// "Sep" on others, and a label must not depend on which Chrome this is.
const MONTHS_SHORT = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
function defaultImageLabel(now) {
  const d = new Date(now === undefined ? Date.now() : now);
  return `Image style · ${d.getDate()} ${MONTHS_SHORT[d.getMonth()]} ${d.getFullYear()}`;
}

// The label input is read at the moment it matters, not on every keystroke.
function currentImageLabel() {
  const input = $('vdImageLabel');
  const typed = input && typeof input.value === 'string' ? input.value.trim() : '';
  return typed || (state.image && state.image.label) || defaultImageLabel();
}

// §4.6 "New image": back to the drop zone. The result just shown is already
// in history (saved when it was analyzed), so nothing is lost; choosing the
// next file asks for a label as it always does.
function newImage() {
  state.image = null;
  state.model = null; state.lastAnalyzedData = null; state.lastPrompt = '';
  state.analyzedAt = null; state.resultUrl = '';
  state.tab = 'overview';
  setSurface('panel');
  renderPanel();
}

// Drag-and-drop onto the zone. Wired once on the panel; the zone itself is
// re-rendered with every paint, so the listeners live one level up.
function onPanelDrag(e) {
  const zone = e.target.closest && e.target.closest('#vdDropZone');
  if (!zone || zone.classList.contains('is-gated')) return;
  e.preventDefault();
  if (e.type === 'dragover') zone.classList.add('is-over');
  if (e.type === 'dragleave') zone.classList.remove('is-over');
  if (e.type === 'drop') {
...
async function analyzeImage() {
  ...
  if (!imageAnalysisAvailable()) { setTab('settings'); return; }

  // Same cap as a website analysis: an image analysis is one generation.
  try {
    if (self.VD_USAGE && typeof self.VD_USAGE.canGenerate === 'function') {
      const allowed = await self.VD_USAGE.canGenerate();
      if (!allowed) { await refreshUsage(); setSurface('panel'); renderPanel(); return; }
    }
  } catch (_) { /* fail open */ }

  const provider = state.provider;
  const modelId = (typeof pickVisionModel === 'function')
    ? pickVisionModel(provider, getActiveModel(provider), getModelsForProvider(provider))
    : null;
  if (!modelId) { showError(`${PROVIDERS[provider].name} has no image-capable model in your list.`); return; }

  state.image.label = currentImageLabel();
  state.cancelled = false;
  try {
    showLoading(`Reading image with ${PROVIDERS[provider].name}…`);
    const text = await analyzeImageWithAI(state.image.dataUrl, provider, state.apiKeys[provider], modelId);
    if (state.cancelled) return;
    // Defensive by design: a reply that is not the profile is not an error to
    // log — it is an image to try again. Nothing about it is recorded.
    const profile = self.VD_MODEL && self.VD_MODEL.parseStyleProfile(text);
    if (!profile) { showError(VD_UI.COPY.imageUnreadable); return; }

    state.lastAnalyzedData = { styleProfile: profile, url: '', name: state.image.label, sourceType: 'image' };
    state.lastAiDirection = null;
    refreshModel();
    if (!state.model) { showError(VD_UI.COPY.imageUnreadable); return; }

    const prompt = (typeof buildImagePrompt === 'function') ? buildImagePrompt(state.model) : '';
    state.resultUrl = '';
    await saveImagePrompt(prompt, profile);
    showResult(prompt, state.lastAnalyzedData, 'image', provider);
  } catch (err) {
    showError(err && err.message ? err.message : VD_UI.COPY.imageUnreadable);
  }
}

// History entry for an image. Keyed so several images coexist (a website is
// keyed by domain; an image has none). The profile is stored so a restore can
// rebuild the model without the provider. The image itself stays local.
async function saveImagePrompt(prompt, profile) {
  const stored = await chrome.storage.local.get(HISTORY_KEY);
  let history = stored[HISTORY_KEY] || {};
  const name = state.image.name;
  const key = `image:${name}:${Date.now()}`;
  history[key] = {
    sourceType: 'image', domain: state.image.label || name, url: '', prompt, source: 'image', platform: 'generic',
    focus: 'all', provider: state.provider !== 'none' ? state.provider : null, savedAt: Date.now(),
    image: state.image, styleProfile: profile,
  };
  const entries = Object.entries(history).sort(([,a],[,b]) => b.savedAt - a.savedAt);
  if (entries.length > MAX_HISTORY) history = Object.fromEntries(entries.slice(0, MAX_HISTORY));
  await chrome.storage.local.set({ [HISTORY_KEY]: history });
  try {
    if (self.VD_USAGE && typeof self.VD_USAGE.incrementUsage === 'function') await self.VD_USAGE.incrementUsage();
    await refreshUsage();
  } catch (e) { console.warn('[vd-usage] post-save refresh failed', e); }
}
...
function showResult(prompt, data, source, providerUsed, isRestored = false) {
  ...
  else if (state.source === 'image') state.resultUrl = '';   // an image belongs to no page
...
function outputsForSource() {
  return state.source === 'image' ? VD_UI.IMAGE_OUTPUTS : VD_UI.OUTPUTS;
}

function setOutput(output) {
  if (!outputsForSource().some(o => o.value === output)) return;
  state.output = output;
...
function runExport() {
  ...
  // §4.6: no Skill from an image. Guesses must not ship as tokens.
  if (state.source === 'image') return downloadDesignMd();
...
function buildDesignMdDoc() {
  ...
  return self.VD_DESIGN_MD.buildDesignMd(state.lastAnalyzedData, {
    ...
    version: version,
    observedAt: new Date().toISOString().slice(0, 10),
    // §4.6: an image model is passed whole. The builder branches on
    // model.sourceType and never re-derives from lastAnalyzedData.
    model: state.source === 'image' ? state.model : undefined,
  });
}
...
function downloadDesignMd() {
  ...
  // An image is named by its label and nothing else — never by the tab that
  // happens to be open beside it.
  const name = state.source === 'image'
    ? self.VD_DOWNLOAD.imageDesignMdFilename((state.image && state.image.label) || (state.lastAnalyzedData && state.lastAnalyzedData.name))
    : self.VD_DOWNLOAD.designMdFilename((state.lastAnalyzedData && state.lastAnalyzedData.url) || state.currentUrl || '');
...
function setupListeners() {
  ...
  // §4.6 drop zone. Delegated, because the zone is re-rendered on every paint.
  ['dragover', 'dragleave', 'drop'].forEach(t => root.addEventListener(t, onPanelDrag));
...
function onPanelClick(e) {
  ...
  if (source) return setSource(source.dataset.source);

  const mode = hit('[data-mode]');
  if (mode) return setMode(mode.dataset.mode);

  // §4.6: the drop zone opens the picker; the button runs the analysis. Both
  // are inert while gated — the zone by pointer-events, the button by
  // disabled — so nothing here re-checks the gate; analyzeImage() does.
  if (hit('#vdDropZone')) { const input = $('vdImageInput'); if (input) input.click(); return; }
  if (hit('#analyzeImageBtn')) return analyzeImage();
...
function onPanelChange(e) {
  ...
  if (id === 'vdImageInput') return chooseImage(e.target.files && e.target.files[0]);
...
function pageContext() {
  ...
  if (state.source === 'image') return 'same';   // an image is never "another page"
...
function renderHeader() {
  ...
  if (state.source === 'image') {
    // §4.6: an image belongs to no tab. The second line names the image and
    // when it was analyzed; the active tab's domain never appears here, and
    // switching tabs cannot change it.
    const label = state.image && (state.image.label || state.image.name);
    const status = state.model ? 'Analyzed ' + shortTime(state.analyzedAt) : 'Not analyzed';
    badge.innerHTML = `<span class="vd-header__domain">${VD_UI.esc(label ? 'Image · ' + label : 'Image')}</span>`
  ...
  if (slot) {
    ...
    slot.innerHTML = VD_UI.accountControl({
      authed: acct.authed, email: acct.email, avatarUrl: acct.avatarUrl, dev: isUnpackedBuild(),
    });
  }
}
...
function viewOptions() {
  const ctx = pageContext();
  return {
    source: state.source, image: state.image,
...
// §4.6: the image source has its own screens. Same tabs, different content.
function renderImagePanel(panel, V, opts) {
  if (!state.model) { panel.innerHTML = V.imageHomeView(opts); return; }
  if (state.tab === 'overview') panel.innerHTML = V.imageOverviewView(state.model, opts);
  else if (state.tab === 'colors') panel.innerHTML = V.imageColorsView(state.model);
  else if (state.tab === 'type') panel.innerHTML = V.imageTypeView(state.model);
  else if (state.tab === 'components') panel.innerHTML = V.imageComponentsView(state.model);
  else if (state.tab === 'motion') panel.innerHTML = V.imageMotionView(state.model);
}
...
function refreshModel() {
  ...
  try {
    ...
    // An image capture is a style profile, not a page capture. It builds an
    // estimated model; there is no website path that could read it.
    if (state.source === 'image' && state.lastAnalyzedData.styleProfile) {
      state.model = self.VD_MODEL.buildModelFromStyleProfile(state.lastAnalyzedData.styleProfile, {
        name: (state.image && state.image.label) || state.lastAnalyzedData.name || '',
      });
      return;
    }
...
function renderSettingsPanel() {
  ...
  const aiBody = [
    ...
    `<p class="vd-settings__help">${U.esc(U.COPY.aiEnhancement + ' ' + U.COPY.aiRequiredForImage)}</p>`,
  ];
  if (on) {
...
async function initUI(options) {
  ...
  const stored = await chrome.storage.local.get([
    ...
    'savedFocus', 'vd_nudge_dismissed', 'vd_ai_enabled', 'vd_output', 'vd_target', 'vd_source',
  ]);
  ...
  // §4.3 Defaults: DESIGN.md. A first-time user is more likely to want the
  // file than the chat brief — the brief is what the old product did, and
  // defaulting to it would have kept 3.0 pointed at 2.x's job.
  state.output = stored.vd_output || 'design-md';
  state.target = stored.vd_target || VD_UI.TARGETS[0];
  state.source = stored.vd_source === 'image' ? 'image' : 'website';
  state.tab = 'overview';
  ...
  try {
    chrome.storage.onChanged.addListener((changes, area) => {
      ...
      if (changes.vd_last_refresh && state.tab === 'settings') renderSessionStatus();
      // The session itself changed — a handoff landed, a refresh rewrote it,
      // a sign-out cleared it. Whoever wrote it, the open panel re-reads and
      // re-renders; it does not depend on a runtime message arriving.
      const authKey = (self.VD_CONFIG && self.VD_CONFIG.AUTH_STORAGE_KEY) || 'auth_session';
      if (Object.keys(changes).some(k => k === authKey || k.indexOf('sb-') === 0 || k.indexOf('auth_') === 0)) {
        refreshAccount();
      }
    });
...
async function refreshAccount() {
  ...
  try {
    const auth = self.VD_AUTH;
    // One read, shared with the worker (VD_AUTH.readAccount). The panel used
    // to apply its own rule here — access_token present, nothing else — while
    // the worker applied another, and the two could disagree about the same
    // storage. Now there is exactly one rule, in one place.
    const acct = (auth && typeof auth.readAccount === 'function')
      ? await auth.readAccount()
      : { authed: false, email: null, avatarUrl: null, reason: 'auth-unavailable' };
    state.account = { authed: !!acct.authed, email: acct.email || null, avatarUrl: acct.avatarUrl || null,
                      reason: acct.reason || null };
  } catch (e) {
...
// The bridge for a build the site does not know by id: the page reads `ext`,
// signs the user in if needed, and posts VD_EXT_LOGIN to that id
// (docs/AUTH-HANDOFF.md, "The bridge page").
const BRIDGE_PATH = 'https://vibedesign.tech/auth/extension-callback';
function bridgeUrl() { return BRIDGE_PATH + '?ext=' + encodeURIComponent(chrome.runtime.id) + '&from=extension'; }

async function signIn() {
  try {
    // An unpacked build never depends on the production site to sign in: it
    // goes to Supabase's OAuth endpoint through chrome.identity, directly.
    // No reachability probe, no tab to vibedesign.tech/login.
    if (isUnpackedBuild()) {
      // Direct only when the auth server says the authorize URL works (a
      // redirect to the provider). Our project answers 400 "Unsupported
      // provider: missing OAuth secret" until a Google OAuth client is set
      // up in the Supabase dashboard — launchWebAuthFlow then fails with
      // "Authorization page could not be loaded" and no window appears. So
      // the default for a dev build is the site bridge, addressed to THIS
      // extension id, which is the one thing the store build's flow cannot
      // know about an unpacked load.
      const pre = (self.VD_AUTH && typeof self.VD_AUTH.preflightDirectAuth === 'function')
        ? await self.VD_AUTH.preflightDirectAuth('google') : { ok: false, reason: 'unavailable' };
      if (pre.ok) {
        console.log('[vd-ui] dev build: direct sign-in (preflight passed)');
        await self.VD_AUTH.openAuthFlow('login', { direct: true });
        await refreshAccount();
        if (state.account && state.account.authed) setTab('settings');
        return;
      }
      console.warn('[vd-ui] dev build: direct sign-in unavailable (' + (pre.reason || 'unknown')
        + (pre.body ? ': ' + pre.body : '') + ') — opening the site bridge for ' + chrome.runtime.id);
    ...
    if (reachable) {
      await chrome.tabs.create({ url: LOGIN_URL + '&ext=' + encodeURIComponent(chrome.runtime.id) });
...
async function signOutAccount() {
  try {
    if (self.VD_AUTH && typeof self.VD_AUTH.signOut === 'function') {
      // A dev build signed in on its own; signing out ends only its own
      // session. A packaged build's sign-out is global on purpose — it is
      // how the vibedesign.tech session ends too (docs/AUTH-HANDOFF.md).
      await self.VD_AUTH.signOut(isUnpackedBuild() ? { scope: 'local' } : undefined);
    }
