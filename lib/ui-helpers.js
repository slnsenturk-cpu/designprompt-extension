...
// Default no-op — popup.js overrides for image focus chips
...
async function activateImagePicker(tabId) {
  await injectPickerScript(tabId);
  await chrome.tabs.sendMessage(tabId, { type: 'ACTIVATE_IMAGE_PICKER' });
  state.imagePickerActive = true;
  updateAnalyzeBtn();
}

async function deactivateImagePicker(tabId) {
  await chrome.tabs.sendMessage(tabId, { type: 'DEACTIVATE_IMAGE_PICKER' });
  state.imagePickerActive = false;
  updateAnalyzeBtn();
}
...
async function handleAnalyze() {
  ...
  if (state.mode === 'image') {
    if (state.capturedImageData) {
      try {
        const imageUrl = state.capturedImageData.croppedDataUrl || state.capturedImageData.src;
        // Vision API analysis if provider configured
        if (imageUrl && state.provider && state.provider !== 'none' && state.apiKeys[state.provider]) {
          try {
            showLoading('Analyzing image with AI…');
            const visionDesc = await analyzeIllustrationWithVision(
              imageUrl, state.provider, state.apiKeys[state.provider],
              getActiveModel(state.provider)
            );
            if (visionDesc) state.capturedImageData.visionIllustrationDescription = visionDesc;
          } catch(e) { console.debug('[VibeDesign] Image vision skipped:', e.message); }
        }
        buildImagePrompt(state.capturedImageData);
      } catch(err) { showError(err.message || 'Image analysis failed.'); }
      return;
    }
    try {
      if (state.imagePickerActive) {
        await deactivateImagePicker(tab.id);
      } else {
        await activateImagePicker(tab.id);
      }
    } catch(err) { showError('Could not activate image picker. Refresh and try again.'); }
    return;
  }
...
async function injectContentScript(tabId) {
  try {
    // noise-filter.js must load before content.js (provides isNoisyVar, hasUsefulValue)
...
function setupListeners() {
  ...
  chrome.storage.onChanged.addListener((changes, area) => {
    ...
    } else if (pending.type === 'IMAGE_PICKED') {
      handleImagePicked(pending.data);
    }
  });
...
const APP_TEMPLATE = `
  ...
  <div class="mode-section">
    <div class="mode-tabs" role="tablist" aria-label="Analysis mode">
      ...
      <button class="mode-tab" data-mode="image" role="tab" aria-selected="false"><span class="mode-icon">⬚</span> Image</button>
    </div>
