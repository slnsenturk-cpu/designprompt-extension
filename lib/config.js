...
var VD_CONFIG = {
  ...
  // The alarm must fire at least once while the token sits inside the leeway
  // window, so the period has to be <= REFRESH_LEEWAY_MS. At the old 50 min it
  // only ever caught the window by luck of phase alignment, and a miss meant
  // the token lapsed until something else refreshed it.
  REFRESH_ALARM_PERIOD_MIN: 5,
