...
//     tailwind.config.js  the same scale as a Tailwind v3 theme
//     tailwind.v4.css     the same scale as a Tailwind v4 @theme, plus keyframes
...
const VD_SKILL = (() => {
  ...
  const { hex, len, num, int, word, duration } = MODEL.accessors;
  ...
  // ── keyframes ───────────────────────────────────────────────────────────
  // The extractor records only the FIRST and LAST rule of each @keyframes
  // block. For a genuinely two-frame animation that is the whole thing; for
  // anything longer it is two samples of a curve, and emitting those as if
  // they were the animation would produce motion the site does not have.
  //
  // The test for "whole thing" is structural, not a name list: the two frames'
  // stops must come to exactly {0%, 100%} or exactly {0%, 50%, 100%}, and the
  // two declaration blocks must actually differ. Anything else — an interior
  // stop at 92%, a frame identical to its partner, a missing second frame — is
  // reported as under-captured rather than guessed at.
  function parseFrame(frame, resolve) {
    if (typeof frame !== 'string') return null;
    const m = frame.match(/^([^{]+)\{([\s\S]*)\}\s*$/);
    if (!m) return null;
    const stops = m[1].split(',').map(x => {
      const t = x.trim().toLowerCase();
      if (t === 'from') return 0;
      if (t === 'to') return 100;
      const n = parseFloat(t);
      return /%$/.test(t) && Number.isFinite(n) ? n : NaN;
    });
    if (!stops.length || stops.some(n => !Number.isFinite(n))) return null;

    // `content:` is the one CSS property that can carry page copy. It is
    // dropped rather than filtered, because no design token needs it.
    const raw = m[2].split(';')
      .map(d => d.trim())
      .filter(d => d && !/^content\s*:/i.test(d));
    if (!raw.length) return null;

    // Radix and similar libraries write keyframes against runtime variables
    // (--radix-accordion-content-height) that only exist while the component
    // is mounted. Those cannot be resolved from a static capture, and emitting
    // them produces an animation that silently does nothing. A frame with an
    // unresolvable reference fails here, and the caller reports the animation
    // as under-captured rather than shipping it broken.
    const decls = [];
    for (let i = 0; i < raw.length; i++) {
      const colon = raw[i].indexOf(':');
      if (colon === -1) return null;
      const prop = raw[i].slice(0, colon).trim();
      const value = cssValue(raw[i].slice(colon + 1), resolve);
      if (!value) return null;
      decls.push(`${prop}: ${value}`);
    }
    return { stops, decls, body: decls.join('; ') + ';' };
  }

  const CANONICAL = [ '0,100', '0,50,100' ];

  function classifyAnimations(t, resolve) {
    const full = [];
    const partial = [];
    (Array.isArray(t.animations) ? t.animations : []).forEach(a => {
      if (!a || typeof a.name !== 'string' || !/^[A-Za-z][\w-]*$/.test(a.name)) return;
      const from = parseFrame(a.from, resolve);
      const to = parseFrame(a.to, resolve);
      if (!from || !to) {
        const unresolved = /var\(\s*--/.test(String(a.from) + String(a.to));
        partial.push({ name: a.name, why: unresolved
          ? 'a frame references a runtime variable that a static capture cannot resolve'
          : 'only one usable frame was captured' });
        return;
      }
      if (from.body === to.body) {
        partial.push({ name: a.name, why: 'both captured frames are identical' });
        return;
      }
      const union = [...new Set(from.stops.concat(to.stops))].sort((x, y) => x - y);
      if (CANONICAL.indexOf(union.join(',')) === -1) {
        partial.push({ name: a.name, why: `stops ${union.join('%, ')}% imply frames that were not captured` });
        return;
      }
      full.push({ name: a.name, frames: [from, to].sort((x, y) => Math.min(...x.stops) - Math.min(...y.stops)) });
    });
    return { full, partial };
  }
  ...
  function buildTailwindV4(model, vars) {
    const t = model.tokens || {};
    const R = model._resolve;
    const L = [];
    const ns = [];   // [namespace, name, value]

    model.colorRoles.forEach(r => ns.push(['color', r, model.colors[r]]));

    const stacks = (model.typography && model.typography.stacks) || {};
    Object.keys(stacks).forEach(role =>
      ns.push(['font', role, `${q(stacks[role])}, ${fallbackStack(role)}`]));

    const detail = (model.typography && model.typography.detail) || {};
    MODEL.SCALE_KEYS.forEach(step => {
      const d = detail[step];
      if (d && d.size) ns.push(['text', step, d.size]);
    });
    ...
    if (model.spacing) {
      ...
      (model.spacing.scale || []).forEach(step => {
        const mult = base ? Math.round(parseFloat(step) / base) : 0;
        if (mult > 0) ns.push(['spacing', String(mult), step]);
      });
    }

    Object.keys(model.radius || {}).forEach(k => ns.push(['radius', k, model.radius[k]]));

    Object.keys(model.shadows || {}).forEach(k => {
      const v = cssValue(model.shadows[k], R);
      if (v) ns.push(['shadow', k, v]);
    });

    // Easings, named by the role the token exporter assigned, so a curve is
    // called the same thing in tokens.json and here.
    const motion = TOKENS.exportTokensFromModel(model).motion || {};
    Object.keys(motion).filter(k => k.indexOf('ease-') === 0).forEach(k => {
      const tok = motion[k];
      const value = Array.isArray(tok.$value) ? `cubic-bezier(${tok.$value.join(', ')})` : tok.$value;
      ns.push(['ease', k.slice('ease-'.length), value]);
    });

    const { full, partial } = classifyAnimations(t, R);
    // An --animate-* shorthand needs a duration, and only the ambient loops
    // were measured for one. A keyframe set with no observed timing is still
    // emitted; the consumer picks its own duration rather than inheriting an
    // invented one.
    const durations = {};
    (Array.isArray(t.ambientAnimations) ? t.ambientAnimations : []).forEach(a => {
      if (a && typeof a.name === 'string' && duration(a.duration)) durations[a.name] = duration(a.duration);
    });
    // Duration and "runs forever" are both observed facts about an ambient
    // loop. The timing function is NOT — omitting it leaves the CSS default
    // (`ease`), which is honest, where naming a curve here would not be.
    const timed = full.filter(a => durations[a.name]);
    timed.forEach(a => ns.push(['animate', a.name, `${a.name} ${durations[a.name]} infinite`]));

    L.push('/* ' + model.source.name + ' — Tailwind v4 theme.');
    L.push(' *');
    L.push(' * Tailwind v4 configures itself from CSS. Import this instead of');
    L.push(' * tailwind.config.js — not as well as it; the two describe the same scale');
    L.push(' * and tailwind.config.js is here for projects still on v3.');
    L.push(' *');
    L.push(' * Generated by VibeDesign from the live page' +
      (model.source.viewport ? ` at ${model.source.viewport.width}×${model.source.viewport.height}` : '') + '. */');
    L.push('');
    ...
    L.push('@theme {');
    let group = null;
    ns.forEach(([space, name, value]) => {
      if (space !== group) { if (group !== null) L.push(''); group = space; }
    ...
    if (full.length) {
      L.push('');
      L.push('/* Keyframes captured in full: both frames were recorded and their stops');
      L.push(' * account for the whole timeline. */');
      full.forEach(a => {
        L.push('');
        L.push(`@keyframes ${a.name} {`);
        a.frames.forEach(fr => {
          L.push(`  ${fr.stops.map(x => x + '%').join(', ')} { ${fr.body} }`);
        });
        L.push('}');
      });
    }

    if (timed.length < full.length) {
      const untimed = full.filter(a => !durations[a.name]).map(a => a.name);
      L.push('');
      L.push('/* These keyframes are complete but their timing was not observed, so no');
      L.push(' * --animate-* shorthand is defined for them. Set your own duration:');
      L.push(` *   ${untimed.join(', ')} */`);
    }

    if (partial.length) {
      L.push('');
      L.push('/* Under-captured — NOT emitted. The extractor records the first and last');
      L.push(' * rule of each @keyframes block; for these that is not the whole');
      L.push(' * animation, and writing them out would produce motion this site does');
      L.push(' * not have:');
      partial.forEach(a => L.push(` *   ${a.name} — ${a.why}`));
      L.push(' */');
    }
  ...
  function buildSkillMd(model, slug, opts) {
    ...
    L.push('| `tailwind.config.js` | The same scale as a Tailwind **v3** theme, under `extend`. |');
    L.push('| `tailwind.v4.css` | Tailwind **v4**: `@theme` tokens plus the `@keyframes` that were captured in full. Import this instead of the v3 config, not as well as it. |');
    L.push('');
    ...
    if (fonts.length) {
      ...
      L.push('');
      L.push('*Hosting is observed; licensing is a name lookup against the Google Fonts '
        + 'catalogue. "Licence unknown" means not found there — not that it is proprietary.*');
      L.push('');
    }
  ...
  function buildReadme(model, slug, opts) {
    ...
    L.push('- `tailwind.config.js` — the same values as a Tailwind v3 theme.');
    L.push('- `tailwind.v4.css` — Tailwind v4 `@theme` tokens and captured `@keyframes`.');
    L.push('');
    ...
    L.push('- **`$type: "other"` is not a DTCG type.** It is used deliberately, for observed values the spec has no type for: a `clip-path` polygon, a radius that is a percentage or a per-corner list, and a CSS easing keyword. The alternative was to drop them, which would lose the most distinctive thing about some designs — a chamfered button has no `border-radius` to record instead. Treat these as strings, and expect a strict DTCG validator to reject them.');
    L.push('- **Suggestions are labelled.** Font substitutes and any contrast advice are marked *suggested, not observed*.');
    L.push('- **Font hosting and font licensing are separate questions.** Where a file was served from is observed. Whether the family is openly licensed is a name lookup against the Google Fonts catalogue, which is a sufficient test for "open" and not a necessary one — so a family missing from it is reported as *licence unknown*, never as proprietary. Self-hosting an openly-licensed family is common and says nothing about its licence.');
    L.push('');
  ...
  function buildSkillBundle(model, options) {
    ...
    const files = [
      ...
      { path: `${slug}/tailwind.v4.css`, text: buildTailwindV4(model, vars) },
    ];
