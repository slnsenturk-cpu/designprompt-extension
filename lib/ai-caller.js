...
Hero entrance sequence: ${extractedData.heroEntranceSequence?.elements?.slice(0, 5).map((el, i) => `${el.tag}(${el.delay || '0ms'}${el.aosType ? ',' + el.aosType : ''}${el.blur ? ',blur→clear' : ''})`).join(' → ') || 'not detected'}
AOS library: ${(() => { const scm = extractedData.sectionContentMap || []; const aosSection = scm.find(s => s.aosConfig); if (!aosSection) return 'not detected'; const a = aosSection.aosConfig; return `type=${a.type}, duration=${a.duration}ms, offset=${a.offset}px, once=${a.once}`; })()}
Per-section animation: ${(() => { const scm = extractedData.sectionContentMap || []; return scm.filter(s => s.animationType).map((s, i) => `S${i + 1}=${s.animationType}`).join(', ') || 'uniform'; })()}
