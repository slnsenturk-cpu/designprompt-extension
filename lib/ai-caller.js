...
// §2: the facts the direction paragraph is not allowed to contradict.
//
// The model has already decided these from the page; the AI is writing prose
// about them, not re-deriving them. Handing them over as constraints is
// cheaper and more reliable than correcting the prose afterwards — though
// _stripContradictions still does that, because a model asked not to say
// something will occasionally say it anyway.
function _vdModelFacts(extractedData) {
  try {
    const M = (typeof self !== 'undefined' && self.VD_MODEL) ? self.VD_MODEL : null;
    if (!M) return null;
    const m = M.buildDesignModel(extractedData, { sourceUrl: extractedData.url || '' });
    const openFonts = (m.fonts || []).filter(f => f.openlyLicensed).map(f => f.family);
    const unknownFonts = (m.fonts || []).filter(f => !f.openlyLicensed).map(f => f.family);
    return {
      model: m,
      chamfer: (m.shape && m.shape.clipPath) ? (m.shape.size || 'a fixed offset') : null,
      radius: m.radius || {},
      openFonts, unknownFonts,
      shadowStyle: Object.keys(m.shadows || {}).some(k => /layered/.test(k)) ? 'layered' : 'simple',
    };
  } catch (e) { return null; }
}

function _vdConstraintBlock(facts) {
  if (!facts) return '';
  const L = ['', 'MEASURED FACTS — do not contradict these. They come from the page, not from you:'];
  if (facts.chamfer) {
    L.push(`- Interactive elements are CHAMFERED by ${facts.chamfer} using a clip-path polygon. They have NO border-radius. Never call them rounded, soft or pill-shaped.`);
  }
  const r = facts.radius;
  if (r.button) L.push(`- Button radius is exactly ${r.button}.`);
  if (r.card) L.push(`- Card radius is exactly ${r.card}.`);
  if (r.input) L.push(`- Input radius is exactly ${r.input}.`);
  if (facts.openFonts.length) {
    L.push(`- These families are openly licensed and available on Google Fonts: ${facts.openFonts.join(', ')}. Do not describe them as unavailable, proprietary, or needing a substitute.`);
  }
  if (facts.unknownFonts.length) {
    L.push(`- These families have an unknown licence: ${facts.unknownFonts.join(', ')}. You may suggest a substitute, but say it is a suggestion.`);
  }
  L.push(`- Shadow style is ${facts.shadowStyle}.`);
  return L.join('\n');
}

// A last line of defence. Any SENTENCE that states something the measurements
// contradict is dropped whole — a half-corrected sentence reads worse than a
// missing one, and the paragraph survives losing one.
function _stripContradictions(direction, facts) {
  if (!direction || !facts) return direction;
  const rules = [];
  if (facts.chamfer) {
    // Buttons are chamfered: nothing may call them rounded or pill-shaped.
    rules.push(/\b(rounded|pill[- ]shaped|soft(ly)? round|border-radius\s*:\s*(?!0)\d)/i);
  }
  (facts.openFonts || []).forEach(family => {
    const f = family.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    rules.push(new RegExp(`${f}[^.]*\\b(unavailable|proprietary|not freely|licen[cs]e|substitute|replace)`, 'i'));
    rules.push(new RegExp(`\\b(substitute|replace|swap)[^.]*${f}`, 'i'));
  });
  if (!rules.length) return direction;

  const kept = direction.split(/(?<=[.!?])\s+/).filter(sentence => {
    // A sentence that names the chamfer AND calls it rounded is contradicting
    // itself; one that merely mentions a radius elsewhere is not.
    const hit = rules.some(re => re.test(sentence));
    if (hit) console.warn('[vd-direction] dropped a sentence contradicting the measurements:', sentence.slice(0, 90));
    return !hit;
  });
  return kept.join(' ').trim() || direction;
}
...
${_vdConstraintBlock(_vdModelFacts(extractedData))}

Produce the brief per the 8-paragraph format defined in the system prompt — use the measurements above literally.`;
}
...
function postProcessDirection(direction, fontRoleDisplay, fontRoleBody, cleanedFonts, extractedData) {
  ...
  // The triple-signal resolver in prompt-builder.js cross-checks (1) button extractor,
  // (2) CSS-var primary, (3) frequency-weighted accent. Here we mirror that intelligence:
  // if signals disagreed (ambiguous), the prompt already carries a verify-before-ship
  // warning — forcing a rewrite would compound the uncertainty. Even when not ambiguous,
  // the AI may legitimately cite a minority signal (e.g. button bg when CSS var won);
  // that's a valid reading, not a hallucination. Only rewrite when the AI's hex matches
  // none of the three extracted signals.
  if (extractedData && !extractedData._primaryColorAmbiguous) {
    // Inline-replicate the three signals without mutating data (avoids circular coupling
    // with prompt-builder.js's resolver, which has side effects on `data`).
    const _sc = (typeof extractSemanticColors === 'function')
      ? extractSemanticColors(extractedData.cssVars || {})
      : {};
    const _css = (_sc.primary && /^#[0-9a-f]{6}$/i.test(_sc.primary)) ? _sc.primary.toLowerCase() : null;
    const _btnRaw = extractedData.buttonStyles?.primary?.backgroundColor;
    const _btn = (_btnRaw && /^#[0-9a-f]{3,8}$/i.test(_btnRaw) && _btnRaw.toLowerCase() !== '#ffffff')
      ? _btnRaw.toLowerCase() : null;
    let _freq = null;
    if (typeof hexSat === 'function') {
      const _accents = extractedData.accentColors || [];
      const _usage = extractedData.colorUsage || {};
      const scored = _accents
        .filter(c => typeof c === 'string' && /^#[0-9a-f]{6}$/i.test(c) && hexSat(c) > 25)
        .map(c => {
          const u = _usage[c.toLowerCase()] || {};
          return { hex: c.toLowerCase(), score: hexSat(c) * (1 + Math.log10(((u.bg || 0) + (u.text || 0)) + 1)) };
        })
        .sort((a, b) => b.score - a.score);
      _freq = scored[0]?.hex || null;
    }
    const _primaryAction = _css || _btn || _freq; // same priority order as the resolver
    const _candidates = new Set([_css, _btn, _freq].filter(Boolean));
    ...
    if (_primaryAction) {
      ...
      if (colorParaMatch) {
        ...
        if (wrongPrimaryMatch) {
          const aiHex = wrongPrimaryMatch[1].toLowerCase();
          // _candidates ⊇ {winner}, so matching any candidate also covers matching the
          // winner. Only rewrite when the AI hex is outside the candidate set entirely —
          // i.e. the AI invented a hex no extraction signal supports.
          if (!_candidates.has(aiHex)) {
            const wrongColor = wrongPrimaryMatch[1];
            const fixedPara = colorPara.replace(
            ...
            console.info(`[VibeDesign] Color role corrected: ${wrongColor} hallucinated as primary-action (resolver: ${_primaryAction}, candidates: ${[..._candidates].join(', ')})`);
          }
        }
      }
    }
...
// ── image analysis (§4.6) ───────────────────────────────────────────────
// A vision model looks at a static image and returns a STYLE PROFILE — a
// guess at palette, type direction, shape, iconography. This is the only
// place an image ever goes: straight to the provider the user chose, with
// their own key, and nowhere else.

// Which of the provider's models will accept an image. The selected model is
// used when it can see; otherwise the newest vision-capable model in the live
// list. The patterns name what each provider's current vision line looks like,
// because none of the three list APIs flags image support explicitly.
const VISION_PATTERNS = {
  claude: /^claude-(fable|opus|sonnet|haiku)-/i,          // every current Claude model accepts images
  openai: /^(gpt-5|gpt-4\.1|gpt-4o|o[134])(?![a-z-]*(audio|realtime|transcribe|tts))/i,
  gemini: /^gemini-(2|3)/i,                               // 2.x and later are multimodal
};

function pickVisionModel(provider, selectedId, liveModels) {
  const re = VISION_PATTERNS[provider];
  if (!re) return null;
  if (selectedId && re.test(selectedId)) return selectedId;
  const list = Array.isArray(liveModels) ? liveModels : [];
  const hit = list.find(m => m && re.test(m.id));
  return hit ? hit.id : null;
}

// Fixed. The reply is parsed by lib/design-model.js#parseStyleProfile, which
// rejects anything that does not fit — so the prompt asks for exactly that
// shape and nothing else.
const IMAGE_PROFILE_PROMPT = [
  'You are reading the VISUAL LANGUAGE of a static image for a design system.',
  'Return ONE JSON object and nothing else — no prose, no markdown fence.',
  '',
  '{',
  '  "image_kind": "ui-screenshot | illustration | pattern | photo | typography | mixed",   // what the image IS',
  '  "palette": [ { "hex": "#rrggbb", "role": "background|text-primary|text-secondary|accent|surface|border" } ],   // 5 to 8 entries, most dominant first',
  '  "typography": {',
  '    "classification": "e.g. geometric sans | humanist sans | grotesk | transitional serif | slab | mono | display",',
  '    "weightCharacter": "e.g. light and airy | medium | bold and tight",',
  '    "families": [ ]   // ONLY if a typeface name is clearly legible in the image; otherwise leave empty',
  '  },',
  '  "shape": "sharp | soft | pill | mixed",',
  '  "iconography": { "count": <number of DISTINCT icons visible>, "style": "stroke | filled | mixed", "weight": "e.g. 1.5px", "corners": "rounded | sharp", "grid": "e.g. 24px" },',
  '  "illustration": "one or two sentences on illustration or texture style, or \"none\"",',
  '  "density": "airy | balanced | dense",',
  '  "mood": [ "two to five words" ],',
  '  "dominantBackground": "#rrggbb",',
  '  "effects": [   // the effects that DEFINE this image, most defining first, at most 6',
  '    { "type": "glass | blur | glow | gradient | shadow | noise | grain | pattern | other", "where": "e.g. card bottom edge", "colour": "#rrggbb", "strength": "subtle | medium | strong", "description": "one or two sentences" }',
  '  ],',
  '  "container": { "shape": "e.g. rounded rectangle", "radius_estimate": "e.g. large ≈ 24px, relative to the card", "surface": "opaque | translucent", "border": "none | hairline | glowing" },',
  '  "hierarchy": [ "the 2 to 3 dominant visual moves, in order, e.g. oversized numeral", "single accent glow", "one chart as the only illustration" ],',
  '  "evidence": { "text": "strong | weak | insufficient — is there VISIBLE TEXT in the image?", "palette": "strong | weak | insufficient", "typography": "...", "shape": "...", "iconography": "...", "illustration": "...", "density": "...", "mood": "...", "effects": "...", "container": "...", "hierarchy": "..." }',
  '}',
  '',
  'Rules: every colour as six-digit hex. Do not invent a typeface name. Do not describe motion — a still image has none.',
  'Evidence is honest: "strong" only when the image shows it plainly; "insufficient" when you would be guessing — and then leave that field empty.',
  'Iconography needs at least three distinct icons; a single chevron or arrow is insufficient.',
  'A geometric or repeating motif is an effect of type "pattern"; anything that fits no other type is "other" with a description. Prefer these over forcing "gradient" or "grain" onto something that is neither.',
  'With no visible text, set evidence.text to "insufficient" and leave typography empty; do not guess a typeface from a pattern.',
].join('\n');

// dataUrl: "data:image/png;base64,....". Returns the raw reply text; the
// caller parses it. Throws on transport or provider errors.
async function analyzeImageWithAI(dataUrl, provider, apiKey, modelId) {
  if (!provider || !apiKey || !modelId) throw new Error('image analysis needs a provider, a key and a model');
  ...
  if (provider === 'claude') {
    const r = await fetch('https://api.anthropic.com/v1/messages', {
      ...
      body: JSON.stringify({
        model: modelId, max_tokens: 1200,
        messages: [{ role: 'user', content: [
          { type: 'image', source: { type: 'base64', media_type: mime, data: b64 } },
          { type: 'text', text: IMAGE_PROFILE_PROMPT },
        ] }],
      }),
    });
    if (!r.ok) throw new Error(r.status === 401 ? 'Claude: Invalid key.' : `Claude: ${r.status}`);
    const d = await r.json();
  ...
  if (provider === 'openai') {
    const r = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${apiKey}` },
      body: JSON.stringify({
        model: modelId, max_tokens: 1200, temperature: 0.2,
        messages: [{ role: 'user', content: [
          { type: 'text', text: IMAGE_PROFILE_PROMPT },
    ...
    if (!r.ok) throw new Error(r.status === 401 ? 'OpenAI: Invalid key.' : `OpenAI: ${r.status}`);
    const d = await r.json();
    return d.choices?.[0]?.message?.content || '';
  }
...
async function generateDirectionWithAI(extractedData, provider, apiKey, modelId, onStatus, onChunk) {
  ...
  try {
    ...
    // §2: the constraints go out with the request, and anything that came back
    // contradicting them is removed. Both, because a constraint is a request
    // and this is a check.
    direction = _stripContradictions(direction, _vdModelFacts(extractedData));
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = Object.assign(module.exports || {}, {
    pickVisionModel, analyzeImageWithAI, IMAGE_PROFILE_PROMPT, VISION_PATTERNS,
  });
}
