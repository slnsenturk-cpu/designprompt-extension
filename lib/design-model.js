...
const VD_MODEL = (() => {
  ...
  const EFFECT_TYPES = ['glass', 'blur', 'glow', 'gradient', 'shadow', 'noise', 'grain'];
  ...
  function parseStyleProfile(text) {
    ...
    const evidence = {
      ...
      typography: evidenceOf('typography', t),
  ...
  const EVIDENCE_FIELDS = ['palette', 'typography', 'shape', 'iconography', 'illustration', 'density', 'mood',
  ...
  function buildModelFromStyleProfile(profile, options) {
    ...
    // Colours: a role map like the website's, every entry a guess.
    const colors = {};
    const taken = new Set();
    ...
    const cls = profile.typography && profile.typography.classification;
    const families = (profile.typography && profile.typography.families) || [];
    const suggested = suggestForClassification(cls);
    ...
    // Evidence per field. A stored profile from before this field existed
    // has none; it is read as "weak" everywhere except icons, which are held
    // to the ≥3 rule retroactively.
  ...
  return {
    ...
    parseStyleProfile,
    PROFILE_SHAPE,
    IMAGE_ROLES, EVIDENCE_FIELDS, INSUFFICIENT,
    descriptionCut: desc,
