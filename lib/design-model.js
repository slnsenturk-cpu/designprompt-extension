...
const VD_MODEL = (() => {
  ...
  const PROFILE_SHAPE = {
    ...
    iconography: 'object',      // { style: 'stroke'|'filled'|'mixed', weight, corners, grid }
  ...
  function parseStyleProfile(text) {
    ...
    (Array.isArray(raw.palette) ? raw.palette : []).forEach((entry, i) => {
      ...
      seen.add(h);
      const role = (entry && typeof entry.role === 'string' && /^[a-z][a-z0-9-]{1,24}$/i.test(entry.role))
        ? entry.role.toLowerCase() : null;
    ...
    return {
      ...
      illustration: str(raw.illustration || raw.texture),
  ...
  function guessRole(entry, index, bgHex, taken) {
    if (entry.role && !taken.has(entry.role)) return entry.role;
    const c = hsl(entry.hex);
    const bg = hsl(bgHex);
    const isBg = entry.hex === bgHex;
    let role;
    if (isBg) role = 'background';
    else if (Math.abs(c.l - bg.l) > 45) role = taken.has('text-primary') ? 'text-secondary' : 'text-primary';
    else if (c.s > 40) role = taken.has('accent') ? 'accent-' + index : 'accent';
    else role = taken.has('surface') ? 'border' : 'surface';
    let out = role, n = 2;
    while (taken.has(out)) out = role + '-' + n++;
    return out;
  }
  ...
  function buildModelFromStyleProfile(profile, options) {
    ...
    profile.palette.forEach((entry, i) => {
      const role = guessRole(entry, i + 1, bgHex, taken);
      taken.add(role);
      colors[role] = hex(entry.hex) || entry.hex;
    });
    ...
    const shape = profile.shape ? { language: profile.shape, clipPath: null, size: null, confidence: EST } : null;
    const iconography = profile.iconography ? Object.assign({ confidence: EST }, profile.iconography) : null;
    ...
    return {
      ...
      heroSurface: null,
      fonts: fonts,
      // Image-only facets, each flagged.
      iconography: iconography,
      illustration: profile.illustration ? { style: profile.illustration, confidence: EST } : null,
      density: profile.density ? { level: profile.density, confidence: EST } : null,
      mood: moods.length ? { words: moods, confidence: EST } : null,
