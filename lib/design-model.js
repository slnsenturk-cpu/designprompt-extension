...
const VD_MODEL = (() => {
  ...
  // ── font hosting and licensing ──────────────────────────────────────────
  // These are two SEPARATE questions and the old code conflated them:
  // "self-hosted" was reported as "not freely available", which is wrong for
  // the many sites that self-host a copy of an openly-licensed family.
  //
  //   hosting  — where the file was served from. Observed.
  //   licence  — whether the family is openly licensed. Looked up by name in
  //              the Google Fonts catalogue (lib/data/google-fonts.json,
  //              regenerated by scripts/update-google-fonts.mjs).
  //
  // Being in the Google Fonts catalogue is a sufficient test for "open", not a
  // necessary one: a family absent from it may still be open. So the negative
  // case says "licence unknown — likely proprietary", not "proprietary".
  const FONT_HOSTS = [
    [/(^|\.)fonts\.googleapis\.com$|(^|\.)fonts\.gstatic\.com$/, 'Google Fonts'],
    [/(^|\.)use\.typekit\.net$|(^|\.)p\.typekit\.net$|(^|\.)typekit\.com$/, 'Adobe Fonts'],
    [/(^|\.)fonts\.bunny\.net$/, 'Bunny Fonts'],
    [/(^|\.)(api|cdn)\.fontshare\.com$|(^|\.)fontshare\.com$/, 'Fontshare'],
    [/(^|\.)use\.fontawesome\.com$/, 'Font Awesome'],
    [/(^|\.)fnt\.simplecdn\.org$|(^|\.)fontlibrary\.org$/, 'Font Library'],
  ];

  // The catalogue, loaded however this file was loaded. Absent (an old build,
  // a test harness that did not load it) means the licence question cannot be
  // answered, and every family falls through to "unknown" — never to a guess.
  const GOOGLE_FAMILIES = (() => {
    let list = null;
    if (typeof module !== 'undefined' && module.exports) {
      try { list = require('./data/google-fonts.json').families; } catch (e) { /* fall through */ }
    }
    if (!list) {
      const g = (typeof self !== 'undefined') ? self : globalThis;
      list = g.VD_GOOGLE_FONTS || null;
    }
    if (!Array.isArray(list)) return null;
    // Lower-cased for lookup: CSS family names vary in case between the
    // @font-face rule and the catalogue.
    return new Set(list.map(f => String(f).toLowerCase()));
  })();

  function isOpenlyLicensed(family) {
    if (!GOOGLE_FAMILIES) return false;
    const name = String(family).trim().toLowerCase();
    if (GOOGLE_FAMILIES.has(name)) return true;
    // Sites routinely ship the variable cut under "<Family> Variable" or
    // "<Family> VF". That is the same family, so the suffix is stripped and
    // retried. Only a SPACE-separated suffix counts: "CameraPlainVariable" is
    // its own family name, not a variable cut of "CameraPlain".
    const stripped = name.replace(/ (variable|vf|var)$/, '');
    return stripped !== name && GOOGLE_FAMILIES.has(stripped);
  }

  // Where the file came from. Null when the URL is unusable.
  function fontHosting(url) {
    let host = '';
  ...
  // The single human-readable line, combining both answers.
  function fontAvailability(url, family) {
    const hosting = fontHosting(url);
    if (!hosting) return null;
    const open = isOpenlyLicensed(family);
    if (open) {
      return hosting === 'self-hosted'
        ? 'open (Google Fonts) — self-hosted copy'
        : `open (Google Fonts) — served from ${hosting}`;
    }
    if (hosting !== 'self-hosted') return `served from ${hosting}`;
    return 'self-hosted, licence unknown — likely proprietary';
  }
  ...
  function buildDesignModel(rawTokens, options) {
    ...
    return {
      // §2: two source types, two kinds of knowledge. A website model is
      // MEASURED — every value was read from a rendered DOM. Renderers key
      // off this one field rather than guessing from the shape of the data.
      sourceType: 'website',
      confidence: 'measured',
      // Provenance
      source: {
  ...
  function deriveFonts(t, typography) {
    ...
    assets.forEach(a => {
      ...
      seen.add(family);
      const hosting = fontHosting(a.url);
      if (!hosting) return;
      const availability = fontAvailability(a.url, family);
      const steps = stepsFor(family);
      const selfHosted = hosting === 'self-hosted';
      const openlyLicensed = isOpenlyLicensed(family);

      // A substitute is only useful advice for a family a reader may not be
      // able to obtain. An openly-licensed one they can simply go and get, and
      // suggesting a replacement for it would be noise — so no substitute is
      // offered for an open family, whoever is hosting it.
      let alternative = null;
      if (!openlyLicensed) {
        const alt = suggestAlternative(family, steps);
        // Never recommend a family to itself.
        alternative = alt.split(' or ').some(x => x.trim().toLowerCase() === family.toLowerCase())
          ? null : alt;
      }
      out.push({ family, availability, hosting, steps, selfHosted, alternative, openlyLicensed });
    });
    return out;
  }

  // ── image source: a style profile, not a capture ──────────────────────
  // A vision model looks at a static image and GUESSES: a palette, a type
  // direction, a shape language, an icon style. That is interpretation, and
  // §4.6 is emphatic that the UI never presents it as measurement. So this
  // path produces the same model shape the renderers already read, with
  // `confidence: 'estimated'` on the root and on every derived value — and
  // with nothing a static image cannot know: no motion, no states, no
  // anatomy, no spacing, no breakpoints.

  const HEX_RE = /^#[0-9a-f]{6}$/i;

  // What the vision call must return. Anything outside this is discarded,
  // and a payload that fails the required parts is rejected whole — the
  // caller shows "Couldn't read this image" rather than a half-filled model.
  const PROFILE_SHAPE = {
    palette: 'array',           // [{ hex, role? }] or ['#hex'], 5–8 entries
    typography: 'object',       // { classification, weightCharacter, families? }
    shape: 'string',            // 'sharp' | 'soft' | 'pill' | 'mixed' | free text
    iconography: 'object',      // { style, weight, corners, grid, count } — count ≥ 3 or insufficient
    illustration: 'string',     // texture / illustration style
    density: 'string',          // 'airy' | 'balanced' | 'dense'
    mood: 'array',              // 2–5 words
    dominantBackground: 'string',
    effects: 'array',           // [{ type, where, colour, strength, description }] — what defines the image
    container: 'object',        // { shape, radiusEstimate, surface, border }
    hierarchy: 'array',         // the 2–3 dominant visual moves, in order
    evidence: 'object',         // { <field>: 'strong' | 'weak' | 'insufficient' }
  };

  // Colour roles an image model may carry. Fixed, so a palette can never
  // sprout "accent-2" or "surface-3": an entry that fits none of these, or a
  // role already taken, is dropped rather than numbered.
  const IMAGE_ROLES = ['background', 'surface', 'text-primary', 'text-secondary', 'text-muted', 'accent', 'border'];
  const EVIDENCE = ['strong', 'weak', 'insufficient'];
  const EFFECT_TYPES = ['glass', 'blur', 'glow', 'gradient', 'shadow', 'noise', 'grain', 'pattern', 'other'];
  // What the image IS. Decides which blocks a renderer may show at all: a
  // pattern has no typography to direct and no container to measure.
  const IMAGE_KINDS = ['ui-screenshot', 'illustration', 'pattern', 'photo', 'typography', 'mixed'];
  const NO_INTERFACE_KINDS = ['pattern', 'illustration', 'photo'];
  const STRENGTHS = ['subtle', 'medium', 'strong'];
  const INSUFFICIENT = 'insufficient';

  // A description may run to 240 characters and is cut at a sentence
  // boundary, never mid-word. Every other string keeps the 80-char guard.
  const DESC_MAX = 240;
  function desc(v) {
    if (typeof v !== 'string') return null;
    ...
    if (sentence > DESC_MAX * 0.4) return head.slice(0, sentence + 1);
    const space = head.lastIndexOf(' ');
    return (space > 0 ? head.slice(0, space) : head).replace(/[,;:\-–—]$/, '') + '…';
  }

  function parseStyleProfile(text) {
    if (typeof text !== 'string') return null;
    // Models wrap JSON in fences or prose despite being told not to. Take the
    // outermost object literal and nothing else.
    const start = text.indexOf('{'), end = text.lastIndexOf('}');
    if (start === -1 || end <= start) return null;
    let raw;
    try { raw = JSON.parse(text.slice(start, end + 1)); } catch (e) { return null; }
    if (!raw || typeof raw !== 'object' || Array.isArray(raw)) return null;

    // Palette is the one part without which there is nothing to show.
    const palette = [];
    const seen = new Set();
    (Array.isArray(raw.palette) ? raw.palette : []).forEach((entry, i) => {
      const h = hex(typeof entry === 'string' ? entry : (entry && entry.hex));
      if (!h || seen.has(h)) return;
      seen.add(h);
      // Only the fixed role set. "accent-2", "primary", "highlight" → unnamed,
      // and guessRole() later fits it into a free slot or drops it.
      const r = entry && typeof entry.role === 'string' ? entry.role.toLowerCase().trim() : '';
      const role = IMAGE_ROLES.indexOf(r) !== -1 ? r : null;
      palette.push({ hex: h, role: role });
    });
    if (palette.length < 3) return null;

    const str = v => (typeof v === 'string' && v.trim()) ? v.trim().slice(0, 80) : null;
    const words = v => (Array.isArray(v) ? v : []).map(str).filter(Boolean).slice(0, 5);
    const oneOf = (v, list) => { const w = str(v); return w && list.indexOf(w.toLowerCase()) !== -1 ? w.toLowerCase() : null; };

    const t = raw.typography && typeof raw.typography === 'object' ? raw.typography : {};
    const ic = raw.iconography && typeof raw.iconography === 'object' ? raw.iconography : {};
    const ct = raw.container && typeof raw.container === 'object' ? raw.container : {};

    // Evidence. Read from the top-level map, with a per-object "evidence" key
    // as a fallback. A field the model did not rate is "weak" — present but
    // unvouched — never "strong" by default.
    const evMap = raw.evidence && typeof raw.evidence === 'object' ? raw.evidence : {};
    const evidenceOf = (key, obj) => {
      const v = oneOf(evMap[key], EVIDENCE) || oneOf(obj && obj.evidence, EVIDENCE);
      return v || 'weak';
    };

    // Effects: what defines the image. Each one typed, placed, coloured (a
    // guess), graded, described — and capped, because "everything is an
    // effect" is the same as nothing being one.
    const effects = (Array.isArray(raw.effects) ? raw.effects : []).map(e => {
      if (!e || typeof e !== 'object') return null;
      const type = oneOf(e.type, EFFECT_TYPES);
      if (!type) return null;
      return {
        type: type,
        where: str(e.where),
        colour: hex(e.colour || e.color),
        strength: oneOf(e.strength, STRENGTHS),
        description: desc(e.description),
      };
    }).filter(Boolean).slice(0, 6);

    const container = (ct.shape || ct.radius_estimate || ct.radiusEstimate || ct.surface || ct.border) ? {
      shape: str(ct.shape),
      radiusEstimate: str(ct.radius_estimate || ct.radiusEstimate),
      surface: oneOf(ct.surface, ['opaque', 'translucent']),
      border: oneOf(ct.border, ['none', 'hairline', 'glowing']),
    } : null;

    const hierarchy = (Array.isArray(raw.hierarchy) ? raw.hierarchy : []).map(desc).filter(Boolean).slice(0, 3);

    const imageKind = oneOf(raw.image_kind || raw.imageKind, IMAGE_KINDS) || 'mixed';

    // Icons: a profile needs at least three distinct icons. One chevron says
    // nothing about an icon system, whatever the model wrote next to it.
    const iconCount = Number.isFinite(Number(ic.count)) ? Number(ic.count)
                    : Number.isFinite(Number(ic.iconCount)) ? Number(ic.iconCount) : null;
    let iconEvidence = evidenceOf('iconography', ic);
    if (iconCount === null || iconCount < 3) iconEvidence = INSUFFICIENT;

    // Visible text. Text roles, a type direction and font suggestions all
    // hang off this: without text in the image there is nothing to read them
    // from. Falls back to the typography rating for profiles that predate it.
    const textEvidence = oneOf(evMap.text, EVIDENCE) || evidenceOf('typography', t);

    const evidence = {
      text: textEvidence,
      palette: evidenceOf('palette', raw.palette),
      typography: textEvidence === INSUFFICIENT ? INSUFFICIENT : evidenceOf('typography', t),
      shape: evidenceOf('shape'),
      iconography: iconEvidence,
      illustration: evidenceOf('illustration'),
      density: evidenceOf('density'),
      mood: evidenceOf('mood'),
      effects: effects.length ? evidenceOf('effects') : INSUFFICIENT,
      container: container ? evidenceOf('container', ct) : INSUFFICIENT,
      hierarchy: hierarchy.length ? evidenceOf('hierarchy') : INSUFFICIENT,
    };
    ...
    return {
      ...
      typography: {
        ...
        weightCharacter: str(t.weightCharacter || t.weight),
        // Family names are accepted only as the model reported them; the
        // prompt tells it to name a family only when text in the image is
        // clearly legible, and a guess would be presented as a fact.
        families: words(t.families),
      },
      shape: str(raw.shape),
      iconography: {
        style: str(ic.style),
        weight: str(ic.weight),
        corners: str(ic.corners),
        grid: str(ic.grid),
        count: iconCount,
      },
      illustration: desc(raw.illustration || raw.texture),
      density: str(raw.density),
      mood: words(raw.mood),
      dominantBackground: hex(raw.dominantBackground) || palette[0].hex,
    };
  }

  // Guesses a role for a palette entry the model left unnamed, from where it
  // sits relative to the background. Labelled a guess by the confidence flag
  // on everything in this model.
  // Returns null when every fitting role is taken: the entry is dropped, not
  // numbered. Roles are the fixed IMAGE_ROLES and nothing else.
  function guessRole(entry, index, bgHex, taken) {
    const free = r => IMAGE_ROLES.indexOf(r) !== -1 && !taken.has(r);
    if (entry.role && free(entry.role)) return entry.role;
    const c = hsl(entry.hex);
    const bg = hsl(bgHex);
    if (entry.hex === bgHex) return free('background') ? 'background' : null;
    const order = Math.abs(c.l - bg.l) > 45
  ...
  // Open families that match a classification, offered as SUGGESTIONS. The
  // website path names what the page loads; an image cannot, so these are
  // labelled and never presented as observed.
  const SUGGESTED_BY_CLASS = [
    [/geometric|grotesk|grotesque|sans/i, ['Inter', 'Manrope', 'DM Sans']],
    [/humanist/i, ['Source Sans 3', 'Nunito Sans', 'Open Sans']],
    [/serif|editorial|transitional|didone/i, ['Fraunces', 'Playfair Display', 'Source Serif 4']],
    [/mono|code|terminal/i, ['JetBrains Mono', 'IBM Plex Mono']],
    [/display|decorative|script|handwrit/i, ['Syne', 'Unbounded']],
    [/pixel|arcade|8[- ]?bit/i, ['Silkscreen', 'Press Start 2P']],
  ];

  function suggestForClassification(cls) {
    for (let i = 0; i < SUGGESTED_BY_CLASS.length; i++) {
      if (SUGGESTED_BY_CLASS[i][0].test(cls || '')) return SUGGESTED_BY_CLASS[i][1];
    }
    return ['Inter', 'DM Sans'];
  }

  const EVIDENCE_FIELDS = ['text', 'palette', 'typography', 'shape', 'iconography', 'illustration', 'density', 'mood',
                           'effects', 'container', 'hierarchy'];

  function buildModelFromStyleProfile(profile, options) {
    if (!profile || !Array.isArray(profile.palette) || !profile.palette.length) return null;
    const opts = Object.assign({ name: 'Image', sourceUrl: '' }, options || {});
    const EST = 'estimated';

    // Normalised here as well as in the parser: a profile handed in from a
    // fixture or a stored history entry must produce the same model as one
    // that came through parseStyleProfile.
    const bgHex = hex(profile.dominantBackground) || hex(profile.palette[0].hex) || '#ffffff';
    const bgL = hsl(bgHex).l;

    // Evidence per field, first — the colour roles depend on it.
    const ev = Object.assign({}, profile.evidence || {});
    EVIDENCE_FIELDS.forEach(k => { if (EVIDENCE.indexOf(ev[k]) === -1) ev[k] = 'weak'; });
    if (!profile.evidence || !profile.evidence.text) ev.text = ev.typography;   // pre-11c profile
    const imageKind = IMAGE_KINDS.indexOf(profile.imageKind) !== -1 ? profile.imageKind : 'mixed';
    const hasText = ev.text !== INSUFFICIENT;
    if (!hasText) ev.typography = INSUFFICIENT;

    // Colours: a role map like the website's, every entry a guess. Text
    // roles exist only when the image shows text; a grey in an image with
    // no words is a surface or a border, never "text-muted".
    const colors = {};
    const taken = new Set(hasText ? [] : ['text-primary', 'text-secondary', 'text-muted']);
    profile.palette.forEach((entry, i) => {
      const e = { hex: hex(entry.hex) || entry.hex, role: entry.role };
      const role = guessRole(e, i + 1, bgHex, taken);
      if (!role) return;                       // no free role fits — dropped
      taken.add(role);
      colors[role] = e.hex;
    });
    if (!colors.background) colors.background = bgHex;
    // A "text" colour that cannot be read against the background is not
    // text — it is a border or a surface the model mislabelled. Below 3:1
    // the role is dropped, and with it its accessibility row.
    const MIN_TEXT_CONTRAST = 3;
    ['text-primary', 'text-secondary', 'text-muted'].forEach(r => {
      if (colors[r] && contrast(colors[r], colors.background) < MIN_TEXT_CONTRAST) delete colors[r];
    });
    colors._meta = { isDark: bgL < 50, inverseButton: false, confidence: EST, resolve: v => hex(v) };

    const cls = hasText ? (profile.typography && profile.typography.classification) : null;
    const families = hasText ? ((profile.typography && profile.typography.families) || []) : [];
    // Suggestions only when there is a type direction to suggest for.
    const suggested = cls ? suggestForClassification(cls) : [];

    // Fonts: whatever was legibly read, plus labelled suggestions.
    const fonts = families.map(f => ({
      family: f, availability: 'read from the image — availability not checked',
      hosting: 'unknown', steps: [], selfHosted: false, alternative: null,
      openlyLicensed: isOpenlyLicensed(f), confidence: EST, suggested: false,
    })).concat(suggested.map(f => ({
      family: f, availability: 'suggested open font for this classification',
      hosting: 'Google Fonts', steps: [], selfHosted: false, alternative: null,
      openlyLicensed: true, confidence: EST, suggested: true,
    })));

    const typography = {
      confidence: EST,
      stacks: families.length ? { body: families[0] } : {},
      scale: {}, detail: {}, weights: [],
      families: families.map(f => ({ family: f, steps: [] })),
      direction: cls || (hasText && profile.typography && profile.typography.weightCharacter) ? {
        classification: cls || null,
        weightCharacter: (profile.typography && profile.typography.weightCharacter) || null,
        confidence: EST,
        evidence: ev.typography,
      } : null,
    };

    // Icons are held to the ≥3 rule retroactively; the other facets to
    // "present or insufficient".
    const icCount = profile.iconography && Number.isFinite(profile.iconography.count) ? profile.iconography.count : null;
    if (icCount === null || icCount < 3) ev.iconography = INSUFFICIENT;
    if (!profile.effects || !profile.effects.length) ev.effects = INSUFFICIENT;
    if (!profile.container) ev.container = INSUFFICIENT;
    if (!profile.hierarchy || !profile.hierarchy.length) ev.hierarchy = INSUFFICIENT;

    const shape = profile.shape ? { language: profile.shape, clipPath: null, size: null, confidence: EST, evidence: ev.shape } : null;
    // Insufficient evidence yields NO icon values — a renderer shows the
    // sentence, never a guessed weight next to a single chevron.
    const iconography = ev.iconography === INSUFFICIENT
      ? { confidence: EST, evidence: INSUFFICIENT, style: null, weight: null, corners: null, grid: null, count: icCount }
      : Object.assign({ confidence: EST, evidence: ev.iconography }, profile.iconography);
    const effects = ev.effects === INSUFFICIENT ? [] : profile.effects.map(e => Object.assign({ confidence: EST }, e));
    const container = ev.container === INSUFFICIENT ? null : Object.assign({ confidence: EST, evidence: ev.container }, profile.container);
    const hierarchy = ev.hierarchy === INSUFFICIENT ? [] : profile.hierarchy.slice(0, 3);

    const moods = profile.mood || [];
    // One word once: a mood of "soft" next to a shape of "soft" is one soft.
    const styleWords = [bgL < 50 ? 'dark' : 'light']
      .concat(profile.density ? [profile.density] : [])
      .concat(shape ? [shape.language] : [])
      .concat(moods.slice(0, 3))
      .filter(Boolean).map(w => String(w).trim().toLowerCase())
      .filter((w, i, arr) => w && arr.indexOf(w) === i);
    const style = styleWords.join(' ') + ' direction';

    return {
      sourceType: 'image',
      confidence: EST,
      imageKind: imageKind,
      hasInterface: NO_INTERFACE_KINDS.indexOf(imageKind) === -1,
      hasText: hasText,
      source: { url: opts.sourceUrl || '', name: opts.name || 'Image', viewport: null },
      theme: { isDark: bgL < 50, style: style, inverseButton: false, confidence: EST },
      colors: colors,
      colorRoles: colorKeys(colors),
      typography: typography,
      // A static image cannot yield these. They are null, not empty, so a
      // renderer can tell "not measurable from this source" from "measured
      // and found nothing".
      spacing: null,
      radius: {},
      shape: shape,
      ...
      heroSurface: null,
      fonts: fonts,
      // Image-only facets, each flagged, each carrying its evidence.
      iconography: iconography,
      illustration: profile.illustration ? { style: profile.illustration, confidence: EST, evidence: ev.illustration } : null,
      density: profile.density ? { level: profile.density, confidence: EST, evidence: ev.density } : null,
      mood: moods.length ? { words: moods, confidence: EST, evidence: ev.mood } : null,
      effects: effects,
      container: container,
      hierarchy: hierarchy,
      evidence: ev,
  ...
  return {
    buildDesignModel,
    buildModelFromStyleProfile,
    parseStyleProfile,
    PROFILE_SHAPE,
    IMAGE_ROLES, EVIDENCE_FIELDS, INSUFFICIENT, IMAGE_KINDS, NO_INTERFACE_KINDS, EFFECT_TYPES,
    descriptionCut: desc,
