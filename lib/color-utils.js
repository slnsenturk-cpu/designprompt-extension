...
function extractSemanticColors(vars) {
  ...
  for (const [k, v] of Object.entries(vars)) {
    ...
    if (!/^#[0-9a-f]{3,8}$/i.test(trimmed)) continue;
    // Reject semi-transparent 8-char hex (alpha < 0xCC ~80%) for role assignment
    if (trimmed.length === 9 && parseInt(trimmed.slice(7), 16) < 0xCC) continue;
    // Normalize 8-char hex to 6-char (strip full/near-full alpha)
    const hex = trimmed.length === 9 ? trimmed.slice(0, 7) : trimmed;
