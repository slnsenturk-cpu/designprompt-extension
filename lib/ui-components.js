...
const VD_UI = (() => {
  ...
  //
  // §3: icon AND label on every tab — an icon-only bar is not accepted. The
  // Settings tab carries a dot when signed out, which is the only place the
  // account is advertised: there is no floating sign-in pill anywhere.
  function tabBar(o) {
    const active = o.active || 'overview';
    const ready = !!o.ready;
    const items = TABS.map(t => {
      const dim = t.category && !ready;
      const dot = (t.id === 'settings' && o.signedOut)
        ? '<span class="vd-tab__dot" aria-hidden="true"></span>' : '';
      return `<button class="vd-tab${t.id === active ? ' is-on' : ''}${dim ? ' is-dim' : ''}"`
        + ` role="tab" aria-selected="${t.id === active}"`
        + (dim ? ' aria-disabled="true"' : '')
        + (t.id === 'settings' && o.signedOut ? ' aria-describedby="vdSignedOut"' : '')
        + ` data-tab="${esc(t.id)}">`
        + `<span class="vd-tab__icon" aria-hidden="true">${esc(t.icon)}</span>${dot}`
        + `<span class="vd-tab__label">${esc(t.label)}</span></button>`;
    }).join('');
    return `<nav class="vd-tabbar" role="tablist" aria-label="Sections">${items}`
      + (o.signedOut ? '<span id="vdSignedOut" class="vd-sr">Not signed in</span>' : '')
      + '</nav>';
  }
  ...
  return {
    ...
    section, card, notice, sheet, tabBar, tabTitle,
...
const VD_VIEWS = (() => {
  ...
  function overviewView(model, o) {
    ...
    const parts = [U.tabTitle('Overview')];
    ...
    parts.push('<div class="vd-home__action">' + pageAction(opts) + aiIndicator(opts) + '</div>');
