...
const VD_VIEWS = (() => {
  ...
  function imageSnapshotRows(model) {
    ...
    if (model.iconography && model.iconography.style) {
  ...
  function imageOverviewView(model, o) {
    ...
    parts.push('<div class="vd-imghead">'
      ...
      + (opts.image && opts.image.name ? `<span class="vd-imghead__name">${esc(opts.image.name)}</span>` : '')
      + '</div>');
  ...
  function imageComponentsView(model) {
    const parts = [U.tabTitle('Components')];
    const body = [];
    if (model.shape && model.shape.language) body.push(kvRow({ label: 'Shape language', value: model.shape.language, mono: false, meta: 'est.' }));
    const ic = model.iconography || {};
    ...
    if (model.illustration) body.push(kvRow({ label: 'Illustration / texture', value: model.illustration.style, mono: false, wrap: true, meta: 'est.' }));
    if (model.density) body.push(kvRow({ label: 'Density', value: model.density.level, mono: false, meta: 'est.' }));
    parts.push(section({ title: 'Components', action: 'Export ▸', actionId: 'gotoExport',
      body: body.join('') || '<p class="vd-line">No component language could be read from this image.</p>' }));
    return parts.join('');
  }
