...
const VD_UI = (() => {
  ...
  const COPY = {
    ...
    // Unpacked builds sign in through Supabase directly, not the site.
    devSignIn: 'DEV · bridge sign-in',
  ...
  // The caption an unpacked build shows under a Sign in button: this build
  // does not go through vibedesign.tech.
  function devCaption(dev) {
    return dev ? `<span class="vd-dev-caption">${esc(COPY.devSignIn)}</span>` : '';
  }
  ...
  return {
    ...
    section, card, notice, sheet, tabBar, tabTitle, accountControl, devCaption,
...
const VD_VIEWS = (() => {
  ...
  function imageOverviewView(model, o) {
    ...
    // The thumbnail is a button: pressing it is the same as "New image".
    parts.push('<div class="vd-imghead">'
      + (opts.image && opts.image.dataUrl
          ? `<button class="vd-imghead__thumbbtn" data-action="newImage" aria-label="${esc(COPY.newImage)}" title="${esc(COPY.newImage)}">`
            + `<img class="vd-imghead__thumb" src="${esc(opts.image.dataUrl)}" alt="" /></button>` : '')
    ...
    // §4.6: two actions after a result. Re-analyze runs the same image again;
    // New image returns to the drop zone. The result just shown stays in
    // history either way. Both are ghost: §1.1 gives a screen one primary
    // action, and on Overview that is Export — Re-analyze leads the pair by
    // position and its icon, not by weight.
    parts.push('<div class="vd-home__action vd-home__action--pair">'
      + U.button({ id: 'analyzeImageBtn', full: true, variant: 'ghost', icon: '↺', label: COPY.reanalyzeImage })
      + U.button({ id: 'vdNewImage', full: true, variant: 'ghost', label: COPY.newImage, action: 'newImage' })
