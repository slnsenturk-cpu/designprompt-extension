...
//
// Deliberately does NOT re-export getSession()/getUser(). Both route through
// gotrue-js __loadSession(), which mints a new token when the current one is
// within 90s of expiring — and that is not gated by autoRefreshToken:false.
// The service worker is the only component allowed to refresh (see
// lib/auth.js#refreshTokenIfNeeded). Callers that need the user should pass
// the JWT explicitly: client.auth.getUser(session.access_token).
