...
(function (global) {
  ...
  // Every Supabase request must be preceded by this. The SDK refreshes the
  // token itself once it is within 90s of expiring, and only the service
  // worker is allowed to refresh — so we top the token up before handing the
  // SDK anything. Best-effort: if this fails we still proceed with whatever
  // is stored rather than silently dropping the user's work.
  async function _ensureFresh() {
    try {
      if (global.VD_AUTH && typeof global.VD_AUTH.ensureFreshToken === 'function') {
        return await global.VD_AUTH.ensureFreshToken();
      }
    } catch (e) {
      console.warn('[vd-cloud] ensureFreshToken failed, proceeding with stored session', e);
    }
    return null;
  }
  ...
  async function _currentUserId() {
    try {
      ...
      if (global.VD_AUTH && typeof global.VD_AUTH.peekSession === 'function') {
        sess = await global.VD_AUTH.peekSession();
        if (sess && sess.user && sess.user.id) return sess.user.id;
      }
      if (!sess || !sess.access_token) return null; // no token, no identity
      var c = _client();
      if (!c) return null;
      // The JWT is passed explicitly on purpose: a bare getUser() routes
      // through gotrue-js's __loadSession(), which mints a new token when the
      // current one is within 90s of expiry, and the service worker is the
      // only component allowed to do that (lib/auth.js#refreshTokenIfNeeded).
      var r = await c.auth.getUser(sess.access_token);
