...
const VD_DESIGN_MD = (() => {
  ...
  function deriveColors(t) {
    ...
    // Named CSS vars are the strongest signal for primary.
    let varRoles = {};
    try {
      if (CU.extractSemanticColors && t.cssVars) varRoles = CU.extractSemanticColors(t.cssVars) || {};
    } catch (e) { varRoles = {}; }

    const btn = t.buttonStyles && t.buttonStyles.primary;
    take('primary', (btn && btn.backgroundColor) || varRoles.primary || palette.find(c => hsl(c).s > 25));

    const bg = hex(t.pageBackground) || (byUsage(usage, 'bg')[0] || {}).hex;
    const isDark = bg ? hsl(bg).l < 50 : false;

    if (out.primary) {
      // Hover moves away from the page background — darker on light, lighter on dark.
      take('primary-hover', shiftLightness(out.primary, isDark ? 0.12 : -0.12));
    }

    take('background', bg);
    take('surface', t.cardStyles && t.cardStyles.backgroundColor);
    // Next most-used backgrounds fill surface (if the card matched the page
    // background and was deduped away) and then surface-raised.
    // Only colors whose dominant role is 'bg' may become surfaces; likewise
    // borders. Otherwise a heavily-used border tone gets consumed as a surface
    // and the border tokens come out empty.
    const bgRanked = byUsage(usage, 'bg').filter(c => dominantRole(usage, c.hex) === 'bg');
    const nextBg = () => (bgRanked.find(c => !seen.has(c.hex)) || {}).hex;
    if (!out.surface) take('surface', nextBg());
    take('surface-raised', nextBg());

    const borderRanked = byUsage(usage, 'border').filter(c => dominantRole(usage, c.hex) === 'border');
    const nextBorder = () => (borderRanked.find(c => !seen.has(c.hex)) || {}).hex;
    take('border', nextBorder());
    take('border-subtle', nextBorder());

    const body = t.typographyPatterns && t.typographyPatterns.body;
    const heading = t.typographyPatterns && t.typographyPatterns.h1;
    // Candidates: every color the page actually uses for text, plus the
    // measured body/heading colors in case usage data is missing.
    const textCandidates = byUsage(usage, 'text')
      .filter(c => dominantRole(usage, c.hex) === 'text').map(c => c.hex);
    [hex(body && body.color), hex(heading && heading.color)].forEach(c => {
      if (c && textCandidates.indexOf(c) === -1) textCandidates.push(c);
    });
    const ranked = out.background
      ? textCandidates.slice().sort((a, b) => {
          const d = (contrast(b, out.background) || 0) - (contrast(a, out.background) || 0);
          return d !== 0 ? d : a.localeCompare(b); // deterministic tie-break
        })
      : textCandidates;
    const freeText = () => ranked.filter(c => !seen.has(c));
    take('text-primary', freeText()[0]);
    take('text-secondary', freeText()[0]);
    take('text-muted', freeText()[0]);

    // Status colors: first unclaimed palette entry inside each hue window.
    ...
    // Whatever vivid colors remain become accents, in palette order.
    let n = 1;
    palette.forEach(c => {
      if (seen.has(c) || hsl(c).s < 20) return;
      if (n > 4) return;
      take('accent-' + n, c);
      n += 1;
    });
  ...
  const SCALE_KEYS = ['h1', 'h2', 'h3', 'h4', 'body', 'small'];
  ...
  function deriveTypography(t) {
    ...
    // "small" isn't extracted directly; caption/label are its real-world stand-ins.
    const source = { h1: tp.h1, h2: tp.h2, h3: tp.h3, h4: tp.h4, body: tp.body, small: tp.caption || tp.label };
    ...
    out.tracking = (Array.isArray(t.letterSpacings) ? t.letterSpacings : []).map(len).filter(Boolean).slice(0, 4);
    return out;
  }
  ...
  function deriveShadows(t) {
    ...
    const out = {};
    out.sm = ranked[0];
    if (ranked.length > 2) out.md = ranked[Math.floor(ranked.length / 2)];
    if (ranked.length > 1) out.lg = ranked[ranked.length - 1];
    // A glow is a shadow with no offset — "0 0 Npx".
    const glow = ranked.find(s => /^0 0 [0-9.]+px/.test(s) || /^0px 0px [0-9.]+px/.test(s));
    if (glow) out.glow = glow;
    // Drop duplicates that collapsed onto the same string.
    const seen = new Set();
    Object.keys(out).forEach(k => {
      if (seen.has(out[k])) delete out[k]; else seen.add(out[k]);
    });
  ...
  function characterize(t, colors, spacing, radius) {
    const words = [];
    ...
    const sat = colors.primary ? hsl(colors.primary).s : null;
    if (sat !== null) words.push(sat >= 60 ? 'vivid' : (sat <= 15 ? 'monochrome' : 'restrained'));
  ...
  function buildFrontmatter(ctx) {
    ...
    if (!isEmpty(colors)) {
      L.push('colors:');
      Object.keys(colors).forEach(k => L.push(`  ${k}: ${yamlStr(colors[k])}`));
    }
  ...
  function sectionColorUsage(ctx) {
    ...
    const ROLE_NOTES = {
      ...
      'primary-hover': 'Hover/pressed variant of the primary action.',
  ...
  function sectionTypography(ctx) {
    ...
    let out = '## Typography\n';
    if (!isEmpty(typo.stacks)) {
      out += '\n' + table(['Role', 'Stack'],
        Object.keys(typo.stacks).map(k => [titleCase(k), '`' + typo.stacks[k] + '`']));
    }
    if (!isEmpty(typo.scale)) {
      const rows = SCALE_KEYS.filter(k => typo.scale[k]).map(k => {
        const [size, weight, lh] = typo.scale[k].split(' / ');
        const rem = remOf(size);
        return [`\`${k}\``, rem ? `${size} (${rem})` : size, weight || '—', lh || '—'];
      });
      out += '\n\n### Scale\n\n' + table(['Step', 'Size', 'Weight', 'Line height'], rows);
    }
    if (typo.weights && typo.weights.length) {
      out += `\n\nWeights in use: ${typo.weights.map(w => '`' + w + '`').join(', ')}.`;
    }
    if (typo.tracking && typo.tracking.length) {
      out += `\n\nLetter-spacing values: ${typo.tracking.map(v => '`' + v + '`').join(', ')}.`;
    }
    out += '\n\nUse the heading stack for `h1`–`h4` only; body copy and UI labels use the body stack. Never substitute a font that is not listed here.';
    return out;
  }
  ...
  function sectionIconography(ctx) {
    ...
    if (vp.hasGlassmorphism === true) add('Glassmorphism', 'yes');
    if (vp.hasNoiseTexture === true) add('Noise texture', 'yes');
    add('Gradient style', word(vp.gradientStyle));
  ...
  function sectionDont(ctx) {
    ...
    if (colors.primary) items.push(`Do not use \`${colors.primary}\` for large background fills — it is an accent.`);
  ...
  function sectionMotion(ctx) {
    ...
    const trans = (Array.isArray(t.transitions) ? t.transitions : []).map(timing).filter(Boolean).slice(0, 5);
  ...
  function sectionInteractionStates(ctx) {
    ...
    if (rows.length) out += table(['Component', 'State', 'Change'], rows) + '\n\n';
    const notes = [];
    if (btnTrans) notes.push(`Buttons transition with \`${btnTrans}\`.`);
    notes.push('Focus: render a visible focus ring on every interactive element — 2px, offset 2px, using `primary` or `border`.');
    notes.push('Active: apply a small downward shift or a darker fill; never remove the focus ring.');
    notes.push('Disabled: reduce opacity to ~0.5 and remove hover/active feedback entirely.');
  ...
  function buildContext(tokens, opts) {
    ...
    const shadows = deriveShadows(t);
    const breakpoints = deriveBreakpoints(t);
    const name = siteName(opts.sourceUrl || t.url || '');
    const style = characterize(t, colors, spacing, radius);
    return { t, colors, typo, spacing, radius, shadows, breakpoints, name, style, opts, tier: opts.tier };
  }
