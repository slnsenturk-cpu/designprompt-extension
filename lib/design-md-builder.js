...
const VD_DESIGN_MD = (() => {
  ...
  function resolveInString(value, resolve) {
    ...
    return value.replace(/var\(\s*(--[a-z0-9-_]+)\s*(?:,[^)]*)?\)/gi, (m, name) => {
      const r = resolve(m);
  ...
  function buttonBlock(t) {
    ...
    ['primary', 'secondary', 'ghost'].forEach(variant => {
      ...
      const rows = componentRows([
        ...
        ['Border', borderValue(b.border, ctxResolve)],
  ...
  function sectionInteractionStates(ctx) {
    ...
    // Computed styles arrive hyphenated; some capture paths use camelCase.
    const PROPS = [
    ...
    (Array.isArray(t.hoverStates) ? t.hoverStates : []).forEach(h => {
      ...
      PROPS.forEach(([label, keys]) => {
        const raw = pick(h, keys);
        if (raw === null) return;
        let shown = null;
        if (label === 'opacity' || label === 'transform') {
          shown = /^[a-z0-9 ().,%-]{1,40}$/i.test(String(raw).trim()) ? String(raw).trim() : null;
        } else if (label === 'box-shadow') {
          // Keep the geometry; resolve the colour, or name the custom
          // property when the capture never recorded it.
          shown = resolveInString(String(raw), resolve);
          if (shown && shown.length > 90) shown = null;
        } else {
          shown = resolve(raw) || resolveInString(String(raw), resolve);
          if (shown && !/^#|^--/.test(shown)) shown = null;
        }
        if (shown) diffs.push(`${label}: \`${shown}\``);
      });
      // ::before layers carry the real fill on chamfered/inverse buttons.
      if (h.before && typeof h.before === 'object') {
        const bBg = resolve(h.before.background);
        const bFg = resolve(h.before.color);
        if (bBg) diffs.push(`::before background: \`${bBg}\``);
        if (bFg) diffs.push(`::before color: \`${bFg}\``);
      }
    ...
    if (rows.length) {
      ...
      out += '### Measured\n\nObserved on the live page. Colours are resolved over the page background.\n\n'
  ...
  function sectionComponentAnatomy(ctx) {
    ...
    if (mg && typeof mg === 'object') {
      ...
      gridRows.push([word(mg.class) || word(mg.name) || 'masonry grid', bits.join(', ')]);
    }
    ...
    (Array.isArray(t.sectionContentMap) ? t.sectionContentMap : []).forEach(sec => {
      ...
      const cols = colPart.split(/\s+/).filter(x => /px|fr/.test(x) && parseFloat(x) > 20).length;
      if (cols > 1) gridRows.push([word(sec.type) || 'section grid', `${cols} columns — \`${g}\``]);
    });
    if (gridRows.length) {
