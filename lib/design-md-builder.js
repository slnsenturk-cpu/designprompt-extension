...
const VD_DESIGN_MD = (() => {
  ...
  function sectionLayout(ctx) {
    ...
    add('Card gap', len(t.cardStyles && t.cardStyles.gap) ? withRem(t.cardStyles.gap) : null);
    const heroBg = ctx.colors._meta && ctx.colors._meta.resolve
      ? ctx.colors._meta.resolve(vp.heroBackground || vp.heroBg || (t.layoutInfo || {}).heroBackground)
      : null;
    if (heroBg) {
      add('Hero background', '`' + heroBg + '`'
        + (vp.isFullBleed === true || vp.hasFullBleedImages === true ? ' (full-bleed)' : ''));
    }
    ...
    if (mg && typeof mg === 'object') {
      const cols = int(mg.columns);
      add('Masonry grid', (word(mg.name) || word(mg.type) || 'present')
        + (cols !== null ? `, ${cols} columns` : ''));
    }
    if (tp.hasPricingGrid === true) {
      const pc = int(tp.pricingColumnCount);
      add('Pricing grid', pc !== null ? `${pc} columns` : 'present');
    }
    if (tp.hasAccordion === true) add('Accordion', 'present');
    if (tp.hasMarquee === true) add('Marquee', 'present');
    if (tp.hasLogoStrip === true) add('Logo strip', 'present');
    if (tp.hasTabSection === true) add('Tab section', 'present');
    if (tp.hasTestimonialCarousel === true) add('Testimonial carousel', 'present');
    if (tp.hasStepIndicator === true) add('Step indicator', 'present');
