...
const VD_DESIGN_MD = (() => {
  ...
  // The variant a rule targets, taken from the selector's FIRST class. Class
  // names are structural and safe to print; element text never is.
  function variantFromSelector(sel) {
    if (typeof sel !== 'string' || !sel) return null;
    const cleaned = sel
      .replace(/\[[^\]]*\]/g, '')      // [data-astro-cid-…]
      .replace(/:not\([^)]*\)/g, '')
      .replace(/::?[a-z-]+/gi, '')      // :hover, ::before
      .replace(/\s+/g, ' ').trim();
    const m = cleaned.match(/\.([a-z][a-z0-9_-]*)/i);
    if (!m) return null;
    return m[1].length <= 40 ? m[1] : null;
  }
  ...
  function sectionInteractionStates(ctx) {
    ...
    (Array.isArray(t.hoverStates) ? t.hoverStates : []).forEach(h => {
      ...
      const variant = variantFromSelector(h.selector);
      // Composite over whichever surface this element was measured on.
      const resolve = ctxResolveFor(variant);
      const diffs = [];
  ...
  function buildContext(tokens, opts) {
    ...
    return { t, colors, typo, spacing, radius, shadows, breakpoints, shape, heroSurface,
             name, style, opts, tier: opts.tier };
  }
