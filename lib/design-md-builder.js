...
const VD_DESIGN_MD = (() => {
  ...
  function buildEstimatedDesignMd(model, opts) {
    ...
    if (dir.classification || dir.weightCharacter) {
      L.push('typography:');
    ...
    // Visual direction
    {
      ...
      if (model.illustration) b.push(`Illustration / texture: ${model.illustration.style}.`);
      parts.push(b.join('\n\n'));
    }
    ...
    // Fonts
    if (readFonts.length || suggFonts.length) {
    ...
    // Components — shape, iconography, illustration, density. No anatomy.
    {
      ...
      const rows = [];
      if (model.shape && model.shape.language) rows.push(['Shape language', model.shape.language]);
      ...
      if (model.illustration) rows.push(['Illustration / texture', model.illustration.style]);
      if (model.density) rows.push(['Density', model.density.level]);
