...
const VD_DESIGN_MD = (() => {
  ...
  // Where a family can actually be obtained, read from the host serving it.
  const FONT_HOSTS = [
  ...
  // Open alternatives by classification. Marked as suggestions everywhere they
  // are printed — these are not observed on the source.
  ...
  function sectionFonts(ctx) {
    ...
    // Which scale steps each family drives, from the typography model.
    const stepsFor = fam => {
      const entry = (typo.families || []).find(f => f.family === fam
    ...
    assets.forEach(a => {
      ...
      rows.push(['`' + family + '`', availability,
        steps.length ? steps.map(k => '`' + k + '`').join(', ') : '—']);
      if (/^self-hosted/.test(availability)) {
        const alt = suggestAlternative(family, steps);
        // The alternatives list contains only openly-licensed families, so a
        // family that turns up in its own suggestion is itself open — the site
        // merely chose to self-host it. Say that instead of recommending the
        // family to itself.
        const parts = alt.split(' or ').map(x => x.trim());
        const isSelf = parts.some(x => x.toLowerCase() === family.toLowerCase());
        if (isSelf) {
          suggestions.push([`\`${family}\``,
            'openly licensed — self-hosted here, but obtainable directly']);
        } else {
          suggestions.push([`\`${family}\``, alt]);
        }
      }
    });
    if (!rows.length) return null;
    ...
    if (suggestions.length) {
      out += '\n\n### Substitutes — suggested, not observed\n\n'
        + 'The families above marked self-hosted are licensed to the source site and '
        + 'cannot be reused. These are open alternatives with a comparable classification. '
        + 'They are **suggestions, not measurements** — nothing here was seen on the page.\n\n'
        + table(['Unavailable family', 'Open alternative (suggested)'], suggestions);
    }
    return out;
  }
  ...
  function sectionInteractionStates(ctx) {
    ...
    if (unique.length) {
      out += '### Measured\n\n'
        ...
        + 'not captured.\n\n'
        + table(['Component', 'Variant', 'Base → Hover'],
            unique.map(r => [r.component, r.variant ? '`' + r.variant + '`' : '—', r.change]))
        + '\n\n';
  ...
  function buildContext(tokens, opts) {
    ...
    // Hero-measured variants: the CTA buttons and any link sampled there.
    ...
    return { t, colors, typo, spacing, radius, shadows, breakpoints, shape, heroSurface,
             name, style, opts };
  }
