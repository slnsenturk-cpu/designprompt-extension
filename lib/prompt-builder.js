...
function generateComponentGuidance(data, style, specsData) {
  ...
  if (data.navStructure?.layout === 'pill-group' && data.navStructure.pillGroup) {
    ...
    lines.push(` Nav link pill group: ${pg.linkCount} links inside a rounded container — radius \`${pg.radius}\`${pg.backdrop ? `, backdrop: \`${pg.backdrop}\`` : ''}${pg.bg && !isTransparent(pg.bg) ? `, bg: \`${pg.bg}\`` : ''}${pg.border ? `, border: \`${pg.border}\`` : ''}.`);
  }
