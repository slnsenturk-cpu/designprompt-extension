...
function buildImagePrompt(model) {
  ...
  if (model.illustration) lines.push(`- Illustration / texture: ${model.illustration.style}`);
  lines.push('');
  ...
  lines.push('## Shape and iconography (estimated)');
  if (model.shape && model.shape.language) lines.push(`- Shape language: ${model.shape.language}`);
  if (ic.style) lines.push(`- Icons: ${[ic.style, ic.weight, ic.corners, ic.grid && ic.grid + ' grid'].filter(Boolean).join(', ')}`);
  lines.push('');
