...
function safeHostname(url) { try{const h=new URL(url).hostname; return h||'unknown';}catch{return url&&url.length<80?url:'unknown';} }
...
function analyzeDesignStyle(data) {
  ...
  // Exclude inset shadows — those are surface depth effects, not brutalist
  const hasBrutalistShadow = shadows.some(s => !s.includes('inset') && /[3-9]\d*px\s+[3-9]\d*px\s+0(px)?\s/.test(s));
...
function resolvedPrimaryActionColor(data, style) {
  ...
  // Priority 2: button background — must have meaningful contrast against page bg.
  // On light sites (#fff bg), a very light button (#e6e6e6) is a surface color, not a CTA.
  // On dark sites (#0a0a0a bg), a very dark button is also not distinctive enough.
  if (isPlausibleHexColor(btnBg)) {
    const btnLum = hexLum(btnBg);
    const pageBgLum = style?.pageBgLum ?? 0.9;
    const contrastDiff = Math.abs(btnLum - pageBgLum);
    // Button must have at least 0.15 luminance difference from page bg to be a real CTA
    if (contrastDiff > 0.15 && btnLum > 0.02 && btnLum < 0.98) return btnBg;
  }
  ...
  const vivid = accents.find(c => isPlausibleHexColor(c) && hexSat(c) > 25);
  if (vivid) return vivid;

  // Priority 4: on monochromatic sites, the CTA may be the darkest or most contrasting color
  // (e.g. black button on white site). Find the color with highest contrast against page bg.
  const pageBgLum = style?.pageBgLum ?? 0.9;
  const allCandidates = [...accents, ...(data?.colors || [])].filter(c => isPlausibleHexColor(c));
  if (allCandidates.length > 0) {
    const sorted = allCandidates
      .map(c => ({ c, contrast: Math.abs(hexLum(c) - pageBgLum) }))
      .filter(x => x.contrast > 0.2) // must be meaningfully different from bg
...
function trimPagePromptByPriority(lines, maxLines = MAX_PROMPT_LINES) {
  if (lines.length <= maxLines) return lines;

  // 3-tier priority: critical > important > trimmable
  const critical = [];   // Core tokens, section map, component patterns
  const important = [];  // Design Direction, Design Brief, Layout
  const trimmable = [];  // Shadow Tokens, Design Specs, verbose extras
  let section = '';

  for (const line of lines) {
    if (/^### /.test(line)) section = line;

    const isCritical = /^### Core Spec|^### (Color Tokens|Typography Tokens|Shape Tokens|Motion Tokens)$/.test(section) ||
      /^### Interaction Paradigm$/.test(section) ||
      /^### (Section Content Map|Component Patterns)$/.test(section) ||
      /^Implementation notes:/.test(line) ||
      /^CRITICAL/.test(line) ||
      /^Add to global/.test(line) ||
      /^@layer base/.test(line);

    const isImportant = /^### (Design Direction|Design Brief|Layout & Page Structure)$/.test(section);

    if (isCritical) critical.push(line);
    else if (isImportant) important.push(line);
    else trimmable.push(line);
  }

  // Build output: all critical, then important, then trimmable until budget
  const out = [...critical];
...
function generateRuleBasedDirection(data, style) {
  ...
  if (vpr.hasParallaxHint) lines.push('Parallax: background moves at different speed than foreground content.');
  ...
  if ((tabComps && tabComps.length > 0) || (fixedChrome && fixedChrome.length > 0)) {
    ...
    if (tabComps && tabComps.length > 0) {
      tabComps.forEach(tc => {
        if (tc.type === 'tab-switcher' || tc.type === 'numbered-switcher') {
...
function generateComponentGuidance(data, style, specsData) {
  ...
  const _navLinksNote = _navLinkLabels.length > 0 ? ` Nav links: ${_navLinkLabels.join(' · ')}.` : '';
  ...
  // ── Nav pill group structure ──
  if (data.navStructure?.layout === 'pill-group' && data.navStructure.pillGroup) {
    const pg = data.navStructure.pillGroup;
    const _pgBgVis = pg.bg && pg.bg !== 'rgba(0, 0, 0, 0)' && pg.bg !== 'transparent';
    lines.push(` Nav link pill group: ${pg.linkCount} links inside a rounded container — radius \`${pg.radius}\`${pg.backdrop ? `, backdrop: \`${pg.backdrop}\`` : ''}${_pgBgVis ? `, bg: \`${pg.bg}\`` : ''}${pg.border ? `, border: \`${pg.border}\`` : ''}.`);
  }
  ...
  if (bs.primary) {
    ...
    if (primaryHover) {
      ...
      if (primaryHover.before) {
        ...
        lines.push(`  Hover: ${_afterEntries.map(([k,v]) => `\`${k}: ${v}\``).join(' · ')}`);
      }
  ...
  // ── Background patterns: grain, scanlines, grid lines ──
  const bgPatterns = data.backgroundPatterns || [];
  const _noiseBgAsset = (data.assets?.backgrounds || []).find(b => /grain|noise/i.test(b.url || ''));
  const hasAnyPattern = bgPatterns.length > 0 || _noiseBgAsset || data.visualProfile?.hasNoiseTexture;

  if (hasAnyPattern) {
    // Render each detected pattern with exact CSS values
    const grainPattern = bgPatterns.find(p => p.type === 'svg-grain' || p.type === 'grain-filter');
    const scanlinePattern = bgPatterns.find(p => p.type === 'scanlines');
    const gridPattern = bgPatterns.find(p => p.type === 'grid-lines');

    if (grainPattern || _noiseBgAsset || data.visualProfile?.hasNoiseTexture) {
      lines.push('**Noise/grain texture overlay (apply to full page):**');
      if (grainPattern?.type === 'svg-grain') {
        lines.push('```html');
        lines.push(`<svg style="position:fixed;inset:0;width:100%;height:100%;pointer-events:none;z-index:10001;opacity:${grainPattern.opacity || 0.08}">`);
        lines.push(`  <filter id="grain"><feTurbulence baseFrequency="${grainPattern.baseFrequency}" numOctaves="${grainPattern.numOctaves}" seed="${grainPattern.seed}"/>`);
        lines.push('  <feComponentTransfer><feFuncA type="discrete" tableValues="0 1"/></feComponentTransfer></filter>');
        lines.push('  <rect width="100%" height="100%" filter="url(#grain)"/>');
        lines.push('</svg>');
        lines.push('```');
      } else if (_noiseBgAsset?.url) {
        lines.push(`  \`background-image: url(${_noiseBgAsset.url}); position:fixed; inset:0; z-index:2; pointer-events:none;\``);
      } else {
        lines.push('  Use SVG feTurbulence or a grain PNG. `position:fixed; inset:0; opacity:0.08–0.15; z-index:2; pointer-events:none`.');
      }
    }

    if (scanlinePattern) {
      lines.push('**Scanline overlay (apply to full page):**');
      lines.push('```css');
      lines.push(`.scanlines { position:fixed; inset:0; pointer-events:none; z-index:10002; background:${scanlinePattern.css}; }`);
      lines.push('```');
    }

    if (gridPattern) {
      lines.push('**Grid background pattern:**');
      lines.push('```css');
      lines.push(`.grid-bg { background-image:${gridPattern.css};${gridPattern.backgroundSize ? ` background-size:${gridPattern.backgroundSize};` : ''} }`);
      lines.push('```');
    }
  }
...
function buildInteractionParadigm(data, vpr) {
  ...
  if (vpr?.hasScrollAnimation) {
    ...
    lines.push('- **Reveal:** Immediate — all content visible on load, no scroll-triggered animations.');
  }
  ...
  if (ap.hasStaggerReveal) {
    const staggerVal = mp.staggerPattern?.delayBetween || mp.staggerDelay || '80ms';
    lines.push(`- **Stagger:** Sequential element reveals with ${staggerVal} offset per item — creates reading rhythm.`);
  }
  ...
  // Motion System CSS block — structured code that AI tools can directly implement
  const revealStyle = mp.revealStyle || 'fade-up';
  const staggerDelay = mp.staggerPattern?.delayBetween || '80ms';
  let revealFrom = 'opacity: 0; transform: translateY(24px);';
  let revealTo = 'opacity: 1; transform: none;';
  if (revealStyle === 'fade-left-or-right') {
    revealFrom = 'opacity: 0; transform: translateX(30px);';
  } else if (revealStyle === 'clip-path-reveal') {
    revealFrom = 'clip-path: inset(0 0 100% 0); opacity: 0;';
    revealTo = 'clip-path: inset(0); opacity: 1;';
  } else if (revealStyle === 'scale-in') {
    revealFrom = 'opacity: 0; transform: scale(0.85);';
  } else if (revealStyle === 'fade-only') {
    revealFrom = 'opacity: 0;';
    revealTo = 'opacity: 1;';
  }

  if (vpr?.hasScrollAnimation) {
    lines.push('');
    lines.push('**Motion System (CSS):**');
    lines.push('```css');
    lines.push(`/* Scroll-reveal base — apply to animated elements */`);
    lines.push(`.reveal-on-scroll {`);
    lines.push(`  ${revealFrom}`);
    lines.push(`  transition: all ${dur} ${easing};`);
    lines.push(`}`);
    lines.push(`.reveal-on-scroll.visible {`);
    lines.push(`  ${revealTo}`);
    lines.push(`}`);
    lines.push('```');
    lines.push(`Trigger: IntersectionObserver at threshold ${mp.revealThreshold || '0.15'}, add \`.visible\` class on entry.`);
    if (ap.hasStaggerReveal) {
      lines.push(`Stagger: \`transition-delay: calc(var(--i) * ${staggerDelay})\` per child, set \`--i\` via CSS counter or JS.`);
    }
  }
...
function buildPagePrompt(data, aiDirection) {
  const site=safeHostname(data.url || state.currentUrl), vars=data.cssVars||{};
  ...
  if(focus==='all'||focus==='colors') {
    ...
    if (focus === 'all') {
      ...
      const textSecondary = allComputedColors.find(c => {
        const lum = hexLum(c);
        return style.isDark ? (lum > 0.3 && lum < 0.7 && hexSat(c) < 20) : (lum > 0.2 && lum < 0.5 && hexSat(c) < 20);
      });
  ...
  if(focus==='all'||focus==='motion') {
    ...
    if(motionVars.length>0||transitions.length>0||animations.length>0||hoverStates.length>0) {
      if (focus === 'motion') {
        ...
        // 'all' mode — structured CSS code blocks for AI tool compatibility
        ...
        if(transitions.length>0) {
          transitions.slice(0,3).forEach(t=>{
            const PROP_CTX = {
              ...
              'transform':'movement, scale, rotate', 'border-color':'border emphasis on hover',
              'box-shadow':'elevation on hover', 'filter':'color or blur filter',
            };
        ...
        // Keyframes as full CSS code blocks (max 3 full, rest as one-liners)
        if(animations.length>0) {
          const fullCount = Math.min(3, animations.length);
          animations.slice(0, fullCount).forEach(a => {
            if (typeof a === 'object' && a.name && a.steps && a.steps.length > 0) {
              // Render full @keyframes CSS block from structured steps
              let cssBlock = `@keyframes ${a.name} {\n`;
              for (const step of a.steps) {
                const props = Object.entries(step)
                  .filter(([k]) => k !== 'offset')
                  .map(([k, v]) => `    ${k}: ${v};`).join('\n');
                if (props) cssBlock += `  ${step.offset} {\n${props}\n  }\n`;
              }
              cssBlock += '}';
              lines.push(`\`\`\`css\n${cssBlock}\n\`\`\``);
            } else if (typeof a === 'object' && a.name) {
              // Legacy format fallback — render from/to text
          ...
          // Remaining keyframes as compact one-liners
          animations.slice(fullCount, 8).forEach(a => {
  ...
  if (focus === 'all' || focus === 'motion') {
    ...
    // Extended motion data from new detectors
    const mp = data.motionProfile || {};

    // Pinned sections (GSAP or CSS sticky)
    const pinnedFromGsap = (mp.gsapScrollTriggers || []).filter(t => t.pin);
    const pinnedFromCSS = mp.pinnedSections || [];
    if (pinnedFromGsap.length > 0 || pinnedFromCSS.length > 0) {
      lines.push('**Scroll-pinned sections:**');
      for (const p of pinnedFromGsap) {
        lines.push(`- \`${p.trigger}\` — pinned, scrub: ${p.scrub || 'false'}, range: ${p.start || 'top'} → ${p.end || 'bottom'}`);
        if (p.timelineChildren && p.timelineChildren.length > 0) {
          lines.push('  Timeline (scroll progress):');
          for (const c of p.timelineChildren) {
            const props = c.props ? Object.entries(c.props).map(([k,v]) => `${k}: ${v}`).join(', ') : '';
            lines.push(`  - ${c.startPct || '?'}%: \`${c.target}\` → ${props}`);
          }
        }
      }
      for (const p of pinnedFromCSS) {
        lines.push(`- \`${p.element}\` — CSS sticky, ${p.heightRatio}× viewport height, ${p.stickyCount} sticky element(s)`);
        if (p.animatedChildren) {
          for (const c of p.animatedChildren) {
            lines.push(`  - \`${c.element}\` → ${c.animation}`);
          }
        }
      }
      lines.push('');
    ...
    if (mp.scrollTimelineRules && mp.scrollTimelineRules.length > 0) {
      lines.push('**CSS scroll-timeline animations:**');
      for (const r of mp.scrollTimelineRules) {
        lines.push(`- \`${r.selector}\`: timeline \`${r.timeline}\`${r.range ? `, range: ${r.range}` : ''}`);
      }
      lines.push('');
    }
    ...
    if (data.parallaxLayers) {
      lines.push(`**Parallax:** ${data.parallaxLayers.length} layer(s) — ${data.parallaxLayers.map(l => `\`${l.element}\` (${l.type}${l.speed ? ', speed:'+l.speed : ''})`).join(', ')}`);
      lines.push('');
    }

    // SVG path drawing
    if (data.svgPathDrawing) {
      lines.push(`**SVG path drawing:** ${data.svgPathDrawing.length} animated path(s)`);
      for (const d of data.svgPathDrawing) {
        lines.push(`- ${d.element} (${d.svgSize.w}×${d.svgSize.h}px), stroke: \`${d.stroke}\`, dasharray: \`${d.dasharray}\`, pathLength: ${d.pathLength || '?'}`);
      }
      lines.push('');
    }

    // Number counters
    if (data.numberCounters) {
      lines.push(`**Number counters:** ${data.numberCounters.map(c => `\`${c.targetValue}\` (${c.element})`).join(', ')} — animate from 0 to target on scroll entry.`);
      lines.push('');
    }

    // Particle systems
    if (data.particleSystems) {
      const ps = data.particleSystems;
      lines.push(`**Particle system:** ${ps.library}${ps.fullViewport ? ' (full viewport)' : ''}, container: \`${ps.container || 'unknown'}\``);
      lines.push('');
    }

    // Spline 3D
    if (data.spline3D) {
      for (const s of data.spline3D) {
        lines.push(`**3D embed (Spline):** ${s.size.w}×${s.size.h}px, ${s.location}${s.interactive ? ', mouse-interactive' : ''}`);
      }
      lines.push('');
    }
    ...
    // Scroll snap
    if (data.scrollSnap) {
      for (const s of data.scrollSnap) {
        lines.push(`**Scroll-snap section:** \`${s.element}\` — ${s.snapType}, ${s.childCount} panels, ${s.direction}`);
      }
      lines.push('');
    }
  }
  ...
  if(focus==='all'||focus==='layout') {
    ...
    if(spacing.sectionPaddingY) lines.push(`- Section vertical padding: \`${spacing.sectionPaddingY}\` — CRITICAL: apply \`padding: ${spacing.sectionPaddingY} 0\` to EVERY \`<section>\` element`);
  ...
  if(focus==='all') {
    ...
    if (scm.length > 0) {
      ...
      scm.forEach((sec, i) => {
        ...
        // Enforce section padding explicitly per section (V0 misses global padding spec)
        const _secPadY = (vpr.spacingSystem || {}).sectionPaddingY;
        if (_secPadY) {
          const isHero = i === 0 || secType === 'hero';
          if (!isHero) {
            lines.push(`  Padding: \`${_secPadY} 0\` (top and bottom)`);
          }
        }
        ...
        if (sec.heroColumns && sec.heroColumns.length >= 2) {
          ...
          if (_splitHint === 'overlapping-float') {
        ...
        // Bento grid: asymmetric card layout with content classification
        if (sec.bentoGrid) {
          const bg = sec.bentoGrid;
          lines.push(`  **Bento grid layout** — ${bg.itemCount} cards, columns: \`${bg.columns?.slice(0, 60) || 'auto'}\``);
          for (const item of bg.items) {
            let itemDesc = `  - Card "${item.heading || '?'}" (${item.size})`;
            if (item.span) itemDesc += ` [${item.span}]`;
            if (item.content) {
              for (const c of item.content) {
                if (c.type === 'chart') itemDesc += ` → ${c.chartType} (colors: ${c.colors?.join(', ') || 'accent'})`;
                else if (c.type === 'code-snippet') itemDesc += ` → code block: \`${c.preview}\``;
                else if (c.type === 'location-map') itemDesc += ` → dot-map (${c.dotCount} dots${c.labels ? ', labels: ' + c.labels.join(', ') : ''})`;
                else if (c.type === 'badge') itemDesc += ` → badge "${c.text}"`;
                else if (c.type === 'icon') itemDesc += ` → icon (${c.size}px)`;
              }
            }
            lines.push(itemDesc);
          }
        ...
        if (secType === 'portfolio-split' && sec.portfolioGridInfo) {
          ...
          lines.push(`  Left: ${pg.cols}×${pg.rows} grid of logos — ${_logoList}.`);
          lines.push(`  Cell style: \`border: 1px solid rgba(255,255,255,0.08)\`, no border-radius, centered logo. Grid lines create a table-like layout — NOT rounded cards.`);
        ...
        // Hero-specific: entrance sequence + CTA button dimensions + floating illustrations
        if (i === 0) {
          // Hero entrance sequence timeline
          const heroSeq = data.heroEntranceSequence?.elements;
          if (heroSeq && heroSeq.length > 0) {
            lines.push('  Hero entrance sequence (sorted by delay):');
            heroSeq.slice(0, 6).forEach((el, idx) => {
              let seqLine = `    ${idx + 1}. ${el.tag.toUpperCase()} "${el.text}"`;
              seqLine += ` — ${el.delay || '0ms'}`;
              if (el.aosType) seqLine += `, ${el.aosType}`;
              else if (el.animName) seqLine += `, ${el.animName}`;
              if (el.blur) seqLine += ` + blur(${el.blur}→0)`;
              lines.push(seqLine);
            });
          }
          if (sec.heroCtaStyle) {
        ...
        if (i === 0) {
          ...
          // Hero interactive components (terminal, search bar, glass panels)
          if (data.heroInteractiveComponents) {
            for (const comp of data.heroInteractiveComponents) {
              if (comp.type === 'terminal-mockup') {
                lines.push(`  [terminal-mockup] CLI component (${comp.size.w}×${comp.size.h}px): \`${comp.text}\``);
                lines.push(`   → bg: \`${comp.bg}\`, radius: \`${comp.radius}\`, font: "${comp.fontFamily}"`);
                if (comp.hasProgressBar) lines.push('   → Contains progress bar(s) inside');
                if (comp.hasKbd) lines.push('   → Has keyboard shortcut badge (⌘K style)');
                lines.push('   → Implement as a glassmorphic card with monospace text, blinking cursor, and styled progress indicators.');
              } else if (comp.type === 'search-bar') {
                lines.push(`  [search-bar] Command palette (${comp.size.w}×${comp.size.h}px), placeholder: "${comp.placeholder}", radius: \`${comp.radius}\``);
              } else if (comp.type === 'glass-panel') {
                lines.push(`  [glass-panel] Glassmorphic UI panel (${comp.size.w}×${comp.size.h}px), blur: \`${comp.blur}\`, ${comp.childCount} children`);
              }
            }
          }
          // Gradient text in headings
          if (data.gradientText) {
            for (const gt of data.gradientText) {
              lines.push(`  [gradient-text] "${gt.text}" — \`-webkit-background-clip: text; background-image: ${gt.gradient || 'linear-gradient(...)'}\``);
            }
          }
        ...
        } else if (sec.aosConfig) {
          // Section-specific AOS animation
          const aos = sec.aosConfig;
          let aosLine = `  Entrance: AOS \`${aos.type}\`, duration \`${aos.duration}ms\`, offset \`${aos.offset}px\``;
          if (aos.easing && aos.easing !== 'ease') aosLine += `, easing \`${aos.easing}\``;
          if (aos.staggerDelays) aosLine += `. Stagger: [${aos.staggerDelays.join(', ')}]ms across ${aos.count} elements`;
          else if (aos.count > 1) aosLine += `. ${aos.count} elements animated`;
          aosLine += '.';
          lines.push(aosLine);
        } else if (sec.animationType) {
          lines.push(`  Entrance: \`${sec.animationType}\` on scroll into view.`);
        } else if (vpr.hasScrollAnimation) {
          lines.push('  '+entranceAnim);
        }
        // Per-section animation binding table
        if (sec.animationBindings && sec.animationBindings.length > 0) {
          lines.push('  Animated elements:');
          for (const b of sec.animationBindings) {
            const anim = b.animation;
            if (!anim) continue;
            const effect = anim.effect || anim.name || anim.type;
            const _hasSuffix = v => /\d+(ms|s)$/.test(v);
            const timing = anim.duration ? `${anim.duration}${_hasSuffix(anim.duration) ? '' : 'ms'}` : '';
            const delay = anim.delay && anim.delay !== '0' && anim.delay !== '0s' ? `, delay ${anim.delay}${_hasSuffix(anim.delay) ? '' : 'ms'}` : '';
            const trigger = b.trigger ? ` [${b.trigger}]` : '';
            lines.push(`  - \`${b.element}\` → ${effect} ${timing}${delay}${trigger}`);
          }
        }
        ...
        if (sec.visualDescriptions && sec.visualDescriptions.length > 0) {
          ...
          if (visuals.length > 0) {
            ...
            visuals.slice(0, 2).forEach(v => {
              ...
              } else if (/\[video-background\]|\[styled-visual\]/.test(v) && v.includes(' — ')) {
  ...
  const _trimmed = trimPagePromptByPriority(_cleaned, MAX_PROMPT_LINES);
...
function setOutputMode(mode) {
  ...
  try {
    if (mode === 'system') {
      ...
      const result = buildDesignSystemPrompt(data, style);
      if (result) state.lastPrompt = result;
    } else {
      // Preserve AI direction when switching back to full mode
      const result = buildPagePrompt(data, state.lastAiDirection || null);
      if (result) state.lastPrompt = result;
    }
  } catch(err) {
    console.warn('[VibeDesign] Output mode switch failed:', err.message);
  }
...
function buildElementPrompt(data) {
  const site=safeHostname(data.url || state.currentUrl),s=data.styles||{},platform=state.platform,lines=[],elType=guessElementType(data);
  ...
  if (_finalLines.length > MAX_PROMPT_LINES) {
    return _finalLines.slice(0, MAX_PROMPT_LINES).join('\n') +
...
async function buildImagePrompt(data) {
  ...
  try {
    const lines=[],site=safeHostname(data.url || state.currentUrl),platform=state.platform,imgFocus=state.imgFocus,palette=data.palette||[];
...
function getPlatformInstruction(p, site, data) {
  ...
  const GOOGLE_FONTS = new Set([
    'inter','inter tight','roboto','open sans','lato','montserrat','poppins','raleway','nunito','ubuntu',
    'newsreader','literata','source serif pro','source serif 4','fraunces','young serif',
  ...
  if (data?.assets?.fonts?.length > 0) {
    ...
    if (nonGoogle.length > 0) {
      ...
      // Generate copy-paste @font-face CSS block
      assetLines.push('\nAdd to global CSS:');
      assetLines.push('```css');
      nonGoogle.forEach(f => {
        const format = f.url.includes('.woff2') ? 'woff2' : f.url.includes('.woff') ? 'woff' : 'truetype';
        assetLines.push(`@font-face { font-family: "${f.family}"; src: url("${f.url}") format("${format}"); font-display: swap; }`);
      });
      assetLines.push('```');
    }
  }
  ...
  // Platform-specific shadcn token override block for v0/Lovable
  let shadcnBlock = '';
  if (p === 'v0' || p === 'lovable') {
    const sc = extractSemanticColors(data?.cssVars || {});
    const primaryHex = sc.primary || data?.accentColors?.[0] || data?.colors?.[0] || '#000000';
    const bgHex = data?.pageBackground || '#ffffff';
    const fgHex = data?.buttonStyles?.primary?.color || (hexLum(bgHex) > 0.5 ? '#000000' : '#ffffff');
    ...
    function hexToHSL(hex) {
      ...
      if(d>0) { s = d/(1-Math.abs(2*l-1)); h = max===r?(g-b)/d+(g<b?6:0):max===g?(b-r)/d+2:(r-g)/d+4; h*=60; }
      return `${Math.round(h)} ${Math.round(s*100)}% ${Math.round(l*100)}%`;
    }
    const bgHSL = hexToHSL(bgHex.length === 7 ? bgHex : '#ffffff');
    const fgHSL = hexToHSL(fgHex.length === 7 ? fgHex : '#000000');
    const primHSL = hexToHSL(primaryHex.length === 7 ? primaryHex : '#000000');
    const primFgHSL = hexToHSL(primFg.length === 7 ? primFg : '#ffffff');
    const isDark = hexLum(bgHex) < 0.2;
    shadcnBlock = `\n\nCRITICAL — Override ALL shadcn/Tailwind defaults:\n\`\`\`css\n@layer base {\n  :root {\n    --background: ${bgHSL};\n    --foreground: ${fgHSL};\n    --primary: ${primHSL};\n    --primary-foreground: ${primFgHSL};\n    --radius: ${radiusVal};\n  }\n}\n\`\`\``;
    if (isDark) {
      shadcnBlock += `\nThis is a DARK-MODE-ONLY design. Set \`<html class="dark">\` and ensure background is \`${bgHex}\`, NOT white.`;
    }
    shadcnBlock += `\nDo NOT fall back to Tailwind zinc/slate/neutral palette. Use the exact hex values from this prompt.`;
  }

  // Motion implementation note
  const mp = data?.motionProfile || {};
  const dur = mp.dominantDuration || '300ms';
  const easing = mp.dominantEasing || 'ease';
  const motionNote = (data?.visualProfile?.hasScrollAnimation)
    ? `\n- Implement scroll-reveal animations: IntersectionObserver at threshold 0.15, transition \`opacity\` and \`transform\` at ${dur} ${easing}. Apply to all section children.`
    : '';
...
- CRITICAL: Prefer the extracted color tokens over Tailwind defaults (slate, zinc, neutral, etc.). Override shadcn/ui CSS variables (--primary, --radius, --background) with the values specified above.
...
- Implement ALL keyframe animations from Motion Tokens section. Copy the CSS code blocks directly.${motionNote}
