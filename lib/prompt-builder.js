...
// ─── HSL-based fallback role inference ────────────────────────────────────────
// When extractSemanticColors() finds no CSS variable for a role, infer from extracted data
function inferSemanticRoles(data, sc) {
  ...
  // Primary action: button bg is ground truth — it's the actual CTA color users click
  if (!sc.primary) {
    if (btnBg && /^#[0-9a-f]{6}$/i.test(btnBg)) sc.primary = btnBg;
    else {
      // Fallback: highest-saturation non-background color
      const candidates = allColors.filter(c => c !== pageBg && hexSat(c) > 15);
      candidates.sort((a, b) => hexSat(b) - hexSat(a));
      if (candidates[0]) sc.primary = candidates[0];
    }
  }

  // Background: from explicit detection or lightest/darkest non-accent color
  if (!sc.bg && pageBg) sc.bg = pageBg;

  // Foreground: body text color or darkest low-saturation color on light sites
  if (!sc.fg) {
    if (bodyColor && /^#[0-9a-f]{6}$/i.test(bodyColor) && bodyColor !== '#000000') sc.fg = bodyColor;
    else {
      const isDark = pageBg && hexLum(pageBg) < 0.35;
      const textCandidates = allColors.filter(c => hexSat(c) < 10 && (isDark ? hexLum(c) > 0.7 : hexLum(c) < 0.3));
      if (textCandidates[0]) sc.fg = textCandidates[0];
    }
  }

  // Secondary: second-highest saturation color different from primary
  if (!sc.secondary) {
    const candidates = allColors.filter(c => c !== sc.primary && c !== pageBg && hexSat(c) > 15);
    candidates.sort((a, b) => hexSat(b) - hexSat(a));
