...
function resolvedPrimaryActionColor(data, style) {
  ...
  const _usageTotal = hex => {
    const u = usage[hex?.toLowerCase?.()] || {};
    return (u.bg || 0) + (u.text || 0) + (u.border || 0);
  };
  const _brandScore = hex => hexSat(hex) * (1 + Math.log10(_usageTotal(hex) + 1));

  // Priority 1: button background — the actual rendered CTA color is usually ground
  // truth. EXCEPT when extractButtonStyles mislabels a rare accent-colored tag (pink
  // badge on Attio, for example) as primary. Cross-check against the top accent using
  // the same saturation × log(freq) score as accent ranking: if the accent outranks
  // the button bg by 2x, the extractor caught an outlier — prefer the brand accent.
  if (isPlausibleHexColor(btnBg) && btnBg.toLowerCase() !== '#ffffff') {
    const topAccent = accents.find(c => isPlausibleHexColor(c) && c.toLowerCase() !== btnBg.toLowerCase() && hexSat(c) > 15);
    if (topAccent) {
      const btnScore = _brandScore(btnBg);
      const accScore = _brandScore(topAccent);
      if (hexSat(btnBg) > 15 && accScore > btnScore * 2) return topAccent;
    }
    return btnBg;
  }

  // Priority 2: explicit semantic CSS var (--primary, --cta, --brand with sat guard)
  if (isPlausibleHexColor(sc.primary)) return sc.primary;

  // Priority 3: vibrant accent
...
function buildDesignSystemPrompt(data, style) {
  ...
  if (namedVars.length > 0) namedVars.slice(0, 14).forEach(([k, v]) => lines.push('- `' + k + '`: `' + v + '`'));
  const _resolvedPrimary = resolvedPrimaryActionColor(data, style);
