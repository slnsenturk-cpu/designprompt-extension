...
async function buildImagePrompt(data) {
  ...
  try {
    const lines=[],site=safeHostname(data.url),imgFocus=state.imgFocus,palette=data.palette||[];
    ...
    // Vision AI description takes priority over rule-based direction
    if (data.visionIllustrationDescription) {
      lines.push('### Visual Analysis');
      lines.push(data.visionIllustrationDescription);
      lines.push('');
    } else {
      lines.push('### Design Direction');
    ...
    // Skip shadcn block and font imports — image mode has no reliable page-level data
    lines.push(`Recreate this visual style faithfully. Use the EXACT hex colors from the palette above.`);
    lines.push(`Source: ${site}`);
    const prompt=lines.join('\n'); state.lastPrompt=prompt;
    await savePrompt(data.url||state.currentUrl, prompt, 'image', 'generic', state.focus);
