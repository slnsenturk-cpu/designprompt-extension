...
function resolvedPrimaryActionColor(data, style) {
  ...
  const btnIsStrong = isPlausibleHexColor(btnBg) && hexSat(btnBg) > 24 && hexLum(btnBg) > 0.08 && hexLum(btnBg) < 0.92;
  if (btnIsStrong) return btnBg;
  if (isPlausibleHexColor(sc.primary)) return sc.primary;
  const accents = data?.accentColors || style?.accents || [];
  const vivid = accents.find(c => isPlausibleHexColor(c) && hexSat(c) > 40);
...
function generateComponentGuidance(data, style, specsData) {
  ...
  if (bs.primary) {
    ...
    // Widen hover match: any selector containing btn/button + a qualifier, or bare button/:hover selectors
    ...
    const hoverProps = primaryHover
      ? Object.entries(primaryHover).filter(([k,v]) => k !== 'selector' && typeof v !== 'object').map(([k,v]) => `\`${k}: ${v}\``).join(', ')
      : isDark ? 'brightness(1.08), 200ms ease-out' : 'brightness(0.92), 200ms ease-out';
  ...
  // ── Global interactive rules (unclaimed hover states + links + blend modes) ──
  {
    ...
    unclaimed.slice(0, 6).forEach(h => {
      ...
      if (/^[.#]/.test(sel)) {
        ...
        // Skip attribute selectors (library internals: [data-sonner-*], [data-radix-*], [data-state], etc.)
...
function canvasFallbackHint(contextStr, accentColor, bgColor) {
  ...
  if (/\b(orb|sphere)\b/.test(ctx) || /\b(glow|voice|wave|audio|sound)\b.*(visual|animation|processing|3d|effect)/i.test(ctx) || /animated.*(orb|sphere|radial|glow)/i.test(ctx)) {
    return `Use: Radial CSS glow — \`background: radial-gradient(ellipse at 50% 50%, ${accent}30 0%, transparent 60%);\` with \`@keyframes pulseGlow { 0%,100% { opacity:0.6; transform:scale(1); } 50% { opacity:1; transform:scale(1.08); } }\` at 3s ease-in-out infinite.`;
  }
  if (/\b(neural|mesh)\b/.test(ctx) || /\b(network|graph)\b.*(topology|infrastructure|agent|node)/i.test(ctx) || /agent.*(network|graph|node)/i.test(ctx) || /\bnode\b.*\bgraph\b/i.test(ctx)) {
    return `Use: SVG dot-network — \`${bg}\` panel, ~40 dots in \`${accent}\` at varied positions, thin lines between nearby dots, subtle opacity pulse on nodes (2s ease-in-out infinite).`;
  }
  if (/globe|world map|geographic|geograph/.test(ctx)) {
    return `Use: CSS sphere — \`border-radius: 50%\` \`background: radial-gradient(circle at 35% 35%, ${accent}80, ${bg})\` with a slow 20s linear rotate animation.`;
  }
  if (/code|terminal|editor|ide|syntax|console/.test(ctx)) {
    return `Use: Mock code editor panel — \`${bg}\` bg, monospace font, syntax lines: keywords \`${accent}\`, strings \`#a8ff78\`, comments \`#666\`.`;
  }
  if (/\b(chart|graph|metric|analytics|dashboard)\b/.test(ctx)) {
    return `Use: SVG chart — axes, data points/bars in \`${accent}\`, bg \`${bg}\`, clean sans-serif labels.`;
  }
  if (ctx.length > 15) {
    return `Use: Abstract animated gradient — \`background: linear-gradient(135deg, ${accent}20, ${bg})\` with slow 8s hue-rotate animation.`;
  }
  return '';
}
...
function buildPagePrompt(data, aiDirection) {
  ...
  const _p0TypeH = data.typographyPatterns?.h1?.fontFamily || data.assets?.fonts?.[0]?.family || null;
  const _p0TypeB = data.typographyPatterns?.body?.fontFamily || data.assets?.fonts?.find(f => f.family !== _p0TypeH)?.family || null;
  ...
  if(focus==='all'||focus==='typography') {
    ...
    if(fonts.length>0||fontVars.length>0) {
      ...
      if(fontVars.length>0) fontVars.slice(0,4).forEach(([k,v])=>lines.push(`- \`${k}\`: ${v}`));
        ...
        // Font family fallbacks: if computed fontFamily is null (CSS custom property stack), use assets.fonts
        const _assetFonts = data.assets?.fonts || [];
        const h1FontResolved = h1Font || _assetFonts[0]?.family || null;
        const bodyFontResolved = bodyFont || _assetFonts.find(f => f.family !== h1FontResolved)?.family || null;
        const labelFont = tp.label?.fontFamily || bodyFontResolved || null;
      ...
      if (focus === 'typography') {
        ...
        if (hasScale) {
          lines.push('');
          lines.push('### Type Scale');
          const _tsAssetFonts = data.assets?.fonts || [];
          const _tsH1Font = tp.h1?.fontFamily || _tsAssetFonts[0]?.family || null;
          const _tsBodyFont = tp.body?.fontFamily || _tsAssetFonts.find(f => f.family !== _tsH1Font)?.family || null;
        ...
        if (i === 0) {
          ...
          if (data.illustrationStyle) {
            ...
            if (ill.type === 'monochrome-line-art') {
              ...
              const _hint2 = canvasFallbackHint('canvas animated visualization', _acc2, data.pageBackground);
          ...
          if (visuals.length > 0) {
            ...
            visuals.forEach(v => {
              if (/\[canvas-animation\]|\[lottie-animation\]/.test(v)) {
                ...
                const _cleaned = v.replace(/\.\s*Recreate as[^.]+\.?$/i, '').trim();
                lines.push(`    - ${_cleaned}`);
                const _hint = canvasFallbackHint(_ctxStr, _accentHex, _bgHex);
                if (_hint) lines.push(`      → ${_hint}`);
              } else if (/\[video-background\]|\[styled-visual\]/.test(v) && v.includes(' — ')) {
                const _ctxM2 = v.match(/ — ([^.]+)/);
                const _ctxStr2 = _ctxM2 ? _ctxM2[1] : '';
                const _hint2 = canvasFallbackHint(_ctxStr2, _accentHex, _bgHex);
                lines.push(`    - ${v}`);
                if (_hint2) lines.push(`      → CSS fallback: ${_hint2}`);
              } else {
                lines.push(`    - ${v}`);
              }
            });
