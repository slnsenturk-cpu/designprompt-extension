  ...
  if(focus==='all'||focus==='motion') {
    ...
    if(motionVars.length>0||transitions.length>0||animations.length>0||hoverStates.length>0) {
      if (focus === 'motion') {
        ...
        const _hasPageTransitions = (data.animations || []).some(a => /page-out|page-in|blur-fade/i.test(a.name));
        if (_hasPageTransitions) {
          ...
          lines.push('  `blur-fade-left/right`: content panel transitions (tab or section swap). If single-page with no routing, skip page transitions — blur-fade applies to in-page content toggles only.');
        }
        lines.push('');
      }
    }
      ...
      scm.forEach((sec, i) => {
        ...
        if (sec.decorativeGradients && sec.decorativeGradients.length > 0) {
          sec.decorativeGradients.forEach(d => {
            ...
            // Detect color-tint overlays (e.g. blue wash rgba(31,88,242,0.2))
            const _isColorTint = /rgba\(\s*\d+\s*,\s*\d+\s*,\s*\d+\s*,\s*0\.[01]\d*\s*\)/.test(d.gradient);
            if (_isColorTint) {
              dsc += `. Apply as overlay: \`position:absolute; inset:0; background: ${d.gradient}; pointer-events:none; z-index:0\`. Section content: \`position:relative; z-index:1\`.`;
            }
            lines.push(dsc);
          });
        }
