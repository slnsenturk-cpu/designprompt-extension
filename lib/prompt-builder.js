...
function generateComponentGuidance(data, style, specsData) {
  ...
  if (data.stickySections?.length > 0) {
    data.stickySections.forEach(sec => {
      const stickyContent = sec.stickyColHasCanvas ? '(contains canvas animation)' : sec.stickyColHasSvg ? '(contains SVG)' : '';
...
function buildPagePrompt(data, aiDirection) {
  ...
  if(focus==='all') {
    ...
    if (scm.length > 0) {
      ...
      // Use blur-fade keyframe name as evidence when revealStyle missed it
      ...
      scm.forEach((sec, i) => {
        ...
        // Stats/metrics section — add metric format spec
