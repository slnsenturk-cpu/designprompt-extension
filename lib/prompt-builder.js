...
function generateRuleBasedDirection(data, style) {
  ...
  if (_scmDp.length >= 4) {
    ...
    if (_transitions >= 2) {
      ...
      lines.push(
        `**Section color rhythm — alternating dark/light:** ${_rhythm.join(' → ')}. ` +
        `Hard-cut transitions — NO gradients between sections. Each background change is an instant flip. ` +
        `Dark sections (\`${_darkBg}\`): dense, focused reading. ` +
        `Light sections (\`${_lightBg}\`): open, scanning. ` +
        `This alternation IS the page's dramatic structure — reproduce the rhythm exactly, do not normalize to one background.`
      );
      lines.push('');
    }
  }
...
function generateComponentGuidance(data, style, specsData) {
  ...
  if (data.stickySections?.length > 0) {
    data.stickySections.forEach(sec => {
      ...
      if (sec.type === 'sticky-tab-scroll' && sec.tabLabels) {
        lines.push(`**Sticky-scroll tab section:** Left column \`position:sticky; top:0; height:100vh\` ${stickyContent}. Contains tab navigation with ${sec.tabLabels.length} items: ${sec.tabLabels.map(t => `"${t}"`).join(' · ')}. Right column scrolls normally — each tab corresponds to a different content panel on the right.`);
        lines.push(`  Tab indicator: active tab has visual highlight (accent dot, border-left, or background shift). Clicking/scrolling updates the right-side content.`);
      } else {
  ...
  if (data.svgDiagramAnimations?.length > 0) {
    data.svgDiagramAnimations.forEach(diag => {
      if (diag.type === 'svg-path-animation') {
        lines.push(`**SVG path animation** (${diag.width}×${diag.height}px): ${diag.pathCount} paths, ${diag.circleCount} circles, ${diag.animationCount} SMIL animations. Colors: ${diag.colors.join(', ')}. ${diag.labels.length ? `Labels: ${diag.labels.map(l => `"${l}"`).join(', ')}.` : ''} Implement as inline SVG with \`<animateMotion>\` for dots traveling along paths.`);
      } else if (diag.type === 'pill-track-diagram') {
        const pillList = diag.pills.map(p => `"${p.text}" (${p.w}×${p.h}px)`).join(', ');
        lines.push(`**Pill-track diagram**: Large oval pill shapes — ${pillList}. Implement as SVG or styled divs with \`border-radius: 9999px\` and connecting path lines between them.`);
  ...
  // ── Section background decorations (absolute SVG) ──
    ...
    data.sectionBackgroundDecorations.forEach(sec => {
      sec.decorations.forEach(d => {
        if (d.type === 'radial-rays') {
          lines.push(`**Section background decoration** (on ${sec.sectionBg || 'page bg'}): Radial ray pattern — ${d.pathCount} lines emanating from center outward, starburst/compass pattern. Colors: ${d.colors.join(', ')}. opacity: ${d.opacity}. \`position:absolute; inset:0; z-index:0; pointer-events:none\`. Implement as inline SVG with lines from center. Should be barely visible.`);
        } else if (d.type === 'grid-lines') {
          lines.push(`**Section background decoration**: Grid line pattern — ${d.lineCount} lines. Colors: ${d.colors.join(', ')}. \`position:absolute; z-index:0; pointer-events:none\`.`);
        } else {
          lines.push(`**Section background decoration**: Organic curves — ${d.pathCount} curved paths. Colors: ${d.colors.join(', ')}. \`position:absolute; z-index:0; pointer-events:none\`. Recreate as SVG with bezier paths.`);
        }
      });
    });
  }
...
function buildPagePrompt(data, aiDirection) {
  ...
  if (_fw?.isFramer) {
    ...
    lines.push('> **Animation note (Framer):** Many visual animations on this site are driven by Framer\'s internal engine and are not extractable as CSS @keyframes. The motion tokens above represent only CSS-level transitions. For Framer-rendered animations (path reveals, scroll-scrub, component transitions), implement equivalents using Framer Motion, GSAP, or CSS @keyframes with IntersectionObserver.');
    lines.push('');
