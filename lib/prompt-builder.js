...
function buildPagePrompt(data, aiDirection) {
  ...
  lines.push(getPlatformInstruction(site, data));
...
function getPlatformInstruction(site, data) {
  ...
  let shadcnBlock = '';
  {
    ...
    const _btnBgValid = _btnBg && /^#[0-9a-f]{6}$/i.test(_btnBg.trim()) && _btnBg.toLowerCase() !== '#ffffff';
    // Only accept solid 6-char hex for shadcn --primary (no semi-transparent 8-char)
    const _isOpaque6 = c => c && /^#[0-9a-f]{6}$/i.test(c.trim());
    const primaryHex = (_btnBgValid ? _btnBg : null)
      || (_isOpaque6(sc.primary) ? sc.primary : null)
      || (_isOpaque6(data?.accentColors?.[0]) ? data.accentColors[0] : null)
      || '#000000';
