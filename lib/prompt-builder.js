...
function dedupeHexStrings(colors) { const s=new Set(); return colors.filter(c=>{if(!c||c.length<4)return false;const n=c.toLowerCase();if(s.has(n))return false;s.add(n);return true;}); }
...
function analyzeDesignStyle(data) {
  ...
  const sorted = dedupeHexStrings(colors).sort((a,b)=>hexLum(b)-hexLum(a));
  ...
  // Last-resort: infer dark/light from body text color — light body text implies
  // a dark site, dark body text implies a light site. Prevents dark→light misclassification
  // when pageBackground scan failed (e.g. Framer sites, gradient-only bodies).
  const bodyTextColor = data.typographyPatterns?.body?.color;
  const bodyTextLum = bodyTextColor ? hexLum(bodyTextColor) : null;
  let pageBgLum;
  if (pageBg) {
    pageBgLum = hexLum(pageBg);
  } else if (bodyTextLum !== null) {
    pageBgLum = bodyTextLum > 0.65 ? 0.1 : 0.85;
  } else {
    pageBgLum = 0.85; // genuinely unknown → most sites are light
  }
  // Cross-check with body text in the mid-gray [0.35, 0.5) zone — a single luminance
  // threshold misclassifies sites like #2a2a2a+white-text (dark) vs #d8d8d8+black-text (light).
  // Light body text in mid-gray → dark site; dark body text in mid-gray → light site.
  const isDark = pageBgLum < 0.35 || (pageBgLum < 0.5 && bodyTextLum !== null && bodyTextLum > 0.6);
  const isLight = pageBgLum > 0.5 || (pageBgLum >= 0.35 && bodyTextLum !== null && bodyTextLum < 0.4);
  ...
  const semanticColors = inferSemanticRoles(data, extractSemanticColors(vars));
...
// extractSemanticColors, _hexHSL, inferSemanticRoles → moved to lib/color-utils.js
...
function scoreConfidence(data) {
  ...
  return {
    ...
    // Shadows: CSS vars with shadow key OR 2+ distinct computed shadows = reliable
    shadows: (Object.keys(vars).some(k => /shadow|elevation/.test(k)) || (data.shadows?.length || 0) >= 2)
      ? 'high'
      : (data.shadows?.length || 0) > 0 ? 'medium' : 'low',
  };
}
...
function computeCoverageScore(data) {
  const vars = data.cssVars || {};
  const cssVarCount = Object.keys(vars).filter(k => /^#[0-9a-f]{3,8}$/i.test((vars[k]||'').trim())).length;
  const tp = data.typographyPatterns || {};
  const colors = (data.colors || []).length + (data.accentColors || []).length;
  const fonts = (data.fonts || []).filter(f => f).length;
  const shadows = (data.shadows || []).length;
  const animations = (data.animations || []).length + (data.transitions || []).length;
  const components = (data.buttonStyles?.primary ? 1 : 0) + (data.cardStyles ? 1 : 0)
    + (data.inputStyles && Object.keys(data.inputStyles).length ? 1 : 0)
    + (data.badgeStyles?.borderRadius ? 1 : 0);
  const sections = (data.sectionContentMap || []).length;

  // Weighted score: CSS vars 30%, color roles 20%, typography 20%, components 15%, motion 15%
  const varScore = Math.min(cssVarCount / 10, 1) * 30;
  const colorScore = Math.min(colors / 6, 1) * 20;
  const typoScore = ((tp.h1 ? 0.4 : 0) + (tp.body ? 0.4 : 0) + (fonts >= 2 ? 0.2 : 0)) * 20;
  const compScore = Math.min(components / 3, 1) * 15;
  const motionScore = Math.min(animations / 4, 1) * 15;
  const pct = Math.round(varScore + colorScore + typoScore + compScore + motionScore);

  return {
    pct,
    colors, fonts, shadows, components, animations,
    sections,
    label: pct >= 80 ? 'Excellent' : pct >= 60 ? 'Good' : pct >= 40 ? 'Partial' : 'Limited',
  };
}
...
// Highest-saturation accent weighted by usage frequency (bg+text counts).
// Only accents with hexSat > 25 qualify — filters neutrals and near-neutrals.
// Score = sat × (1 + log10(freq + 1)), same shape as A3's accent ranking.
function _frequencyWeightedAccent(data) {
  const accents = data?.accentColors || [];
  const usage = data?.colorUsage || {};
  const scored = accents
    .filter(c => isPlausibleHexColor(c) && hexSat(c) > 25)
    .map(c => {
      const u = usage[c.toLowerCase()] || {};
      const freq = (u.bg || 0) + (u.text || 0);
      return { hex: c.toLowerCase(), score: hexSat(c) * (1 + Math.log10(freq + 1)) };
    })
    .sort((a, b) => b.score - a.score);
  return scored[0]?.hex || null;
}

// Triple-signal primary resolver. Returns { color, confidence, ambiguous, candidates }.
// Signals: (1) button extractor, (2) CSS var names, (3) frequency-weighted accent.
// - 2-of-3 agreement → confidence 'high'
// - all three disagree → confidence 'low', priority order CSS var > button > frequency,
//   sets data._primaryColorAmbiguous so buildPagePrompt can emit a verify-before-ship warning
// - only one signal present → confidence 'medium'
// - no signals → fallback chain, confidence 'low', not flagged ambiguous
// Also sets data._primaryColorCandidates so the warning block has the three raw values.
function resolvedPrimaryActionColor(data, style) {
  const btnRaw = data?.buttonStyles?.primary?.backgroundColor;
  const button_primary = (isPlausibleHexColor(btnRaw) && btnRaw.toLowerCase() !== '#ffffff')
    ? btnRaw.toLowerCase()
    : null;

  // Recompute semantic colors from CSS vars — independent of style.semanticColors which
  // may have been enriched by inferSemanticRoles using the very signals we're cross-checking.
  const sc = (typeof extractSemanticColors === 'function')
    ? extractSemanticColors(data?.cssVars || {})
    : {};
  const css_var_primary = isPlausibleHexColor(sc.primary) ? sc.primary.toLowerCase() : null;

  const frequency_primary = _frequencyWeightedAccent(data);

  const signals = [button_primary, css_var_primary, frequency_primary].filter(Boolean);

  // No signals — fallback to whatever the old chain would have picked
  if (signals.length === 0) {
    const accents = data?.accentColors || style?.accents || [];
    const fallback = (isPlausibleHexColor(style?.semanticColors?.primary) && style.semanticColors.primary)
      || accents.find(c => isPlausibleHexColor(c) && hexSat(c) > 15)
      || accents[0]
      || null;
    return { color: fallback, confidence: 'low', ambiguous: false, candidates: { button_primary, css_var_primary, frequency_primary } };
  }

  // Single signal — use it, medium confidence
  if (signals.length === 1) {
    return { color: signals[0], confidence: 'medium', ambiguous: false, candidates: { button_primary, css_var_primary, frequency_primary } };
  }

  // Two or three signals — count agreements
  const counts = new Map();
  for (const s of signals) counts.set(s, (counts.get(s) || 0) + 1);
  const [[topValue, topCount]] = [...counts.entries()].sort((a, b) => b[1] - a[1]);

  if (topCount >= 2) {
    return { color: topValue, confidence: 'high', ambiguous: false, candidates: { button_primary, css_var_primary, frequency_primary } };
  }

  // All three disagree (three distinct values) — pick by declared priority and flag ambiguous
  const winner = css_var_primary || button_primary || frequency_primary;
  data._primaryColorAmbiguous = true;
  data._primaryColorCandidates = { button_primary, css_var_primary, frequency_primary };
  return { color: winner, confidence: 'low', ambiguous: true, candidates: { button_primary, css_var_primary, frequency_primary } };
}
...
function cleanPromptLines(lines) {
  return lines
    ...
    // Collapse adjacent duplicate lines — multiple extraction passes (e.g. Framer
    // Pass 2 + Pass 3 section map, or inline hover + grouped hover) can emit the
    // same description back-to-back. Element-reference dedup upstream doesn't
    // catch identical rendered strings.
    .filter((l, i, arr) => i === 0 || l !== arr[i - 1]);
}

// Per-section budget cap — prevents a 12-section site's Section Content Map
// from consuming the entire mustKeep quota. Each section keeps its first N lines
// (headers/layout/CTAs come first by convention), trimming verbose decorative detail.
// ─── Shared hover-state grouping ────────────────────────────────────────────
// Lifted out of the focus='motion' branch so focus='all' can call it too,
// replacing the flat per-component hover renderings scattered across earlier sections.
function _motionResolveVar(vars, v) {
...
function _motionTimingForProp(transitions, prop) {
...
function renderGroupedHoverStates(hoverStates, data, vars, transitions) {
  const COMP_MOTION = [
  ...
  const primaryIsDark = /^#[0-9a-f]{6}$/i.test(primaryBg) && parseInt(primaryBg.slice(1, 3), 16) < 120;
  const seenComp = new Set();
  const compInteractions = [];
  for (const h of hoverStates) {
    if (SKIP_M.some(p => p.test(h.selector))) continue;
    const match = COMP_MOTION.find(p => p.regex.test(h.selector));
    if (!match) continue;
  ...
  return compInteractions.map(({ label, h }) => {
    const parts = Object.entries(h).filter(([k]) => k !== 'selector').map(([k, v]) => {
      ...
      const resolved = _motionResolveVar(vars, v);
      if (/^\[.+\]$/.test(String(resolved))) return null;
      const timing = _motionTimingForProp(transitions, k);
...
// Maps raw shadow strings to component roles using already-extracted component
// styles. Replaces anonymous shadow-sm/md/lg/xl labels with semantic ones
// (card-resting, button-primary, card-hover) so the LLM knows which shadow
// belongs to which surface instead of guessing by elevation order.
function inferShadowRoles(data) {
  const roles = new Map();
  const _norm = s => (s || '').replace(/\s+/g, ' ').trim();
  const _add = (role, shadow) => {
    const n = _norm(shadow);
    if (!n || n === 'none') return;
    if (!roles.has(n)) roles.set(n, role);
  };
  _add('button-primary', data.buttonStyles?.primary?.boxShadow);
  _add('button-secondary', data.buttonStyles?.secondary?.boxShadow);
  _add('button-ghost', data.buttonStyles?.ghost?.boxShadow);
  _add('card-resting', data.cardStyles?.boxShadow);
  _add('badge', data.badgeStyles?.boxShadow);
  _add('input', data.inputStyles?.boxShadow);
  const cardHover = (data.hoverStates || []).find(h =>
    /card|item|tile/i.test(h.selector) && (h['box-shadow'] || h.boxShadow)
  );
  if (cardHover) _add('card-hover', cardHover['box-shadow'] || cardHover.boxShadow);
  const btnHover = (data.hoverStates || []).find(h =>
    /btn|button|cta/i.test(h.selector) && (h['box-shadow'] || h.boxShadow)
  );
  if (btnHover) _add('button-hover', btnHover['box-shadow'] || btnHover.boxShadow);
  return roles;
}

function trimSectionContentMap(lines) {
  const HEADER_RE = /^### Section Content Map$/;
  const NEXT_HEADER_RE = /^### /;
  const ENTRY_RE = /^\*\*Section [\d.]+: ([a-z0-9-]+)\*\*/i;

  // Higher budget for sections that carry more structural weight; low budget
  // for connective tissue (logo strips, ticker bars).
  const BUDGETS = {
    hero: 40,
    'cta-section': 25, cta: 25,
    feature: 20, features: 20,
    testimonial: 20, testimonials: 20,
    pricing: 20,
    footer: 15, 'footer-contact': 15, contact: 15,
    'logo-strip': 8, 'ticker-strip': 8,
  };
  const DEFAULT_BUDGET = 22;
  ...
  const flushBuffer = () => {
    if (!buffer) return;
    out.push(...buffer.slice(0, bufferBudget));
    buffer = null;
    bufferBudget = 0;
  };
  ...
  for (const line of lines) {
    ...
    if (inMap && NEXT_HEADER_RE.test(line)) {
      flushBuffer();
      inMap = false;
      out.push(line);
      continue;
    ...
    if (entryMatch) {
      flushBuffer();
      bufferBudget = BUDGETS[entryMatch[1].toLowerCase()] ?? DEFAULT_BUDGET;
      buffer = [line];
      continue;
    }
    // Post-entry structural content (SCROLL REVEAL block, FAILSAFE rule, Contact/Footer
    // sections) — these are non-indented and not section-entry headers, so they must
    // not be gobbled into the last entry's budget. Flush and passthrough.
    const isEntryBody = line.startsWith(' ') || line.startsWith('\t') || line.startsWith('- ');
    if (buffer && !isEntryBody) {
      flushBuffer();
      out.push(line);
      continue;
    }
...
function generateRuleBasedDirection(data, style) {
  const _vdModel = _vdModelFor(data);
  ...
  const primaryColor = resolvedPrimaryActionColor(data, style).color
  ...
  // §2: the shape narrative is chosen from booleans over an unordered radius
  // list, which on rig.ai reports "moderate rounding (6px)" for buttons that
  // are chamfered with a clip-path and have NO radius at all. The model knows
  // the actual shape language, so it decides first and the heuristics only
  // run when it has nothing to say.
  if (_vdModel && _vdModel.shape && _vdModel.shape.clipPath) {
    const chamfer = _vdModel.shape.size || 'a fixed offset';
    const container = (_vdModel.radius && _vdModel.radius.card) || '0px';
    lines.push('Angular geometry — interactive elements are chamfered by '+chamfer
      + ' with a `clip-path` polygon, NOT a border-radius. Containers use '+container
      + ' radius. Do not substitute rounded corners; the cut corner is the motif.'
      + (hasLayeredShadows ? ' Layered shadows — don\'t simplify to a single layer.' : ''));
  } else if (hasFullRound&&hasGlowEffect) {
  ...
  if (scm && scm.length > 0) {
    lines.push(conf.sectionMap === 'medium'
      ...
      : '**Page flow**');
    const _flowBase = data.spacingScale?.baseUnit || 0;
    scm.slice(0, 8).forEach((sec, i) => {
      let desc = `${i+1}. **${sec.type}**`;
      if (sec.bgColor) desc += ` (${sec.bgColor})`;
      if (sec.heading) desc += `: "${sec.heading}"`;
      // Spacing-system annotation — show padding as multiple of the base unit so the
      // LLM produces systematic rhythm (all sections on the same grid) instead of
      // arbitrary 96/100/104px values. Only when padding conforms exactly to base.
      if (sec.paddingY && _flowBase > 0) {
        const _py = parseInt(sec.paddingY);
        if (_py > 0) {
          if (_py % _flowBase === 0) desc += ` Padding: ${_py}px (${_py/_flowBase}×${_flowBase}).`;
          else desc += ` Padding: ${_py}px.`;
        }
      } else if (sec.paddingY) {
        desc += ` Padding: ${sec.paddingY}.`;
      }
      // Layout narrative
      if (sec.layout) {
  ...
  if (bs?.primary || badge || inp) {
    ...
    if (bs?.primary) {
      ...
      // "rounded" as a fallback is a claim, not a measurement — and a wrong one
      // wherever the buttons are chamfered. Ask the model before guessing.
      const _vdShape = _vdModel && _vdModel.shape && _vdModel.shape.clipPath
        ? `chamfered ${_vdModel.shape.size || ''}`.trim()
        : null;
      let btnDesc = `Buttons: Primary = ${isPill ? 'pill-shaped' : (_vdShape || bp.borderRadius || 'rounded')}`;
      ...
      if (bs.secondary) {
        const sec = bs.secondary;
        lines.push(`Secondary: \`${sec.backgroundColor||'muted'}\` bg, \`${sec.color||'inherit'}\` text${sec.border ? ', border `'+sec.border+'`' : ''}.`);
      }
      if (bs.textOnly) {
        const t = bs.textOnly;
        lines.push(
...
// COMPONENT GUIDANCE
// ═══════════════════════════════════════════════════════════════════════════
// §3: a button whose own text fails 3:1 against its own recorded background is
// not a design instruction — it is a measurement taken where the two were not
// adjacent. posthog's secondary button reads `#000000` bg with `#23251d` text
// (1.35:1); printing that pair unannotated tells the reader to build something
// illegible. Naming the surface it was measured on turns a wrong instruction
// into a true observation.
function _vdContrastNote(bgValue, textValue, model) {
  try {
    if (!model || !model._resolve) return '';
    if (typeof wcagContrast !== 'function') return '';
    const bg = model._resolve(bgValue);
    const fg = model._resolve(textValue);
    if (!bg || !fg) return '';
    const ratio = wcagContrast(fg, bg);
    if (!Number.isFinite(ratio) || ratio >= 3) return '';
    const surface = (model.heroSurface && model.heroSurface.hex)
      || (model.colors && model.colors.background) || null;
    return surface
      ? ` (measured on \`${surface}\`, where the two are not adjacent)`
      : ' (measured where the two are not adjacent)';
  } catch (e) { return ''; }
}
...
function generateComponentGuidance(data, style, specsData) {
  ...
  const sc = semanticColors || {};
  const accent = resolvedPrimaryActionColor(data, style).color
    || sc.primary
  ...
  // §2: radiusSample is "the first radius seen anywhere on the page", which is
  // not the same thing as "this component's radius". On rig.ai it is 6px —
  // belonging to neither its cards (10px) nor its inputs (1px) nor its buttons
  // (0px, they are chamfered). The model derives a radius PER ROLE, so ask it
  // by role and keep the page-wide sample only as a last resort.
  const _vdRole = (role) => {
    const r = _vdModel && _vdModel.radius ? _vdModel.radius[role] : null;
  ...
  if (bs.primary) {
    ...
    const pRadius = p.clipPath ? null : (p.borderRadius || _vdRole('button'));
    // Shape description
    let shape;
    if (p.clipPath) {
      ...
      shape = `\`${p.borderRadius||_vdRole('button')}\` radius`;
    }
    ...
    let btn = `${shape}, \`${pBg}\` bg, text \`${_pColor}\`` + _vdContrastNote(pBg, _pColor, _vdModel);
  ...
  if (bs.ghost) {
    ...
    const ghostShape = g.clipPath ? `chamfered via clip-path` : `\`${g.borderRadius||_vdRole('button')}\` radius`;
    ...
    if (g.borderRadius||_vdRole('button')) ghostCss.push(`  border-radius: ${g.borderRadius||_vdRole('button')};`);
  ...
  // Secondary button — extracted but often overlooked in output
  if (bs.secondary && bs.secondary !== bs.primary && bs.secondary !== bs.ghost) {
    const s = bs.secondary;
    const secShape = s.clipPath ? 'chamfered via clip-path' : `\`${s.borderRadius||_vdRole('button')}\` radius`;
    const secHover = hoverStates.find(h =>
      /secondary|subtle|muted/i.test(h.selector) && /btn|button/i.test(h.selector)
    );
    if (secHover) claimedSelectors.add(secHover.selector);
    let secDesc = `${secShape}, \`${s.backgroundColor||'muted'}\` bg, text \`${s.color||'inherit'}\``
      + _vdContrastNote(s.backgroundColor, s.color, _vdModel)
      + `, padding \`${s.padding}\``;
    if (s.border) secDesc += `, border \`${s.border}\``;
    if (s.fontSize) secDesc += `, font \`${s.fontSize}/${s.fontWeight||'500'}\``;
    if (secHover) {
      const secHoverProps = Object.entries(secHover).filter(([k,v]) => k !== 'selector' && typeof v !== 'object').map(([k,v]) => `\`${k}: ${v}\``).join(', ');
      secDesc += `. Hover: ${secHoverProps}`;
    }
    lines.push('**Secondary button:** ' + secDesc);
    const secCss = [];
    if (s.backgroundColor) secCss.push(`  background-color: ${s.backgroundColor};`);
    if (s.color) secCss.push(`  color: ${s.color};`);
    if (s.padding) secCss.push(`  padding: ${s.padding};`);
    if (s.borderRadius) secCss.push(`  border-radius: ${s.borderRadius};`);
    if (s.border) secCss.push(`  border: ${s.border};`);
    if (s.fontSize) secCss.push(`  font-size: ${s.fontSize};`);
    if (s.fontWeight) secCss.push(`  font-weight: ${s.fontWeight};`);
    if (secCss.length) lines.push('  Spec: ' + secCss.map(l => '`' + l.trim().replace(/;$/, '') + '`').join(' · '));
  }
  ...
  if (vpr.hasGlassmorphism || vpr.hasFloatingCards || hasLayeredShadows) {
    let card = vpr.hasGlassmorphism
      ? 'backdrop-filter:blur(16px), semi-transparent bg, '+_vdRole('card')+' radius. Padding 24–32px.'
      : isDark ? 'Dark surface, rgba(255,255,255,0.06) border. '+(hasLayeredShadows?'Layered shadow from tokens.':'Border for definition.')+' '+_vdRole('card')+' radius. Padding 24–32px.'
      : '`'+(pageBg||'#ffffff')+'` bg, '+(hasLayeredShadows?'layered shadow from tokens':'subtle border')+'. '+_vdRole('card')+' radius. Padding 24–32px.';
  ...
  if (inputData && Object.keys(inputData).length > 0) {
    const inputHover = hoverStates.find(h =>
      /input:hover|input:focus|\.input:hover|\.input:focus|textarea:focus/i.test(h.selector)
    );
    if (inputHover) claimedSelectors.add(inputHover.selector);
    const inputFocusDesc = inputHover
      ? Object.entries(inputHover).filter(([k,v]) => k !== 'selector' && typeof v !== 'object').map(([k,v]) => `\`${k}: ${v}\``).join(', ')
      : `outline 2px solid ${accent||'currentColor'} offset 2px`;
    lines.push(`**Inputs:** \`${inputData.backgroundColor||'inherit'}\` bg, \`${inputData.border||'1px solid '+(isDark?'rgba(255,255,255,0.12)':'#e0e0e0')}\`, \`${inputData.borderRadius||_vdRole('input')}\` radius. Focus: ${inputFocusDesc}.`);
  }
  ...
  // ── Global interactive rules (unclaimed hover states + links + blend modes) ──
  {
    ...
    if (specsData.links) {
      ...
      if (isPlausibleHexColor(linkColor) && isPlausibleHexColor(style.pageBg) && Math.abs(hexLum(linkColor) - hexLum(style.pageBg)) < 0.14) {
        linkColor = resolvedPrimaryActionColor(data, style).color || '#002eff';
      }
...
async function buildPromptFromData(data, source) {
  ...
  if (useAI && source === 'page') {
    aiDirection = await generateDirectionWithAI(
      ...
      // §4.5: the status line shows the STAGE, never the content. This used to
      // paste the model's streaming markdown into the panel, which turned the
      // progress area into a scrolling wall of half-written prose.
      // The partial text is still received — it is simply not displayed.
      () => showStage(6)
    );
  ...
  // v2.0.0-beta.2 — dual-write to Supabase for authenticated users.
  // Fire-and-forget so the UI doesn't wait on cloud latency; any failure
  // is logged by the cloud module but does not affect the local result.
  try {
    // Only http(s) URLs are syncable. Skip file://, about:, chrome://,
    // data:, etc. — these have no meaningful hostname and aren't
    // cross-device shareable. Extension's local flow still captures them.
    const isHttp = typeof url === 'string'
      && (url.toLowerCase().indexOf('http://') === 0 || url.toLowerCase().indexOf('https://') === 0);
    if (isHttp
        && self.VD_AUTH && typeof self.VD_AUTH.isAuthenticated === 'function'
        && self.VD_CLOUD && typeof self.VD_CLOUD.syncAnalysis === 'function') {
      self.VD_AUTH.isAuthenticated().then((authed) => {
        if (!authed) return;
        const hostname = safeHostname(url);
        const primary_color =
          (data && data.primary_color) ||
          (data && data.colors && (data.colors.primary || data.colors.accent)) ||
          (data && data.tokens && data.tokens.primary_color) ||
          null;
        const page_background =
          (data && data.page_background) ||
          (data && data.colors && data.colors.background) ||
          null;
        const page_title =
          (data && data.page_title) ||
          (typeof document !== 'undefined' ? document.title : null);
        let extraction_version = null;
        try {
          if (typeof chrome !== 'undefined' && chrome.runtime && typeof chrome.runtime.getManifest === 'function') {
            const m = chrome.runtime.getManifest();
            extraction_version = m.version_name || m.version || null;
          }
        } catch (_) { /* noop */ }
        const tokens_json = data || {};
        const ai_provider = (state.provider && state.provider !== 'none') ? state.provider : null;
        const was_ai_enhanced = !!aiDirection;
        const target_platform = (state.platform && typeof state.platform === 'string') ? state.platform : 'generic';
        const focusVal = state.focus || 'all';
        self.VD_CLOUD.syncAnalysis({
          url, hostname, primary_color, page_background, page_title, extraction_version, tokens_json
        })
          .then((ar) => {
            if (!ar || !ar.ok || !ar.id) return;
            const analysisId = ar.id;
            return self.VD_CLOUD.syncPrompt({
              prompt_text: prompt,
              target_platform: target_platform,
              focus: focusVal,
              ai_provider: ai_provider,
              was_ai_enhanced: was_ai_enhanced
            }, analysisId).then((pr) => {
              // Roll back the analysis if the prompt insert failed so the
              // dashboard doesn't show an orphan row with no prompt.
              if (!pr || !pr.ok) {
                console.warn('[vd-cloud] prompt sync failed, rolling back analysis', analysisId, pr && pr.error);
                if (typeof self.VD_CLOUD.deleteAnalysis === 'function') {
                  return self.VD_CLOUD.deleteAnalysis(analysisId).catch(
                    (e) => console.warn('[vd-cloud] orphan rollback failed', analysisId, e)
                  );
                }
              }
            });
          })
          .catch((e) => console.warn('[vd-cloud] chained sync failed', e));
      }).catch((e) => console.warn('[vd-cloud] auth check failed', e));
    } else if (!isHttp && url) {
      console.log('[vd-cloud] skipping non-http entry (live):', url);
    }
  } catch (e) { console.warn('[vd-cloud] dual-write setup failed', e); }

  const coverage = source === 'page' ? computeCoverageScore(data) : null;
  showResult(prompt, { url }, source, aiDirection ? state.provider : null, false, coverage);
  flashSaveIndicator();
}
...
// ── the design model ────────────────────────────────────────────────────
// Every VALUE the prompt prints is resolved through lib/design-model.js — the
// same model DESIGN.md renders from — so the two documents cannot disagree
// about a colour, a radius or a shadow.
//
// The prompt's STRUCTURE is untouched: same 17 sections, same Focus gates,
// same prose. This is a resolution layer, not a rewrite.
function _vdModelFor(data) {
  try {
    const M = (typeof self !== 'undefined' && self.VD_MODEL) ? self.VD_MODEL : null;
    if (!M) return null;
    return M.buildDesignModel(data, { sourceUrl: (data && data.url) || '' });
  } catch (e) {
    console.warn('[vd-prompt] could not build the design model', e);
    return null;
  }
}

// Colour expressions the consumer's project cannot resolve. A `var(--blue)`
// means nothing outside the page it came from, and an `oklch()` with alpha is
// a value most people cannot read at a glance — the point of the prompt is to
// hand over something usable.
const _VD_UNRESOLVED = /var\(\s*--[a-z0-9-_]+\s*(?:,[^()]*)?\)|(?:oklch|lab|lch|color)\((?:[^()]|\([^()]*\))*\)/gi;

// A colour function WRAPPING a custom property — `rgb(var(--bg))`. Resolving
// the inner var and leaving the wrapper behind produces `rgb()`, which is
// worse than what it replaced: the original at least said where the value came
// from. So the whole expression is resolved or the whole expression goes.
const _VD_WRAPPED = /\b(?:rgba?|hsla?|oklch|lab|lch|color)\(\s*var\(\s*--[a-z0-9-_]+\s*(?:,[^()]*)?\)[^()]*\)/gi;

// `prop: value` inside a code span. When the value resolves to nothing there is
// no half of this worth printing, so the span goes with it.
const _VD_DEAD_SPAN = /`\s*[a-z-]+\s*:\s*`(?:,\s*)?/gi;

// A page-local SVG filter reference. It cannot survive the copy — there is no
// #noise in the reader's document — so it is described rather than quoted.
const _VD_URL_REF = /`?filter:\s*url\(#[a-z0-9-_]+\)`?|`url\(#[a-z0-9-_]+\)`|url\(#[a-z0-9-_]+\)/gi;

// Tailwind writes a dozen --tw-* custom properties into every transition. They
// are implementation detail of the framework that produced the page, not of
// the design, and they crowd out the properties that matter.
const _VD_TW_NOISE = /(^|[,\s])--tw-[a-z0-9-]+\s*(?:,\s*)?/gi;

function _vdResolveLine(line, model) {
  if (typeof line !== 'string' || !line) return line;
  let out = line;
  let dropped = false;

  if (model && model._resolve) {
    // Wrapped first: rgb(var(--x)) must be judged whole, not gutted.
    out = out.replace(_VD_WRAPPED, (match) => {
      const hex = model._resolve(match);
      if (hex) return hex;
      dropped = true;
      return '';
    });
    out = out.replace(_VD_UNRESOLVED, (match) => {
      const hex = model._resolve(match);
      if (hex) return hex;
      // An unresolvable reference is dropped rather than printed: a reader
      // cannot act on `var(--radix-accordion-content-height)`, and leaving it
      // in suggests it is a value they should copy.
      dropped = true;
      return '';
    });
  }

  if (_VD_URL_REF.test(out)) {
    _VD_URL_REF.lastIndex = 0;
    out = out.replace(_VD_URL_REF, 'a repeating noise texture');
  }

  if (/--tw-/.test(out)) {
    out = out.replace(_VD_TW_NOISE, '$1');
    dropped = true;
  }

  // Tidying runs ONLY where something was actually removed. Applied to every
  // line it does more harm than good: it turned "animation-timeline: scroll()"
  // into "scroll" and put an em-dash after every colon that preceded a code
  // span. A cleanup pass has to be as narrow as the mess it is cleaning.
  if (dropped) {
    out = out.replace(_VD_DEAD_SPAN, '')          // `background-color: ` with no value
             .replace(/``/g, '')                  // an emptied code span
             .replace(/,\s*,/g, ',')
             .replace(/\(\s*,\s*/g, '(')
             .replace(/,\s*\)/g, ')')
             .replace(/\s{2,}/g, ' ')
...
// Applied to the whole prompt, so a section that formats its own values still
// cannot emit an unresolved one.
function resolvePromptValues(lines, model) {
  return lines.map(l => _vdResolveLine(l, model));
}

// ── image prompt (§4.6) ───────────────────────────────────────────────────
// The brief for an estimated model. Same voice as the page prompt, but every
// value is introduced as an estimate and the sections a still image cannot
// support — motion, states, anatomy, layout, spacing — are not written at
// all, rather than written empty.
function buildImagePrompt(model) {
  if (!model || model.sourceType !== 'image') return '';
  ...
  lines.push('Analysis confidence: Estimated (from a static image)');
  lines.push('');
  lines.push('> This is a style direction inferred from an image, not a measured system; treat values as starting points.');
  lines.push('> Nothing below was measured. Do not present these values as tokens without confirming them against a real screen.');
  lines.push('');
  lines.push(`Inspired by: ${name}`);
  lines.push('Source type: image' + (model.imageKind ? ` (${model.imageKind})` : ''));
  if (noInterface) lines.push('This image contains no interface; the direction covers palette, motif and texture only.');
  lines.push('');

  lines.push('## Style direction (estimated)');
  const bits = [];
  ...
  if (model.mood && model.mood.words.length) lines.push(`- Mood: ${model.mood.words.join(', ')}`);
  if (model.illustration) lines.push(`- Illustration / texture: ${lacking('illustration') ? NO_EVIDENCE : model.illustration.style}`);
  lines.push('- Hierarchy, in order: ' + (lacking('hierarchy') ? NO_EVIDENCE
    : (model.hierarchy || []).map((h, i) => `${i + 1}) ${h}`).join('; ')));
  lines.push('');

  lines.push('## Effects (estimated)');
  if (lacking('effects')) lines.push(`- ${NO_EVIDENCE}`);
  else (model.effects || []).forEach(e => lines.push(
    `- ${e.type}${e.where ? ' at ' + e.where : ''}${e.colour ? ' ' + e.colour : ''}${e.strength ? ' (' + e.strength + ')' : ''}${e.description ? ': ' + e.description : ''}`));
  lines.push('');

  if (roles.length) {
    lines.push('## Palette (estimated roles)');
    roles.forEach(k => lines.push(`- ${k}: ${colors[k]} (est.)`));
    lines.push('');
  }

  if (!noInterface && hasText) {
  lines.push('## Typography (estimated)');
  if (dir.classification) lines.push(`- Classification: ${dir.classification}`);
  if (dir.weightCharacter) lines.push(`- Weight character: ${dir.weightCharacter}`);
  const read = fonts.filter(f => !f.suggested).map(f => f.family);
  const sugg = fonts.filter(f => f.suggested).map(f => f.family);
  if (read.length) lines.push(`- Legible in the image: ${read.join(', ')}`);
  if (sugg.length) lines.push(`- Suggested open fonts (suggested, not observed): ${sugg.join(', ')}`);
  lines.push('- No type scale: sizes cannot be read from an image. Choose one to match the classification and density.');
  lines.push('');
  }

  lines.push('## Container, shape and iconography (estimated)');
  const ct = model.container || {};
  if (noInterface) { /* a pattern, illustration or photo has no container */ }
  else if (lacking('container')) lines.push(`- Container: ${NO_EVIDENCE}`);
  else lines.push(`- Container: ${[ct.shape, ct.radiusEstimate && 'radius ' + ct.radiusEstimate, ct.surface, ct.border && ct.border + ' border'].filter(Boolean).join(', ')}`);
  if (model.shape && model.shape.language) lines.push(`- Shape language: ${model.shape.language}`);
  if (lacking('iconography')) lines.push(`- Icons: ${NO_EVIDENCE}`);
  else if (ic.style) lines.push(`- Icons: ${[ic.style, ic.weight, ic.corners, ic.grid && ic.grid + ' grid'].filter(Boolean).join(', ')}`);
  lines.push('');

  lines.push('## Not included');
  lines.push('Motion, interaction states, component anatomy, layout and spacing are omitted: a static image does not contain them. Analyze the live site to measure them.');
  lines.push('');

  lines.push('## Rules');
  lines.push('- Use the palette roles and type classification as defaults and say in your output that they are estimates.');
  lines.push('- Do not invent a type scale, spacing scale, shadows, motion or states from this brief.');
  lines.push('- A measured DESIGN.md for the same product overrides every value here.');
  return lines.join('\n');
}
...
function buildPagePrompt(data, aiDirection) {
  ...
  // Emits a standard low-confidence blockquote so the LLM applies fallback defaults
  // instead of writing its own "verify with DevTools" disclaimer in generated code.
  const _lowConf = (level, label, hint) => {
    if (level !== 'low') return;
    lines.push(`> ⚠ ${label} confidence: low — ${hint}`);
  ...
  // Pre-warm the primary resolver so data._primaryColorAmbiguous is populated before
  // the warning check runs. The resolver is idempotent — later callsites get the same result.
  resolvedPrimaryActionColor(data, style);
  if (data._primaryColorAmbiguous) {
    const { button_primary, css_var_primary, frequency_primary } = data._primaryColorCandidates || {};
    lines.push('');
    lines.push(`> ⚠ **Primary action color could not be determined confidently.** Three extraction signals disagreed: button says \`${button_primary || 'none'}\`, CSS var says \`${css_var_primary || 'none'}\`, frequency says \`${frequency_primary || 'none'}\`. The color below is a best guess — verify against the live site before shipping.`);
  }
  ...
  // The former ### Design Brief section was removed — its 4 bullets (personality,
  // visual strategy, motion, constraints) repeated the same 8 axes already covered
  // in depth by Design Direction + Core Spec + Motion Tokens. Personality + visual
  // strategy are now inlined as a TL;DR at the top of Design Direction below.

  // P0 core spec — highest-priority, conflict resolver for downstream builders
  const _p0Primary = resolvedPrimaryActionColor(data, style).color;
  ...
  if (_p0Bg) {
    lines.push(`- Base background: \`${_p0Bg}\``);
    // Hard-enforce: LLMs (v0, Bolt, Cursor) default to framework neutrals when
    // uncertain — bg-white, bg-background, slate-50, shadcn --background. Name them
    // explicitly so the model cannot fall back without violating the spec.
    lines.push(`  Use ONLY this exact value for the page/body background. Do NOT substitute with \`bg-white\`, \`bg-background\`, \`bg-slate-50\`, \`bg-zinc-50\`, shadcn \`--background\` default, or any other neutral. The entire page renders on this color.`);
  }
  if (_p0Primary) {
    lines.push(`- Primary action color: \`${_p0Primary}\``);
    lines.push(`  Use ONLY this exact hex for primary CTAs, buttons, focus rings, and active states. Do NOT substitute with Tailwind \`blue-500\`, \`indigo-600\`, \`violet-600\`, shadcn \`--primary\` default, or any other color. If a component you generate doesn't reference this token, the component is wrong.`);
  }
  ...
  // TL;DR replaces the former ### Design Brief section — two lines of high-signal
  // summary (personality + visual strategy) at the top of Direction give the LLM the
  // same orientation without duplicating the 8-axis narrative that follows.
  if (focus === 'all') {
    const _tldrPersonality = generateBriefPersonality(data, style);
    const _tldrVisual = generateBriefVisualStrategy(data, style, vpr);
    // generateBriefPersonality returns comma-joined words ("professional, brutalist, …")
    // without trailing punctuation. generateBriefVisualStrategy returns a full sentence.
    // Add an explicit period so the concatenation reads as two sentences, not a run-on.
    lines.push(`**TL;DR:** ${_tldrPersonality}. ${_tldrVisual}`);
    lines.push('');
  }
  ...
  if(focus==='all'||focus==='colors') {
    ...
    _lowConf(conf.colors, 'Color', 'site may use CSS-in-JS or dynamic theming.');

    // All named CSS color vars — noise filter is authoritative (see lib/noise-filter.js)
    const namedVars = Object.entries(vars).filter(([k,v]) =>
      /^#[0-9a-f]{3,8}$/i.test(v.trim()) && !isNoisyVar(k)
    );
    ...
    const remainingAccents = dedupeHexStrings([...accents, ...style.vibrantColors].filter(c=>hexSat(c)>=22))
    ...
    // Implicit bg for downstream filters — when style.pageBg is null, `bgColor?.toLowerCase()`
    // would be undefined and every `!== undefined` check passes, leaking the true page
    // background into fg/surface pools. Fall back to the isDark-appropriate extreme so
    // the filter still screens out white-on-light / black-on-dark leakage.
    const _bgForFilter = (bgColor || (style.isDark ? '#000000' : '#ffffff')).toLowerCase();
    ...
    const surfaceColor = allComputedColors.find(c => {
      ...
      if (c.toLowerCase() === _bgForFilter) return false;
    ...
    // Tonal scale from primary action color — Tailwind-compatible 50-900
    if (primaryColor && hexSat(primaryColor) > 15) {
      const scale = generateTonalScale(primaryColor);
      if (scale) {
        lines.push('');
        lines.push('Brand color scale (use in Tailwind config):');
        for (const [step, hex] of Object.entries(scale)) {
          const marker = step === '500' ? ' <- extracted primary' : '';
          lines.push(`- brand-${step}: \`${hex}\`${marker}`);
        }
      }
    }
  ...
  if(focus==='all'||focus==='typography') {
    ...
    if(fonts.length>0||fontVars.length>0) {
      ...
      _lowConf(conf.typography, 'Typography', 'heading metrics may be inferred from non-semantic elements (Framer RichText, generic divs).');
  ...
  if(focus==='all'||focus==='shadows') {
    ...
    if(shadowVars.length>0||shadows.length>0) {
      ...
      _lowConf(conf.shadows, 'Shadow', 'site may use borders or flat-design elevation instead of drop shadows.');
      const _shadowRoles = inferShadowRoles(data);
      if(shadowVars.length>0) shadowVars.slice(0,5).forEach(([k,v])=>lines.push(`- \`${k}\`: ${v}`));
      else shadows.slice(0,4).forEach((s,i)=>{
        // Delegates to lib/shadow-utils.js — same logic as content.js uses during
        // extraction, no more drift between render-side and extraction-side parsers.
        const realLayers = splitShadowLayers(s).filter(isRealShadowLayer);
        ...
        if (realLayers.length > 0) {
          ...
          // Prefer a semantic role label (card-resting, button-primary, card-hover)
          // when the shadow string maps to an extracted component; fall back to
          // elevation ordinal (sm/md/lg/xl) otherwise.
          const _role = _shadowRoles.get(fullShadow.replace(/\s+/g, ' ').trim());
          const label = _role ? `shadow-${_role}` : `shadow-${['sm','md','lg','xl'][i]||i}`;
          lines.push(`- ${label}${typeNote}: \`${fullShadow}\``);
        }
      });
      lines.push('');
    }
  ...
  if(focus==='all'||focus==='components'||focus==='shadows') {
    const radiiVars=Object.entries(vars).filter(([k])=>
      /radius|rounded/.test(k) && !isNoisyVar(k)
    );
  ...
  if (focus === 'all' || focus === 'layout') {
    ...
    if (spScale) {
      ...
      lines.push(`Base unit: **${spScale.baseUnit}px** (${spScale.conformRate}% of values conform)`);
      lines.push(`Scale: ${spScale.commonValues.map(v => '`' + v + 'px`').join(' · ')}`);
      lines.push(`Use multiples of ${spScale.baseUnit}px for ALL spacing — padding, gap, margin.`);
      if (spScale.outliers.length) {
        lines.push(`Exceptions: ${spScale.outliers.map(v => v + 'px').join(', ')} — use as-is where detected.`);
      }
      lines.push('');
    }
  }
  ...
  if(focus==='all'||focus==='motion') {
    ...
    if(motionVars.length>0||transitions.length>0||animations.length>0||hoverStates.length>0) {
      if (focus === 'motion') {
        ...
        lines.push('');
        _lowConf(conf.motion, 'Motion', 'few transitions extracted and no animation library detected — site may rely on scroll libraries or runtime JS.');
        ...
        // 1. Interaction patterns — hover states grouped by component type (shared helper)
        const compLines = renderGroupedHoverStates(hoverStates, data, vars, transitions);
        ...
        // Hover states grouped by component (same shape as focus='motion' — consistent
        // across modes, collapses what used to be a scattered flat listing)
        const _allCompLines = renderGroupedHoverStates(hoverStates, data, vars, transitions);
        if (_allCompLines.length > 0) {
          lines.push('**Interaction patterns:**');
          _allCompLines.forEach(l => lines.push(l));
        }
        if(animations.length>0) {
  ...
  if (focus === 'all' || focus === 'components') {
    const ico = data.iconographySystem;
    if (ico && (ico.svgIcons?.count > 0 || ico.iconFonts?.length > 0 || ico.decorativeElements?.length > 0)) {
      lines.push('### Iconography & Visual System');
      if (ico.system) lines.push(`Icon system: ${ico.system}`);
      if (ico.svgIcons?.count > 0) {
        lines.push(`Inline SVG icons: ${ico.svgIcons.count} detected`);
        if (ico.svgIcons.dominantSizes?.length)
          lines.push(`- Size: ${ico.svgIcons.dominantSizes.join(', ')}`);
        if (ico.svgIcons.style)
          lines.push(`- Style: ${ico.svgIcons.style}${ico.svgIcons.strokeToFillRatio ? ' (' + ico.svgIcons.strokeToFillRatio + ')' : ''}`);
        if (ico.svgIcons.strokeWidths?.length)
          lines.push(`- Stroke weight: ${ico.svgIcons.strokeWidths.join(', ')}`);
        if (ico.svgIcons.accentColors?.length)
          lines.push(`- Icon accent colors: ${ico.svgIcons.accentColors.map(c => '`' + c + '`').join(', ')}`);
      }
      if (ico.iconFonts?.length)
        lines.push(`Icon library: ${ico.iconFonts.join(', ')}`);
      if (ico.decorativeElements?.length > 0) {
        lines.push(`Decorative SVG elements: ${ico.decorativeElements.length}`);
        ico.decorativeElements.forEach(el => {
          const flags = [
            el.hasGradients ? `${el.gradientCount} gradient(s)` : null,
            el.hasAnimations ? 'animated' : null,
          ].filter(Boolean).join(', ');
          lines.push(`- ${el.size}px in ${el.context}${flags ? ' [' + flags + ']' : ''}`);
        });
      }
      lines.push('');
    }
  ...
  if ((focus === 'all' || focus === 'layout') && data.breakpoints && data.breakpoints.length > 0) {
    ...
    data.breakpoints.forEach(bp => {
      const px = typeof bp === 'object' ? bp.px : bp;
      const ruleCount = typeof bp === 'object' ? bp.ruleCount : 0;
      const twMatch = TAILWIND_MAP.reduce((best, [twPx, name]) =>
        Math.abs(px - twPx) < Math.abs(px - best[0]) ? [twPx, name] : best, [9999, 'custom']);
      const twLabel = Math.abs(px - twMatch[0]) <= 32 ? `≈ Tailwind \`${twMatch[1]}:\`` : 'custom breakpoint';
      const rulesNote = ruleCount > 0 ? ` (${ruleCount} rules)` : '';
      lines.push(`- ${px}px — ${twLabel}${rulesNote}`);
    });
    lines.push('');
  }
  ...
  if(focus==='all') {
    ...
    if (scm.length > 0) {
      ...
      // Hard count + order lock — LLMs (v0, Bolt) default to "common SaaS shape"
      // (hero → features → testimonials → pricing → FAQ → CTA → footer) and invent
      // sections when unsure. Pin the exact count and forbid common additions by name.
      lines.push(`**This page has exactly ${scm.length} sections in the order listed below.** Do NOT add sections (no invented testimonials, pricing, FAQ, newsletter, team, stats, or logo-wall blocks unless explicitly listed below). Do NOT omit or reorder sections. Each section's background, layout, and content are measured from the live site:`);
      lines.push('');
  ...
  lines.push(getPlatformInstruction(site, data, _p0Primary));
  // Every value passes through the design model on the way out.
  const _cleaned = cleanPromptLines(resolvePromptValues(lines, _vdModel));
  const _sectionTrimmed = trimSectionContentMap(_cleaned);
  const _trimmed = trimPagePromptByPriority(_sectionTrimmed, getMaxPromptLines(focus));
...
function setOutputMode(mode) {
  ...
  try {
    if (mode === 'system') {
      ...
      // Pass the stored AI direction, not null. Rebuilding with null silently
      // downgraded an AI-enhanced prompt to the rule-based one as soon as the
      // user visited Global Tokens and came back.
      state.lastPrompt = buildPagePrompt(data, state.lastAiDirection);
    }
  } catch(err) {
...
function buildDesignSystemPrompt(data, style) {
  const site = safeHostname(data.url), vars = data.cssVars || {};
  const specsData = getDesignSpecsData(data, style);
  // Global Tokens is a separate entry point, so it needs the same resolution
  // pass — an unresolved var() is no more useful here than in a page prompt.
  const _vdModel = _vdModelFor(data);
  const lines = [];
  ...
  lines.push('## Color Tokens');
  const namedVars = Object.entries(vars).filter(([k,v]) => /^#[0-9a-f]{3,8}$/i.test(v.trim()) && !isNoisyVar(k));
  if (namedVars.length > 0) namedVars.slice(0, 14).forEach(([k, v]) => lines.push('- `' + k + '`: `' + v + '`'));
  const _resolvedPrimary = resolvedPrimaryActionColor(data, style).color;
  if (_resolvedPrimary) lines.push('- primary-action: `' + _resolvedPrimary + '`');
  ...
  if (shadows.length > 0) {
    lines.push('## Shadow Tokens');
    const _roles = inferShadowRoles(data);
    shadows.slice(0,4).forEach((s,i) => {
      const _r = _roles.get(s.replace(/\s+/g, ' ').trim());
      const _lbl = _r ? `shadow-${_r}` : `shadow-${['sm','md','lg','xl'][i]}`;
      lines.push(`- ${_lbl}: \`${s}\``);
    });
    lines.push('');
  }
  ...
  return resolvePromptValues(lines, _vdModel).join('\n');
}

function buildElementPrompt(data) {
  const site=safeHostname(data.url),s=data.styles||{},lines=[],elType=guessElementType(data);
  ...
  lines.push(getPlatformInstruction(site, data, null));
...
function getPlatformInstruction(site, data, resolvedPrimary) {
  const _vdModel = _vdModelFor(data);
  ...
  // Build per-font weight map from typography patterns, capped at 4 most relevant weights
  const _fontWeightMap = {};
  const tp = data?.typographyPatterns || {};
  for (const role of Object.values(tp)) {
    if (role?.fontFamily && role?.fontWeight) {
      const fam = role.fontFamily.toLowerCase();
      if (!_fontWeightMap[fam]) _fontWeightMap[fam] = new Set();
      _fontWeightMap[fam].add(String(parseInt(role.fontWeight) || '400'));
    }
  }
  const _defaultWeights = (data?.fontWeights || ['400','500','600','700']).slice(0, 4);

  const fontImport = googleFonts.length > 0
    ? `\nAdd to global CSS:\n@import url('https://fonts.googleapis.com/css2?${googleFonts.map(f => {
        const mapped = _fontWeightMap[f.toLowerCase()];
        const weights = mapped && mapped.size > 0
          ? [...mapped].sort().slice(0, 4)
          : _defaultWeights;
        return `family=${encodeURIComponent(f)}:wght@${weights.join(';')}`;
      }).join('&')}&display=swap');`
    : '';
  ...
  if (data?.assets?.fonts?.length > 0) {
    ...
    if (nonGoogle.length > 0) {
      ...
      // §4: one policy, from the model. A family's HOSTING (where the file
      // came from) and its LICENCE (whether anyone may use it) are different
      // questions, and the old blanket line below got the second one wrong.
      const _vdFont = (family) => {
        if (!_vdModel || !_vdModel.fonts) return null;
        return _vdModel.fonts.find(x => x.family === family) || null;
      };
      nonGoogle.forEach(f => {
        const meta = _vdFont(f.family);
        let note = '';
        if (meta && meta.openlyLicensed) {
          note = ' — openly licensed (Google Fonts); this copy is self-hosted, '
               + 'so use either the URL or the Google Fonts release';
        } else if (meta && meta.alternative) {
          note = ` — licence unknown; if you cannot use it, ${meta.alternative}`
               + ' is a close substitute (suggested, not observed)';
        } else if (meta) {
          note = ' — licence unknown';
        }
        assetLines.push(`- "${f.family}": ${f.url}${note}`);
      });
      ...
      // The old line here read "Do not substitute with Google Fonts
      // alternatives" for EVERY family — including the ones that are on
      // Google Fonts. That contradicted both the list above and DESIGN.md.
      const _openNames = (_vdModel && _vdModel.fonts ? _vdModel.fonts : [])
        .filter(x => x.openlyLicensed && nonGoogle.some(f => f.family === x.family))
        .map(x => `"${x.family}"`);
      if (_openNames.length) {
        assetLines.push('These URLs work as-is. ' + _openNames.join(', ')
          + (_openNames.length === 1 ? ' is' : ' are')
          + ' also on Google Fonts, so either source is fine for '
          + (_openNames.length === 1 ? 'it' : 'them')
          + '. For the rest, use the URL above rather than guessing a lookalike.');
      } else {
        assetLines.push('These URLs work as-is. Use them rather than guessing a lookalike; where a family is marked "licence unknown" above, the labelled substitute is the safe alternative.');
      }
    }
  }
  ...
  let shadcnBlock = '';
  {
    ...
    // Use the same resolved primary-action as Core Spec to avoid conflicts
    const _isOpaque6 = c => c && /^#[0-9a-f]{6}$/i.test(c.trim());
    const primaryHex = (_isOpaque6(resolvedPrimary) ? resolvedPrimary : null)
      || (_isOpaque6(data?.buttonStyles?.primary?.backgroundColor) ? data.buttonStyles.primary.backgroundColor : null)
      || (_isOpaque6(sc.primary) ? sc.primary : null)
      || '#000000';
    const primLum = hexLum(primaryHex);
    const primFg = primLum > 0.5 ? '#000000' : '#ffffff';

    const radii = data?.borderRadii || [];
    // §2: shadcn's --radius drives every component it generates, so a value
    // that belongs to neither the buttons nor the cards mis-shapes the whole
    // build. rig.ai was emitting 6px — a radius that is on the page somewhere
    // but on none of its components. The model answers by role.
    const _vdModelRadius = (_vdModel && _vdModel.radius) ? _vdModel.radius : null;
    const radiusVal = _vdModelRadius
      ? (_vdModelRadius.card || _vdModelRadius.button || '0px')
      : (radii.find(r => r !== '9999px' && r !== '50%') || '0px');
  ...
  // Stack detection line
  const _fw = data?.frameworkDetection;
  const _cssF = _fw?.cssFrameworks || [];
  const _jsF = _fw?.jsFrameworks || [];
  let stackNote = '';
  if (_jsF.includes('nextjs') && _cssF.includes('tailwind')) {
    stackNote = '\n- Detected stack: Next.js + Tailwind CSS — use utility classes and override tailwind.config with extracted tokens.';
  } else if (_jsF.includes('nextjs')) {
    stackNote = '\n- Detected stack: Next.js — use CSS modules or styled-components with extracted tokens.';
  } else if (_cssF.includes('tailwind')) {
    stackNote = '\n- Detected stack: Tailwind CSS — use utility classes and override tailwind.config with extracted color tokens.';
  } else if (_cssF.includes('bootstrap')) {
    stackNote = '\n- Detected stack: Bootstrap — override Bootstrap SCSS variables or CSS custom properties with extracted tokens.';
  } else if (_cssF.includes('chakra-ui')) {
    stackNote = '\n- Detected stack: Chakra UI — use extendTheme() with extracted color and spacing tokens.';
  } else if (_cssF.includes('material-ui')) {
    stackNote = '\n- Detected stack: Material UI — use createTheme() with extracted palette and typography tokens.';
  }
...
- When a value is not specified in this prompt, a sensible default is acceptable.${stackNote}${fontImport}${customFontNote}${assetSection}${shadcnBlock}`;
}
