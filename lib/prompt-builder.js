...
function generateComponentGuidance(data, style, specsData) {
  ...
  if (bs.secondary && bs.secondary !== bs.primary && bs.secondary !== bs.ghost) {
    ...
    const secShape = s.clipPath ? 'chamfered via clip-path' : `\`${s.borderRadius||defaultRadius}\` radius`;
    ...
    let secDesc = `${secShape}, \`${s.backgroundColor||'muted'}\` bg, text \`${s.color||'inherit'}\`, padding \`${s.padding}\``;
  ...
  if (inputData && Object.keys(inputData).length > 0) {
    ...
    lines.push(`**Inputs:** \`${inputData.backgroundColor||'inherit'}\` bg, \`${inputData.border||'1px solid '+(isDark?'rgba(255,255,255,0.12)':'#e0e0e0')}\`, \`${inputData.borderRadius||defaultRadius}\` radius. Focus: ${inputFocusDesc}.`);
  }
