...
function resolvedPrimaryActionColor(data, style) {
  ...
  // Priority 1: button background — the MOST RELIABLE source because it's directly extracted
  // from the computed style of the actual primary CTA button on the page.
  // Any hex within visible luminance range is a valid CTA color.
  if (isPlausibleHexColor(btnBg) && hexLum(btnBg) > 0.03 && hexLum(btnBg) < 0.97) return btnBg;

  // Priority 2: explicit semantic CSS var (--primary, --cta, --brand)
  // Less reliable than button bg because CSS vars can refer to accent/status colors, not CTA bg.
...
function generateRuleBasedDirection(data, style) {
  ...
  // ── Framer-specific animation note ──
  if (data.frameworkDetection?.isFramer) {
    lines.push('**Framer animation note:** Many micro-interactions and component transitions on this site are driven by Framer\'s internal animation engine — they are NOT extractable as CSS @keyframes. The motion tokens above represent only CSS-level transitions.');
    lines.push('  When implementing, preserve the *timing personality* even without the exact Framer animations:');
    lines.push('  - All interactions feel decisive: snappy (150ms) on hover/click, never lingering');
    lines.push('  - Scroll-triggered elements appear immediately when in viewport — no slow-build entrance');
    lines.push('  - Hover = color/opacity shift only — no transforms, no shadow lift unless explicitly noted');
    lines.push('  **Principle:** If an animation could be described as "playful" or "delightful", it is wrong for this brand. All motion should feel like instrument feedback — precise, immediate, unembellished.');
    lines.push('');
  }
...
function generateComponentGuidance(data, style, specsData) {
  ...
  if (data.stickySections?.length > 0) {
    data.stickySections.forEach(sec => {
      ...
      if (sec.type === 'sticky-tab-scroll' && sec.tabLabels) {
        ...
        lines.push(`  This is NOT a standard tab component — it is a scroll-driven tour that forces sequential feature discovery. Clicking a tab or scrolling to its section reveals the corresponding right-side content panel. The user cannot skip; they must scroll through every feature.`);
        lines.push(`  The left sticky panel stays locked while the right column scrolls normally. As each right-side content block enters the viewport, the corresponding left tab becomes active (highlight, dot indicator, or border-left shift). Implement with IntersectionObserver on right panels driving active tab state.`);
        lines.push(`  Design intent: the tension between a minimal left navigator and rich right content IS the UX — the sparsity of the left forces the eye right. Do NOT add right-side navigation, duplicate CTAs, or additional structure that breaks the guided single-column flow.`);
      } else {
        lines.push(`**Sticky-scroll section:** Left column \`position:sticky; top:0; height:${sec.isFullViewportHeight ? '100vh' : 'tall'}\` ${stickyContent}. Right column scrolls normally with ${sec.scrollBlockCount} content blocks.`);
        if (sec.scrollBlockHeadings?.length > 0) {
          lines.push(`  Content blocks: ${sec.scrollBlockHeadings.filter(Boolean).map(h => `"${h}"`).join(' → ')}`);
        }
        lines.push(`  Left: anchor visual context while right reveals sequential content. Neither column is the "main" content — they're dialogue partners.`);
      }
      lines.push(`  Implementation: left col = \`<div class="sticky top-0 h-screen overflow-hidden">\`. Right col = normal flow div. Wrap both in a flex/grid container.`);
    });
  }
  ...
  if (data.svgDiagramAnimations?.length > 0) {
    data.svgDiagramAnimations.forEach(diag => {
      if (diag.type === 'svg-path-animation') {
        ...
        lines.push(`  This diagram REPLACES body copy entirely — the user reads the system by watching it operate. It is the highest-trust visual device on the page.`);
        lines.push(`  Animated dots (8–12px circles) follow \`<animateMotion path="...">\` paths tracing rails and connections. Each dot color matches its category accent. Connection lines: 1px, extending beyond the frame width to imply infinite continuity.`);
        lines.push('  Event labels: small pill-shaped chips (border-radius: 100px) suspended by thin vertical connector lines above/below the rails — like commit markers on a git branch visualization.');
        lines.push(`  Do NOT simplify to a static diagram — the motion IS the message. If SMIL is not feasible, use Framer Motion or GSAP to animate dot position along a path.`);
      } else if (diag.type === 'pill-track-diagram') {
        const pillList = diag.pills.map(p => `"${p.text}" (${p.w}×${p.h}px, bg:${p.bg})`).join(', ');
        lines.push(`**Pill-track diagram — product architecture visualization:** ${pillList}.`);
        lines.push(`  Large oval pill rails represent organizational pillars (Science · Operations · Finance or equivalent). Animated dots traveling along connecting paths between the pills represent live data signals.`);
        lines.push(`  Pill implementation: \`border-radius: 9999px\`, 1px stroke border, subtle fill or transparent. Text centered inside. Pill widths are large (200–500px) relative to height.`);
        lines.push(`  Horizontal connection lines run the full width of the diagram at pill mid-height — they represent "tracks" on which events occur. Dots animate along these tracks left-to-right (or bidirectional).`);
        lines.push('  Event cards/chips: pill-shaped (border-radius: 100px) with text ("Unexpected tox", "CRO delay", etc.) appear above/below with a 1px vertical connector line. These are NOT tooltips — they are permanently visible annotations.');
  ...
  if (data.sectionBackgroundDecorations?.length > 0) {
    ...
    lines.push('  These decorations exist below the content layer (z-index:0 or lower). They are NOT optional — without them, solid-color sections feel sterile. Their purpose: give each section a spatial identity without photography.');
    data.sectionBackgroundDecorations.forEach(sec => {
      sec.decorations.forEach(d => {
        const base = `\`position:absolute; inset:0; z-index:0; pointer-events:none; overflow:hidden\`. opacity: ${d.opacity}.`;
        if (d.type === 'radial-rays') {
          const colorNote = d.colors.length > 0 ? ` Colors: ${d.colors.join(', ')}.` : '';
          lines.push(`  - **Radial rays** (on ${sec.sectionBg || 'page bg'}): ${d.pathCount} lines radiating from a convergence point toward viewport edges.${colorNote} ${base}`);
          lines.push(`    Design intent: creates centrality and focus on dark sections — a sense of authority emanating from a single point. Authority/focus metaphor.`);
          lines.push(`    Implementation: inline SVG with \`<line>\` elements from center point (e.g. 50% 110%) to viewport edges, OR \`conic-gradient\` at very low opacity (0.06–0.12). Stroke-only, no fill.`);
        } else if (d.type === 'grid-lines') {
          const colorNote = d.colors.length > 0 ? ` Colors: ${d.colors.join(', ')}.` : '';
          lines.push(`  - **Grid lines** (on ${sec.sectionBg || 'page bg'}): ${d.lineCount} structural lines.${colorNote} ${base} 1px stroke, structural precision aesthetic.`);
        } else if (d.type === 'gradient-decoration') {
          lines.push(`  - **Gradient decoration** (on ${sec.sectionBg || 'page bg'}): low-opacity gradient overlay — \`${(d.gradient || '').slice(0, 80)}\`. opacity: ${d.opacity}. ${base}`);
          lines.push(`    Implementation: \`position:absolute; inset:0; background: ${(d.gradient || 'inherit').slice(0, 80)}; pointer-events:none; z-index:0;\``);
        } else {
          const colorNote = d.colors.length > 0 ? ` Colors: ${d.colors.join(', ')}.` : '';
          lines.push(`  - **Organic curves** (on ${sec.sectionBg || 'page bg'}): ${d.pathCount} large-radius arcs suggesting cellular/molecular paths.${colorNote} ${base}`);
          lines.push(`    Implementation: SVG with cubic bezier \`<path>\` elements sweeping across the section.`);
        }
      });
    });
    lines.push('  **Rule:** if the decoration draws attention, it is too strong — reduce opacity. If the section feels like a plain flat color, the decoration is missing. Target opacity: 0.06–0.15.');
  }
  ...
  if(focus==='all') {
    ...
    if (scm.length > 0) {
      ...
      scm.forEach((sec, i) => {
        ...
        // Sidebar-switcher / feature-showcase section details with composite content
        if (secType === 'sidebar-switcher' || secType === 'feature-showcase') {
          if (secType === 'sidebar-switcher') {
            lines.push(`  Layout: sidebar-switcher — left sidebar with navigation items, right panel shows corresponding content.`);
            lines.push(`  Left sidebar (~280px, sticky or fixed height): clickable nav items. Active item has visual highlight (background shift + left border indicator).`);
            lines.push(`  Right panel: content swaps on click/tab change. Implement with \`useState(activePanel)\` — each nav item shows a different feature panel.`);
          } else {
            lines.push(`  Layout: feature-showcase — numbered feature section with multiple panels.`);
          }
          // Output composite sub-content if available
          if (sec.compositeContent) {
            const cc = sec.compositeContent;
            if (cc.tabLabels?.length > 0) {
              lines.push(`  Tab/nav items: ${cc.tabLabels.map(t => `"${t}"`).join(' · ')}`);
            }
            if (cc.subHeadings?.length > 0) {
              lines.push(`  Feature panels: ${cc.subHeadings.map(h => `"${h}"`).join(' · ')}`);
            }
            if (cc.featureCtas?.length > 0) {
              lines.push(`  Per-panel CTAs: ${cc.featureCtas.map(c => `"${c}"`).join(' · ')}`);
            }
            if (cc.subDescriptions?.length > 0) {
              lines.push(`  Panel descriptions: ${cc.subDescriptions.map(d => `"${d.slice(0, 50)}..."`).join(' | ')}`);
            }
          }
    ...
    // ── Additional detected sections not in the main section map ──
    // Pill-track diagrams and sticky-scroll panels are detected separately
    // and may represent sections that extractSectionContentMap() missed (e.g. Framer nesting)
    if (data.svgDiagramAnimations?.some(d => d.type === 'pill-track-diagram' || d.type === 'svg-path-animation')) {
      const _pillDiag = data.svgDiagramAnimations.find(d => d.type === 'pill-track-diagram' || d.type === 'svg-path-animation');
      // Only add if not already described in a section visual
      const _alreadyInSection = scm.some(s => s.visualDescriptions?.some(v => /pill-track|animateMotion/i.test(v)));
      if (!_alreadyInSection) {
        lines.push('**Additional section: Product architecture diagram**');
        if (_pillDiag.type === 'pill-track-diagram') {
          lines.push(`  Pill-track diagram with ${_pillDiag.pills?.length || 0} category pills: ${(_pillDiag.pills || []).map(p => `"${p.text}"`).join(' · ')}.`);
          lines.push('  Large oval rails with animated dots traveling between them. See Component Patterns > Pill-track diagram for implementation spec.');
        } else {
          lines.push(`  SVG path animation (${_pillDiag.width}×${_pillDiag.height}px, ${_pillDiag.pathCount} paths). See Component Patterns > SVG path animation for implementation spec.`);
        }
        lines.push('');
      }
    }
    if (data.stickySections?.length > 0) {
      const _stickyInSection = scm.some(s => s.layout === 'sticky-scroll' || (s.visualDescriptions || []).some(v => /sticky/i.test(v)));
      if (!_stickyInSection) {
        data.stickySections.forEach(ss => {
          if (ss.type === 'sticky-tab-scroll' && ss.tabLabels?.length > 0) {
            lines.push(`**Additional section: Sticky-scroll tab panel**`);
            lines.push(`  Tabs: ${ss.tabLabels.map(t => `"${t}"`).join(' · ')}. See Component Patterns > Sticky-scroll tab section for implementation spec.`);
            lines.push('');
          }
        });
      }
    ...
    // ── Contact / closing CTA section + footer content ──
    if (data.contactSection) {
      const cs = data.contactSection;
      const _sectionLabel = cs.type === 'closing-cta' ? 'Closing CTA section' : 'Contact section';
      let contactLine = `**${_sectionLabel}**`;
      ...
      if (cs.hasBgDecoration) contactLine += ' Has background SVG decoration (radial rays or similar — see Section background decorations above).';
      lines.push(contactLine);
      if (cs.ctas?.length > 0) lines.push(`  CTAs: ${cs.ctas.map(c => `"${c}"`).join(', ')}`);
      if (cs.type === 'closing-cta') {
        lines.push('  This is the page\'s final persuasion — large display heading + single prominent CTA. Treat as a full-viewport closing statement.');
      }
      lines.push('');
    }
