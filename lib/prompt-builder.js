...
function buildInteractionParadigm(data, vpr) {
  ...
  const _hasHeroEntrance = !!(data.heroEntranceSequence || ap.hasTextReveal || ap.hasHeroAnimation || (data.rotatingText && data.rotatingText.length > 0));
...
function buildPagePrompt(data, aiDirection) {
  ...
  if(focus==='all') {
    ...
    if (scm.length > 0) {
      ...
      lines.push('');
      // Determine entrance animation type for sections — revealStyle-aware with failsafe
      const sap = vpr.animationPatterns || {};
      const _mp = data.motionProfile || {};
      const _revealType = _mp.revealStyle || 'fade-up';
      let entranceAnim;
      if (_revealType === 'fade-left-or-right') {
        entranceAnim = 'Entrance: Section headings/eyebrow use blur-fade reveal on scroll entry.\n  `.reveal-text { opacity:0; translate:-24px 0; transition: opacity 0.5s cubic-bezier(0.19,1,0.22,1), translate 0.5s cubic-bezier(0.19,1,0.22,1); }`\n  `.reveal-text.is-visible { opacity:1; translate:0; }` H2 delay: 0.1s after eyebrow.\n  Include mandatory 2s timeout failsafe (see Interaction Paradigm).';
      } else if (_revealType === 'clip-path-reveal' || sap.hasMaskReveal) {
        entranceAnim = 'Entrance: `clip-path: inset(100% 0 0 0)→inset(0); transition: 0.6s ease-out` on scroll into view.\n  Include mandatory 2s timeout failsafe.';
      } else if (sap.hasStaggerReveal) {
        entranceAnim = 'Entrance: children stagger in with `opacity: 0→1; translateY(20px)→0`, 80ms delay between each, 400ms ease-out. Trigger on scroll via IntersectionObserver.\n  Include mandatory 2s timeout failsafe.';
      } else {
        entranceAnim = 'Entrance: `opacity: 0→1; transform: translateY(20px)→0; transition: 0.4s ease-out` on scroll into view (IntersectionObserver).\n  Include mandatory 2s timeout failsafe.';
      }
      ...
      scm.forEach((sec, i) => {
        ...
        if (secType === 'portfolio-split' && sec.portfolioGridInfo) {
          ...
          lines.push(`  Container: \`border-top: 1px solid rgba(255,255,255,0.08); border-left: 1px solid rgba(255,255,255,0.08)\`.`);
          lines.push(`  Each cell: \`border-right: 1px solid rgba(255,255,255,0.08); border-bottom: 1px solid rgba(255,255,255,0.08); border-radius: 0\`.`);
          lines.push(`  This creates a continuous grid-line table effect. Do NOT use card-style borders with gap.`);
          lines.push(`  Cell content: logo centered, min-height: 120px. Hover: \`background: #232529\`, 150ms.`);
          ...
          lines.push(`  Right panel spec: 100% height, min-height: 360px. \`background: #000000; border: 1px solid rgba(255,255,255,0.08); overflow: hidden\`.`);
          lines.push(`  CANVAS FALLBACK (required if canvas not feasible): SVG particle field on #000000.`);
        ...
        if (i === 0) {
          const _heroHasAnim = sap.hasTextReveal || sap.hasHeroAnimation || (data.rotatingText && data.rotatingText.length > 0) || data.heroEntranceSequence;
          if (_heroHasAnim) {
