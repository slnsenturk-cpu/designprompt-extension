  ...
  // ── Global interactive rules (unclaimed hover states + links + blend modes) ──
  {
    ...
    if (specsData.links) {
      ...
      if (isDark && linkColor && linkColor !== '#ffffff') {
        ...
        globalParts.push(`EXCEPTION: Navigation links (nav a, header a) use muted foreground color (opacity 0.7 default, 1 on hover). The \`${linkColor}\` link rule applies to in-content links and CTAs only — NOT nav bar links`);
      }
    }
